...
#
# ⚠️ main's catalog_rev.sql puts `rev` on tours/makers/places only, and gives
# `stops` a trigger that bumps its PARENT TOUR's rev. So the "only a STOP
# changed" case below still has to surface its tour -- that is the behaviour
...
BASE="schema.sql add_video_urls.sql add_video_role.sql add_link_pin_kind.sql
      add_link_pins.sql add_country.sql places.sql catalog_rev.sql"
