...
### Twenty-one link pins from nineteen creators, and a caption that named no place at all (branch `claude/tour-links-upload-vhsf8a`, session 132 — content)

**The owner sent 21 links — 8 TikTok, 13 Instagram.** Branch cut clean off `main` at `9080764`.
**linkPins 533 → 554 · makers 208 → 224 · tours unchanged at 1,552 · places unchanged at 80.**
Content only — no Swift, no SQL, no build. Full detail: `archive/HANDOFF-260901-2.md`.

- **✅ ALL 21 ALIVE AND PINNABLE — 0 already pinned, 0 in-batch duplicates.** No dead posts, no
  `5009` CDN refusals. **Miami, Vancouver and Cozumel are new cities; no new country.** ⚠️ **One
  link is an Instagram `/p/` post and it ships like any reel** — `derivable_embed` returns a player
  for it, unlike a **TikTok `/photo/` URL, which can never be a link pin**; do not conflate the two.
- **🔴 THE HERO AUDIT CLOSED THE ONE POST WHOSE CAPTION NAMES NOTHING AT ALL.** `@thearchitectscookbook`'s
  caption is, in full, *"Btw the marble in this place was 👌🏾👌🏾👌🏾"* — no place, no city, no country.
  The frame reads **"THIS IS OSLO'S WALKABLE ROOF"** over Snøhetta's Opera House, **whose roof is the
  white Carrara marble the caption is admiring** — the caption is only comprehensible once you have
  seen the picture. A second was identified the same way: *"a time capsule of mosaics"* in Times
  Square is **Edward Meshekoff's Mosaic Map, installed 1957**, on Broadway between 42nd and 43rd,
  its inlaid metal landmarks legible in frame. ⚠️ **It is none of the four Times Square markers the
  catalogue already holds** (an Atlas tour plus pins for Times Square, One Times Square and Roy
  Lichtenstein's *Times Square Mural*); nearest is 65 m. **All 21 heroes were opened and read against
  their captions — zero wrong subjects**, fourteen naming themselves in frame.
- **🔴 ARTHUR ASHE STADIUM TAKES FOUR COINCIDENT PINS — EXACTLY `HomeView.maxStackedPlacecards`.**
  Three name the stadium outright and the fourth is a US Open visitor guide whose frame is a match in
  a stadium bowl. All four render and all four are reachable **with no headroom**: a fifth US Open pin
  would put one permanently out of reach, invisibly. ⚠️ **`TourSetMap.maxStacked` is 3 but is NOT
  binding here** — each pin has a different maker, so no maker page stacks more than one; it would
  bind on a place page or a list, and **a place collapses its members into one capsule anyway**, which
  is the Windsor Castle lesson. **A genuine place candidate; not created.**
  - **⚠️ A SEPARATED PLACEMENT WAS CONSIDERED AND REJECTED, AND THE REASONING MATTERS.** The food pin's
    frame is demonstrably the outdoor plaza, not the stadium interior, so moving it would have been
    *more* accurate and would also have relieved the cap. But **OSM maps no "South Plaza" or "Food
    Village" by name** and every candidate point reverse-geocoded to an unnamed pedestrian way —
    inventing a coordinate to relieve a cap is exactly the manufacturing these rules forbid. All four
    sit on the stadium and the cap is flagged instead.
- **The other four multi-pin groups are subjects the owner sent twice:** Grove at Grand Bay ×3,
  Bellevue/Lockridge Library ×2, Vancouver House ×2, Banyan Tree Mayakoba ×2. **`check-place-candidates.py`
  goes 0 EXACT → 5 and 22 NEAR → 23, exiting 1** — ending the clean-exit state #683 restored. **None was
  nudged together to manufacture a group and none nudged apart to dodge the checker.** ⚠️ Its exit code
  was read **directly, not through a pipe** (the session-90 `PIPESTATUS` trap).
- **🔴 A DEFECT OF MY OWN, CAUGHT BEFORE THE COMMIT BY READING THE SEED SQL: every URL shipped
  `images//`.** The `--image-base` I passed already ended in a slash and `build_entry` appends its own
  (its selftest uses `"https://x/images"`, no trailing slash). **Both forms return 200**, so nothing
  ...
  - **⚠️ AND CLAUDE.md'S CLAIM THAT SESSION 125 ALREADY FIXED THIS IS WRONG.** That entry says the
    ...
    `twin-palms-`, `villa-e-1027-nikolamatus`). **Deliberately not fixed here** — unrelated to this
    batch and it would muddy the diff; two minutes and zero risk whenever anyone wants it.
- **⚠️ COORDINATES: 9 OF 13 SUBJECTS NAMED EXACTLY, AND THE FOUR THAT WERE NOT ARE ALL DOCUMENTED
  SHAPES.** **Arthur Ashe reverse-geocodes to "Fountain Plaza", type `pitch`** — so rather than judge it
  on distance the point was **tested against OSM way 284859611, its own stadium polygon: inside** (the
  Inter&Co / Swan House rule — *fetch the polygon, don't argue with the reverse*). **150 N Riverside**
  reverses to a financial-advisor tenant at the right address and **Oslo** to the opera's own gift
  shop — both the inner-POI case. **87th Street took three attempts**: the first derived mid-block point
  landed on **86th Street**, one block south; the shipped point reverse-verifies to *"47, East 87th
  Street"*. **The Grand Central Stones are mapped in OSM by name**, which is what sited the one pin whose
  hero shows the trailhead rather than the subject.
- **⚠️ Banyan Tree Mayakoba is 1,208 m from the existing Rosewood Mayakoba PLACE, and Xcaret 1,295 m
  from the existing Hotel Xcaret Arte pin.** Genuinely different venues inside one resort development —
  the Fernbank Museum-and-Forest shape. **Not merged; do not "tidy" them together.**
- **⚠️ THREE HAND RE-CROPS — the vertical `--focus` gap, TWELFTH batch running.** Every source is 9:16,
  so the square is width-limited and **`--focus` does nothing at all**. Re-rendered through a mirror of
  the tool's own pipeline (importing `trim_bars`, same blur/pad, same filenames): **Grove at Grand Bay
  0.14**, without which the hero is an **unidentifiable marina** with the subject not in frame at all;
  **Vancouver House 0.22**, recovering the tower's top and twist; **Banyan Tree 0.62**, recovering the
  word **MAYAKOBA**. All three *recover what the crop destroyed* rather than removing what the creator
  put there. ⚠️ **Bellevue's clipped "115 Atlantic St SW" was deliberately left** — the library's own
  name survives, so what is lost is a detail, not the subject (the California Academy rule).
- **⚠️ FIVE WEAK HEROES, FLAGGED NOT RESOLVED — a link pin re-hosts only the thumbnail, so no other
  frame exists.** **The Grand Central Stones shows the elevated 242nd Street subway platform**, the
  trailhead its caption tells you to start from, not the thirteen monoliths — the sharpest mismatch.
  **Banyan Tree `@everythingeryn` is a bathroom vanity** with nothing identifying it as the hotel (the
  weaker of that pair; the other names itself). **150 N Riverside is a selfie** with the tower behind;
  **87th Street is a man on a doorstep**; **Xcaret is dominated by a swimsuit figure from behind.**
  ⚠️ The Bellevue interior is a small child from behind, unidentifiable, in the creator's own thumbnail.
- **⚠️ TWO EDITORIAL FLAGS.** **`Arthur Ashe Stadium's Renovation` is a RENDERING, not a photograph** —
  an architectural visualisation of the $800M rebuild carrying a QNS/amNY watermark, so **nothing this
  catalogue authors asserts the renovated stadium exists** (the Portsmouth Square precedent), and the
  title says "Renovation" for that reason. **`San Gervasio` is a tour operator's own promotional post**
  with a WhatsApp booking link — not marked `#AD`, but a business advertising itself (the Coca-Cola /
  German Doner Kebab class the owner has previously kept). **Creators' own errors stay theirs:** the
  Bellevue card reads **"Belleview"** while its caption spells it correctly, so the pin is titled
  correctly and the caption kept verbatim (the Schweizer convention).
- **⚠️ ARCHITECT TAGS FOLLOW THE CAPTION, AND TWO OMISSIONS ARE DELIBERATE.** `Bjarke Ingels` on the two
  Grove pins and the `@archimarathon` Vancouver House that name him; `Adjaye Associates` on all three DC
  libraries — always **alongside** `Designed by a Master`. **🔴 `Snøhetta` is NOT tagged on the Oslo Opera
  House**: that caption names no architect at all (the Jules Dalou rule), and **Snøhetta is absent from
  the vocabulary anyway** — with its two Glasshouse Theatre pins from #659 it is now among the most
  conspicuous absences. **`Bjarke Ingels` is NOT tagged on the `@allison_jodoin` Vancouver House**, whose
  caption names three estate agents and no architect — **correct by construction even though the pin
  beside it names him; do not "finish the job" here.** Verified absent and shipping the generic tag:
  `Goettsch Partners`, `Magnusson Klemencic Associates`, `Raymond Jungles`, `Edward Meshekoff`.
- **⚠️ 19 CREATORS, 16 NEW MAKER ROWS** — `@archimarathon`, `@iammikaeljohansson` and `@nerdingoutonnyc`
  already existed and the uuid5 scheme reproduced their ids **exactly**, so those merged. **All 11
  Instagram creators ship `avatarURL: null` BY DESIGN** (the embed exposes none); only the 8 TikTok
  creators get one, so 29 files cover 21 pins.
- **Verification.** Validator mirror — vocabulary parsed from **both** `Models/Tag.swift` **and** the
  Swift validator, refusing to run if they disagree or either parse is empty (they agree at **385 tags
  across 5 facets**) — **self-tested against 40 injected fault classes, 40/40 caught**, then **0 errors,
  2 warnings across 1,552 tours + 554 pins + 80 places**, **both pre-existing** (the same mirror against
  `origin/main` reports the identical pair). `make-link-pin.py --selftest` **71/71** — ⚠️ **with Pillow**;
  a fresh container has none and reports 62/62, so install it before reading that as a pass. **0**
  duplicate tour/stop/maker ids, **0** already-pinned sourceURLs (529 distinct indexed from 533 pins —
  the documented `@malata.antwerp` five-pins-one-URL case), **0** byte-duplicate heroes, **0** perceptual
  pairs even nominated at Hamming ≤ 45. **0 of 29 target paths pre-existed** — ⚠️ **one HEAD-check returned
  503, which is neither 404 nor 200**, and three retries all returned 404; reading a 503 either way is a
  false result. **The bare-slug check came back clean, so the handle suffix was not load-bearing here.**
  `Tours.json` **byte-stable under a Python re-dump before editing**; diff **1,098 insertions / 0
  deletions**. `seed_from_toursjson.py` clean at **224 / 2,106 / 2,478 / 80**. gh-pages `19d7a985`: the
  remote head was **re-read in the same command as the push**, tree diff **exactly 29 additions, 0
  deletions, nothing outside `images/`**. ⚠️ **Nothing compiled — CI on the PR is the authoritative
  validator.**

### Four place cards, and the Empire State Building moved onto its own address (branch `claude/place-cards-hereinnyc`, session 131 — content)

**Owner: *"Make place pages for Empire State Building, St. John divine, Rockefeller center, Morgan
library"*** — the four EXACT groups `check-place-candidates.py` had reported since #682 merged.
**Places 76 → 80. `makers` is BYTE-IDENTICAL and exactly ONE Atlas tour changed.** Content only — no Swift, no
SQL, no gh-pages push, no build. Full detail: `archive/HANDOFF-260901.md`.

- **🔴 THE CHECKER'S EXACT GROUPS WERE THE SMALLER HALF OF EACH SITE, AND READING AROUND THEM CHANGED
  ALL FOUR PLACES.** Each group was a coincident **pin pair** — but every one of the four subjects
  also has **an Atlas tour with a gallery** (92 m, 30 m, 124 m and 6.5 m away), and three of them have
  further pins the checker never grouped. **Do not build a place from the checker's group alone;
  sweep the catalogue for the subject first.**
- **✅ EVERY HERO IS A THIRD PHOTOGRAPH AND NOTHING WAS SOURCED.** Because each site has an Atlas tour
  with a gallery, all four heroes are promoted from a member tour's own images — already uploaded,
  already verified, all four live **200**. This is the tier-1 shape, not the tier-2 borrowed-hero
  case, so **the borrowed-hero warning count does not move.** Every candidate was **rendered and
  looked at**, not chosen by filename: Empire State takes the tower daylit between two buildings
  (over a night skyline too close to the tour's own hero), the cathedral takes its nave, the Morgan
  takes the Madison corner showing the low villa beside the taller block — which is what its tour's
  opening sentence describes — and Rockefeller takes the tree, the rink and Prometheus together.
  ⚠️ **Rockefeller's is SEASONAL**, exactly the **Grand Central** trade-off (whose place hero carries
  a Christmas wreath), resolved the same way because the only alternative is a Prometheus close-up.
- **🔴 THE EMPIRE STATE BUILDING'S ANCHOR WAS PUT TO THE OWNER AND THEY MOVED IT ONTO THE BUILDING —
  SETTLED, DO NOT REVERT.** Reverse-geocoded at zoom 18 the two candidates are **not** equivalent: the
  **pin** lands on `350 5th Avenue, 10118`, the building's own address and its dedicated ZIP, while
  the **tour** landed on `Chase, 349 5th Avenue`, a bank branch on the opposite pavement. The tour's
  coordinate was not an error — its narration opens *"From 34th and Fifth, the Empire State Building
  rises into the frame like the postcard it became"*, a deliberate vantage, and the **Grand Central**
  precedent (88 m, identical shape) anchors such a place on the tour. It was built that way, flagged,
  and the owner said **move the tour**. So the place sits on the building and **the Atlas tour moved
  92 m onto it** — the only tour in the catalogue this branch changes; the two pins were already
  there and are **byte-identical to `main`** again. ⚠️ **This diverges from Grand Central deliberately**
  — do not "restore consistency" by moving it back.
- **🔴 MOVING IT MEANT RE-DERIVING THE RADIUS, NOT INHERITING IT — the IAC / Barcelona Pavilion rule.**
  The stop carried the 30 m city default, which from the building would **not** reach the vantage the
...
- **🔴 EXACTLY ONE ATLAS TOUR CHANGED IN THE WHOLE CATALOGUE, AND IT IS THE EMPIRE STATE ONE ABOVE.**
  ...
  those four plus `triggerRadiusMeters`. **Every entry involved is `manual`**, so no geofence is
  disturbed and **no other radius changes** — both checked rather than reasoned about.
- **⚠️ MEMBERSHIP WENT PAST THE COINCIDENT PAIRS TWICE, AND BOTH ARE ONE LINE TO REVERSE.** **The
  Penn Station lamppost joins the cathedral** — its own caption reads *"A piece of the original Penn
  Station **inside** the Cathedral of St. John the Divine"* and its hero shows the post standing
  indoors, which is the **Arab Hall / Elizabeth Oak / Tudor Kitchens** shape rather than the
  **Beauchamp Tower** exclusion (a separate building) or the **Great Ball Court** one (a separate
  destination). **The Rink and the Lost Theatre join Rockefeller Center**, whose own tour describes
  *"fourteen Art Deco buildings"* across three blocks; max spread is **124 m**, well inside the
  **Windsor Castle** precedent of 6 members across 376 m. ⚠️ **The place is named `Rockefeller
  Center`, not the tour's `Rockefeller Center, the Plaza`** — the place covers the complex, the tour
  covers the plaza; a deliberate divergence from the take-the-tour's-title convention.
- **⚠️ FLAGGED, NOT RESOLVED: `Dante at the Morgan Library` carries `#morganpartner`** — a paid
  partnership, the same class as the German Doner Kebab and Coca-Cola pins. It is a real post about a
  real thing in the building and the Coca-Cola precedent (#677) says the owner may keep it; it ships
  as a member.
- **⚠️ Descriptions assert only what the member tours support.** The Empire State copy **does not name
  Shreve, Lamb and Harmon**, because its tour never does (the Jules Dalou rule); the Morgan names
  McKim and Piano because its tour names both. Addresses are editorial, corroborated by reverse
  geocoding rather than taken from it.
- **Verification.** Place ids `uuid5(NAMESPACE_URL, "atlas-place:<city-slug>:<name-slug>")`,
  **reverse-verified against 74 of the 76 existing places** (the two misses are the documented legacy
  uppercase ids). Validator mirror — vocabulary from **both** Swift files, refusing to run on
  disagreement or an empty parse (**385 tags**) — **self-tested 40/40**, then **0 errors, 2 warnings**
  across 1,552 tours + 533 pins + 80 places, **both pre-existing**. ⚠️ **Three further place-layer
  faults were injected against the NEW places specifically** — a member claimed by two places, an
  empty name, and a member nudged 55 m off its anchor — **3/3 caught, control clean**, because a
  mirror whose suite predates the layer you just changed is not evidence about it. `Tours.json`
  **byte-stable under a Python re-dump before editing**; diff **111 insertions / 44 deletions**.
  **0** members off their place coordinate, **0** duplicate place ids, **0** tours claimed twice.
  `check-place-candidates.py` **EXACT 4 → 0 and exits 0**, restoring the clean-exit state broken by
  #682; **NEAR 24 → 22**, falling by exactly the two Empire State pairs resolved, so **no new
...
### Fifty more link pins from @hereinnyc, and a Plus Code 970 m from its own subject (branch `claude/tour-links-r0n7s5`, session 130 — content)
...
- **✅ 50 DISTINCT POSTS FROM 51 LINKS, ALL PINNABLE.** One short link was pasted twice (once bare,
  ...
  coordinate-bearing copy was kept. **Every one is TikTok `@hereinnyc`** — the same creator as the
  105-pin batch in #675 — and there is **zero overlap** with the 483 pins already in the catalogue.
  **18 links carried a location hint.** No dead posts, no `/photo/` carousels, and **no `5009` CDN
  refusals** (the failure class that hit 13 of 95 thumbnails in #674).
- **🔴 THE HAND-WRITTEN PLUS CODE DECODER FAILED ITS OWN SELF-TEST, AND THE BUG WAS REAL — the
  opposite of the documented pattern.** `pip install openlocationcode` still cannot build a wheel
  here. CLAUDE.md warns twice that the *memory* of a vector is the usual culprit and the arithmetic
  is usually right; **this time the arithmetic was wrong** — the pair section's **first place value
  is 20⁴, not `PAIRPREC`** — which put every decode about **105 degrees** out. It was caught only
  because every expectation is derived from the specification's own pair table rather than recalled.
  **Keep deriving them.** The passing self-test: **8 codes** against hand-derived SW corner + cell
  size, **3 published anchors** (`7FG49QCJ+2V`, `7FG49Q00+`, and the Eiffel Tower `8FW4V75V+8Q` →
  48.8583125, 2.2944375), the **documented integer-encode bug** (`encode(20.375, 2.775, 10)` must be
  `7FG49QGG+22`; a float encode emits `7FG49QFG+X2`), **2,000 random round-trips both directions**,
  and `recover_nearest` across four hemispheres. ⚠️ **NYC straddles the −74° pair boundary**, so
  Manhattan is `87G8…` while a reference at −74.006 is `87G7…`; three of this batch's codes correctly
  recover to `87G7` (Battery Park City, the 56 Leonard bean, the Greenwich Street bookshop). A
  decoder that got that wrong would put pins in the Hudson.
- **🔴 A SUPPLIED COORDINATE IS NOT A VERIFIED COORDINATE, AND TWO OF THE EIGHTEEN DID NOT CHECK
  OUT.** All 18 were reverse-geocoded at zoom 18; sixteen land on their subject, several named
  exactly by OSM (`Grace Church` 5 m, `Blackwell Island Light`, `Bayard-Condict Building` 9 m,
  `Longacre Theatre` 14 m, `Studebaker Building` 31 m, `19 Cranberry Street` 5 m).
  **Pomander Walk was CORRECTED 970 m** — its code decodes onto a bare stretch of West End Avenue
  near W 103rd while OSM names the footway at `40.793994, -73.973304`, which reverse-geocodes to
  **"19 Pomander Walk"**; the video is entirely about the gated walk, so a pin a kilometre north is
  the Depot Boijmans / Chelsea Hotel shape. ⚠️ **The Will & Grace location was KEPT and FLAGGED**:
  its code lands at **547 Riverside Drive, Morningside Heights**, while the caption hashtag **and the
  creator's own burned-in text** both say Upper West Side, and nothing available settles which
  building it is. Owner's coordinate kept, pin titled **"A Will & Grace Filming Location"** with no
  neighbourhood claim — the session-128 #9 Park Avenue rule: **flag, do not override.**
- **✅ ALL 50 HEROES OPENED AND READ AGAINST THEIR CAPTIONS — zero wrong subjects.** This creator
  burns a title card into every frame, which is what makes a batch this size tractable. That pass
  settled four subjects the captions alone would not have: the Roman urn is at **Grace Church** (its
  own sign in frame), *"New York's First Modern Skyscraper"* is the **Bayard-Condict Building** on
  Bleecker, the *"curse of the bambino"* theatre is the **Longacre** (Harry Frazee built it with the
  money from selling Babe Ruth), and the faded Chelsea ad reads **"Griffon Cutlery Works"**.
- **🔴 AND IT FOUND ONE POST THAT IS NOT IN NEW YORK AT ALL.** The Palazzo Vecchio carving
  (*"Do you think it was done by Michelangelo?"*) is in **Florence** — it ships `city: "Florence",
  country: "Italy"`, which is the established shape for this maker, who already carries eight Italy
  and London pins on the same row (Padua ×2, Bologna ×2, Venice, Carrara, Genga, Treia, London ×4).
- **🔴 `Louis Sullivan` IS NOT IN THE VOCABULARY AND THE BAYARD-CONDICT IS HIS ONLY NEW YORK
  BUILDING.** #493 rejected him because the catalogue named him in three Chicago tours and owned
  none of his work; that is no longer true. **But this pin's caption names no architect**, so under
  the caption-driven rule it carries neither his name nor the generic `Designed by a Master` —
  correct by construction, and **do not "finish the job" here.** He is now the catalogue's most
  conspicuous architect absence; adding him is a `Models/Tag.swift` **code** change.
  **`Stanford White` IS in the vocabulary** and both captions that use him name him as the designer
  outright, so the Tiffany Building and the Robb Mansion carry his name **alongside**
  `Designed by a Master`. **`Anish Kapoor` deliberately NOT tagged** on the 56 Leonard bean (the
  caption never names him, and he made the sculpture not the building — the Kiki Smith rule), and
  **`McKim, Mead & White` deliberately NOT tagged** on the Morgan Library or the Municipal Building.
- **⚠️ FOUR PINS ARE FLAGGED FOR THE OWNER, NOT RESOLVED.** (1) **`Stanford White's Tiffany Building`
  has a hero of VENICE** — the creator's frame is the Palazzo Grimani on the Grand Canal, the
  building that *inspired* 401 Fifth Avenue, while the pin sits in New York; a link pin re-hosts only
  the thumbnail, so no other frame exists. **The likeliest pull candidate.** (2) **`German Doner
  Kebab on Sixth Avenue` is a paid partnership** (`#Partner`) — the Coca-Cola precedent (#677) says
  the owner may keep it. (3) **`WildPlay Jones Beach` is a paid AD carrying a discount code AND the
  venue has paused operations for 2026** — a pull candidate on two independent grounds; ships
  `city: "Wantagh"`. (4) **`The Weather Star`'s square crop clips the star itself** — the source is
  1080×1920 so the crop covers only the middle 56%, the star sits at ~9% and the creator's title card
  at ~71%, and **they cannot both fit**; left as rendered, because the card names the subject and
  removing it would be the editorialising the owner ruled out. **No hand re-crop was needed anywhere
  else — the first clean run in many batches.**
- **⚠️ `check-place-candidates.py` GOES 0 EXACT → 4 AND THEREFORE EXITS 1**, ending the clean-exit
  state #679 established. All four are genuine same-building pairs and **none was manufactured** —
  the pins were deliberately not nudged apart: **Empire State Building** (secret floors) + the
  mooring mast · **St. John the Divine** (twin towers) + the roof tour · **Rockefeller Center** (the
  Beam) + the last rivet · **the Morgan Library** facade + the Dante inside it. Every group has
  **two** members, inside `TourSetMap.maxStacked = 3`, so nothing is unreachable; each is a real
  place candidate needing the owner's copy, address, photograph and approval. NEAR goes 21 → 24.
  ⚠️ **`All'Antico Vinaio` needed disambiguating** — the catalogue already holds a pin of that name
  **in Florence** — so the New York one ships as **"All'Antico Vinaio on Eighth Avenue"** (the Los
  Angeles Union Station precedent).
- **⚠️ THE MAKER ROW ALREADY EXISTED and the uuid5 scheme reproduced its id exactly**
  (`5657EA40-DA53-552F-823A-D040F31FF526`), so the pins merged into it — **0 new maker rows** — and
  **the regenerated avatar was byte-identical to the live file and EXCLUDED rather than overwritten**:
  51 files generated, 50 uploaded (the `@urbanistariel` case, eighth time).
- **⚠️ `echo "EXIT=$?"` AFTER A PIPE READS THE LAST COMMAND'S STATUS, AND IT NEARLY SHIPPED A FALSE
  PASS.** `check-place-candidates.py | tail` with `echo $?` reported **EXIT=0** when the script had
  actually exited **1**. That is the documented `PIPESTATUS` trap from session 90, live again.
  Redirect to a file and read `$?`, or use `${PIPESTATUS[0]}`.
- **⚠️ A TRANSIENT TIKTOK CDN SSL FAILURE KILLED THE FIRST GENERATION RUN OUTRIGHT** and, because the
  driver had no retry, took the whole run's state with it. The rewritten driver **checkpoints after
  every pin and retries four times with backoff**, so a 50-row batch survives one bad packet. Worth
  folding into any future batch driver.
- **Verification.** Validator mirror — vocabulary parsed from **both** `Models/Tag.swift` **and** the
  Swift validator, refusing to run if they disagree or either parse is empty (they agree at **385
  tags across 5 facets**), and refusing to run if it parses fewer than 5 `TourCategory` cases —
  **self-tested against 40 injected fault classes, 40/40 caught**, then **0 errors, 2 warnings across
  1,552 tours + 533 pins + 76 places**, **both pre-existing**. `make-link-pin.py --selftest` **71/71**;
  `check-image-duplicates.py --selftest` **OK**. **0** duplicate tour/stop/maker ids, **0**
  already-pinned sourceURLs, **0** filename collisions against **6,290** gh-pages `images/` paths,
  **0** byte-duplicate heroes; closest perceptual pair **25.5** (two grey metal close-ups — the tonal
  false positive the two-stage check exists to reject). ⚠️ **The bare-slug check came back clean —
  0 of 50 would have overwritten a live hero**; the handle suffix is still load-bearing, it simply was
  not called on here. `seed_from_toursjson.py` regenerates cleanly at **208 makers / 2,085 tours /
  2,457 stops / 76 places**. `Tours.json` **byte-stable under a Python re-dump before editing** (the
...
merged). **Places 56 → 66. Tours (1,552), link pins (483 after merging #675) and makers (208) all unchanged,
byte-for-byte.** Content only — no Swift, no SQL, no gh-pages push, no build. **NO PR OPENED** (this
session's harness forbids opening one unasked). Full detail: `archive/HANDOFF-260831-4.md`.
...
### One hundred and five pins from one NYC historian, and a hero that is the same subway car twice (branch `claude/link-tours-upload-qw2l2a`, session 128 — content)

**The owner sent 107 links — all TikToks by `@hereinnyc`, a New York history creator.** Branch
restarted clean off `origin/main` at `308ad23` (the previous PR on this branch name, #674, is
merged, so this is a fresh change and not stacked on merged history). **linkPins 378 → 483 ·
tours unchanged at 1,552 · makers unchanged at 208 · places unchanged at 56.** Content only —
no Swift, no SQL, no build. **NO PR OPENED** (this session's harness forbids opening one
unasked). Full detail: `archive/HANDOFF-260831-3.md`.

- **✅ 105 DISTINCT POSTS FROM 107 LINKS, ALL PINNABLE — the largest batch to date, against a
  previous record of 95.** No dead posts, no `/photo/` carousels, and **no `5009` CDN refusals**
  (the failure class the previous batch hit on 13 of 95 thumbnails). Two exact repeats: one post
  pasted twice with different `web_id` parameters — which `normalize_url` strips, so both
  canonicalise identically — and one pasted twice with two Venice coordinates 2 m apart. **64
  links carried a coordinate; 41 did not.**
- **🔴 THE CREATOR BURNS A TITLE CARD INTO EVERY FRAME, AND THAT IS WHAT MADE 105 TRACTABLE.**
  All 105 heroes were opened and read against their captions — **zero wrong subjects** — and the
  burned-in text settled **four subjects the captions alone would not have**: **Big Apple
  Corner** (a `W 54 St` sign in frame, confirming the supplied coordinate), **the Pearline ghost
  sign** (a `2 Avenue & East 72 Street` subway sign), **Jericho Cider Mill** (its own address,
  `213 ROUTE 106 JERICHO, NY 11753`, printed on the bag) and **the Schinasi Mansion** (the
  caption names no building; the reverse-geocode gives 351 Riverside Drive and the frame shows
  the free-standing white marble mansion that stands there).
- **🔴 #9 AND #61 ARE TWO DIFFERENT PARK AVENUE SUBJECTS, AND #9's SUPPLIED COORDINATE
  REVERSE-GEOCODES TO #61's.** #61 is explicitly *"Inside the Park Avenue Tunnel"* and its frame
  is a tunnel; #9 is *"Restoring Park Avenue"* and its frame is a **planted mall with benches and
  paths**. The owner's #9 point resolves to "Park Avenue Tunnel" only because that is the nearest
  named feature. **#9 keeps the owner's coordinate; #61 was moved to the tunnel's south portal**
  (`40.7472, -73.98075`, reverse-verified to *9 Park Avenue, Murray Hill*), putting ~190 m
  between two pins that would otherwise have sat 23 m apart under confusingly similar names.
  ⚠️ **#9's hashtag says `#uppereastside` while its coordinate is Murray Hill — flagged, not
  overridden.**
- **⚠️ `1 Cherry Street` UNBOUNDED MATCHED JOHNSON CITY, 250 km UPSTATE** — the documented
  unbounded-search trap. Bounded to Manhattan it resolves onto Cherry Street, where the Samuel
  Osgood House site lies beneath the Brooklyn Bridge approach. **The 91st Street Garden returned
  nothing by name at all**; probing the Riverside Park promenade found `Riverside Walk`.
  ⚠️ **Three supplied coordinates reverse-geocode onto a neighbour and are all right**: Breakfast
  at Tiffany's lands on *164 E 72nd* while **169 E 71st, the actual brownstone, is 26 m away**;
  the Hayes Theater lands on the Broadhurst across the street; the Red Room lands on *2 Wall
  Street* beside One Wall Street.
- **🔴 THE STACK CAP WAS CHECKED AND IS CLEAR — the deepest coincident group is 2, against
  `TourSetMap.maxStacked = 3`.** That matters because the batch is essentially one creator, so
  her own maker page is where coincident pins would bite: Gracie Mansion (0.0 m), Madison Square
...
- **⚠️ #35 IS THE ONE BUILDING THIS BATCH COULD NOT NAME.** A Beaux-Arts mansion near Riverside
  ...
  asserting no attribution — the Banksy precedent.
- **⚠️ SEVEN HAND RE-CROPS — the vertical `--focus` gap, ELEVENTH batch running.** Every source
  is 9:16, so the square crop is width-limited and `--focus` does nothing at all. Seven sliced
  title cards were re-rendered through a mirror of the tool's own pipeline (importing
  `make-link-pin.trim_bars` so the two cannot drift) at the same filenames. ⚠️ **A programmatic
  clipping detector was written and then discarded as untrustworthy** — it measures "a dark card
  touches the bottom edge", which is not "text is cut": it **missed #13** entirely and flagged
  ~22 heroes whose padding was cut but whose text was wholly legible. **All 105 outputs were
  eyeballed instead.**
- **🔴 THE HANDLE SUFFIX PREVENTED ONE LIVE-HERO OVERWRITE:
  `images/empire-state-building_hero.webp` is the live Atlas NYC tour's own hero**, and since
  #567 a downloaded tour would never see the correction. **The maker row already existed** and
  the uuid5 scheme reproduced its id exactly, so it merged; **the regenerated avatar was
  byte-identical to the live file and was EXCLUDED rather than overwritten** — 106 generated,
  105 uploaded (the `@urbanistariel` case, seventh time).
- **⚠️ EXACTLY ONE ARCHITECT TAG IN THE WHOLE BATCH — `Giles Gilbert Scott`** on the red
  telephone box, whose caption names him outright; it carries `Designed by a Master` **alongside**,
  never replacing. **Deliberately NOT tagged under the caption-driven Jules Dalou rule**, all in
  the vocabulary and none named by its post: **Christopher Wren** (the Painted Hall), **McKim,
  Mead & White** (the three Penn Station pins), **Calvert Vaux** (Bow Bridge). **Verified absent
  from the vocabulary:** Raymond Hood, Warren & Wetmore, Reed & Stem, Ralph Adams Cram, Heins &
  LaFarge, Shreve Lamb & Harmon, Hildreth Meière, Frederick Clarke Withers.
- **⚠️ `check-place-candidates.py` GOES 10 EXACT / 30 NEAR → 11 / 41, AND THE ONE NEW EXACT GROUP
  IS THIS BATCH'S OWN GRACIE MANSION PAIR** — two posts about the same building on the same
  geocoded node. That is honest rather than manufactured, and **the pins were deliberately not
  nudged apart to dodge the checker**. The +11 NEAR are new pins beside existing Atlas tours of
  the same subject (Empire State 92 m, MSG 19/20 m, Yankee Stadium 31 m, Riverside Church
  9/253/256 m, Times Square 100/256 m, the Red Room 108 m). ⚠️ The Yankee Stadium pin sits beside
  the **existing Yankee Stadium place**, which already has 6 members. **Flagged, none created.**
- **Verification.** Validator mirror — vocabulary parsed from **both** `Models/Tag.swift` **and**
  ...
  **385 tags across 5 facets**) — **self-tested against 42 injected fault classes, 42/42 caught**,
  then **0 errors, 2 warnings across 1,552 tours + 483 pins + 56 places**, **both pre-existing**
  (the same mirror against `origin/main` reports the identical pair). `make-link-pin.py
  --selftest` **71/71**. **0** duplicate tour/stop/maker ids, **0** already-pinned sourceURLs,
  **0** byte-duplicate heroes. gh-pages `f94e16fa`: the remote head was **re-read in the same
  command as the push**, tree diff **exactly 105 additions, 0 deletions, nothing outside
  `images/`**, and the deploy read **`in_progress`, never `cancelled`**. `Tours.json`
  **byte-stable under a Python re-dump before editing**; diff **4,725 insertions / 0 deletions**.
  `seed_from_toursjson.py` regenerates cleanly at 208 makers / 2,035 tours / 2,407 stops / 56
  places. **CI has not run: no PR is open.**
...
`origin/main` (the previous PR on this branch name, #666, is merged) and **rebased mid-session onto
[#673](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/673)**, with the assembly re-run on their
base rather than a JSON conflict hand-resolved. **linkPins 283 → 378 · makers 206 → 208 · tours
unchanged at 1,552 · places unchanged at 56.** Content only — no Swift, no SQL, no build. **NO PR
OPENED** (this session's harness forbids opening one unasked). Full detail:
`archive/HANDOFF-260831-2.md`.

- **✅ 95 DISTINCT POSTS FROM 96 LINKS, ALL PINNABLE** — no dead posts, no `/photo/` carousels.
  **The largest batch to date by a wide margin**, against a previous record of 35. Two short links
  resolve to the same Winchester Great Hall video, and **the one kept is the one carrying the
  `3M7J+33` Plus Code**, so the dedupe cost nothing. **92 are TikToks by `@history_alice` (Alice
  Loxton)**, whose maker row **already existed** and whose uuid5 id the scheme reproduced exactly, so
  it merged; two new rows are `TikTok @domus` and `Instagram @romeartstories`.
- **🔴 THE INSTAGRAM REEL IS NOT DOMUS.** It sat directly above the "Domus" heading in the message
  and is `@romeartstories`, on the Sistine Chapel's **exterior** ("the outside rarely tells the whole
  story"). **Read the payload, not the heading a link sits under.**
- **🔴 SIX POSTS ARE ABOUT WINDSOR CASTLE AND SIX ABOUT WESTMINSTER ABBEY, AND `TourSetMap.maxStacked`
  IS 3.** `HomeView` caps at 4, but the maker page caps at **3** — and this batch is essentially one
  creator, so **her own page is exactly where six coincident pins would put three of them out of
  reach.** Solved with **distinct coordinates inside each site, not a place page**: Westminster
  Abbey's six now sit on six interior points — nave, sanctuary, the Confessor's shrine, the Lady
  Chapel, the Weston Tower and the abbey node — **each reverse-geocoded to confirm it falls inside
  the building**. Windsor's six split across the State Apartments (3), the castle node (2) and the
  Lower Ward. **Max coincident in the batch is 3, so nothing is unreachable.**
  - ⚠️ **A candidate point at the Great West Door reverse-geocoded to Broad Sanctuary — the road —
    and was REJECTED.** That rejection is what makes the other five trustworthy; without it the whole
    set would be asserted rather than checked.
- **🔴 A NEW FAILURE CLASS: 13 POSTS WHOSE oEMBED THUMBNAIL TIKTOK'S CDN WILL NOT SERVE.** It answers
  the signed `tplv-tiktokx-dmt-logom` URL with **52 bytes of `{"code":5009,"error":"fail to make
  process filters"}`**. **It is not a dead post** — oEmbed returns full metadata. **It is not
  transient** — five spaced attempts, all 13, all failed. **It is not a host or transform pattern** —
  working and failing posts share both, so it is **per object**. `make-link-pin.py` behaved correctly
  and refused with `COULD NOT VERIFY` rather than shipping a broken hero.
  - **The fix is the SAME post's `originCover`, read off the post page**, which uses the
    **`tplv-tiktokx-origin`** transform and serves fine — and is a better source anyway (no logo
    overlay, larger). It was fed in by **monkeypatching the tool's own `best_thumbnail`**, so
    `trim_bars`, the square crop, the blurred 4:3 pad, the filenames and the ids are all its own
    code. **13/13 recovered. If this recurs, fold the fallback into `best_thumbnail` itself.**
- **✅ ALL 95 HEROES OPENED AND READ AGAINST THEIR CAPTIONS — and it caught FOUR wrong subjects the
  captions alone would have shipped.** **#21 is Avebury MANOR, not the stone circle** (*"I'm at
  Avebury Manor in Wiltshire"* burned into the frame); **#28 is specifically the Beauchamp Tower**;
  **#91 is a Charles I portrait, not the Round Tower** its caption implied — and the perceptual pass
  independently scored it against #62 at **11.5**, the batch's closest pair; and **#4 and #16 name no
  place at all in their captions**, turning out to be **the Royal Crescent at Bath** and **Christ
  Church Cathedral in Dublin**, both named in the creator's own on-screen text. ⚠️ Tiles were mapped
  to pins **through `heroImageURL`, never by slug prefix** — the documented session-121b bug.
- **⚠️ ONE WEAK HERO, FLAGGED NOT FIXED: #88 Windsor Castle is a postbox.** Its caption is *"Historic
  delights of Windsor Castle!"* and Windsor does have a well-known Victorian wall postbox, but on the
  map it reads as a generic red box. A link pin re-hosts only the thumbnail, so no other frame exists.
- **⚠️ 18 PLUS CODES, ALL RECOVERED AND REVERSE-VERIFIED ONTO A NAMED SUBJECT.** The decoder was
  hand-written again (`pip install openlocationcode` still cannot build a wheel here) and
  **self-tested against four official vectors in BOTH directions plus 2,000 round-trips** before use.
  Notable: **Hougoumont Farm AND the Lion's Mound** for the two Waterloo posts — 1.2 km apart and
  genuinely different places, not a duplicate — plus the Weston Tower, the Florence Nightingale
  memorial, Flatford's Granary for *The Hay Wain*, the Mary Rose Museum and Winchester's Great Hall.
  ⚠️ **`Al Qarna, Egypt` does not geocode**; the reference had to be re-queried as `Al Qurna, Luxor,
  Egypt`. **A Plus Code is useless without a resolvable reference.**
- **⚠️ RE-QUERY BEFORE CONCLUDING A SUBJECT IS UNMAPPED — sixth batch running.** A bounded search for
  **Avebury Manor** returned nothing; unbounded with the county it resolved instantly. Christ Church
  Cathedral Dublin was the same shape.
- **🔴 THE HANDLE SUFFIX PREVENTED SEVEN LIVE-HERO OVERWRITES — the most in any batch, against a
  previous record of three.** `westminster-abbey`, `national-gallery`, `natural-history-museum`,
  `leighton-house`, `churchill-war-rooms`, `charles-dickens-museum` and `caixaforum-madrid` are **all
  live Atlas tour heroes**; a bare slug would have written over every one, and since #567 a
  downloaded tour would never see the correction. **`avatar-tiktok-history-alice.webp` regenerated
  byte-identically and was EXCLUDED rather than overwritten** — 97 files generated, 96 uploaded (the
  `@urbanistariel` case, sixth time).
- **⚠️ `check-place-candidates.py` GOES 0 EXACT → 10 AND 9 NEAR → 30, AND THAT IS NOT A REGRESSION OF
  #673's AUDIT.** That session had just driven EXACT to zero for the first time in the script's
  history. **Every one of the ten is a same-subject pair the owner sent twice** — Harvington Hall ×2,
  Hatfield House + the Elizabeth Oak, Syon Park ×2, York Minster + Roman York, **Windsor Castle ×3**,
  Hampton Court ×2, Hever Castle ×2, the National Gallery ×2, Leighton House + the Arab Hall — plus
  **CaixaForum Madrid**, where the new Domus pin lands on the existing Atlas tour. **Flagged, not
  created**: a place needs its own copy, address, photograph and human approval.
- **⚠️ BOTH DOMUS SUBJECTS ALREADY EXIST AS ATLAS TOURS** — **Ewha Campus Complex** (Seoul) and
  **CaixaForum Madrid** — so both pins **match the tour's tagging exactly** and the pairs share
  shelves, carrying `Dominique Perrault` and `Herzog & de Meuron` respectively. ⚠️ Ewha was
  identified from the creator's own burned-in text (*"L'architettura invisibile di Dominique Perrault
  a Seoul"*), not from the caption, which names no building.
- **⚠️ ARCHITECTS VERIFIED ABSENT FROM THE VOCABULARY, all shipping the generic `Designed by a
  Master`:** **John Vanbrugh** (Castle Howard, Blenheim), **Charles Barry** and **Augustus Pugin**
  (Palace of Westminster), **Basil Spence** (Coventry Cathedral), **Henry Yevele** (Westminster Abbey
  nave), **Capability Brown** (Blenheim park), **Robert Adam** (Syon House). In the vocabulary and
  used by name: `Dominique Perrault`, `Herzog & de Meuron`, `Alfred Waterhouse` (Natural History
  Museum). Adding the absent names is a `Models/Tag.swift` **code** change, kept out of a content
  batch.
- **Verification.** Validator mirror — vocabulary parsed from **both** `Models/Tag.swift` **and** the
  Swift validator, refusing to run if they disagree or either parse is empty (they agree at **385
  tags**) — **self-tested against 20 injected fault classes, 20/20 caught**, then **0 errors, 6
  warnings across 1,552 tours + 378 pins + 56 places**, **all six pre-existing** (the same mirror
  against `origin/main` reports the identical set). `make-link-pin.py --selftest` **71/71**. **0**
  duplicate tour/stop/maker ids, **0** already-pinned sourceURLs, **0** byte-duplicate heroes;
  closest perceptual pair **11.5**. `Tours.json` **byte-stable under a Python re-dump before
  editing**, re-checked after the rebase; diff **4,353 insertions / 0 deletions**. gh-pages: `git
  ls-remote` re-read **in the same command as the push**, tree diff **exactly 96 additions, 0
  deletions, nothing outside `images/`** (`12b2764e`), the deploy read **`in_progress`, never
  `cancelled`**, and after it landed **all 96 live URLs were hash-verified against the uploaded bytes
  — 96 ok, 0 bad**, with the head re-confirmed as this batch's own commit.
  `check-image-duplicates.py --pins` **OK** over 381 images. **CI has not run: no PR is open.**


### Seven place cards built from an audit of what was missing — places 49 → 56, and EXACT reaches zero (branch `claude/place-cards-audit-px3z3o`, session 126 — content)

**Owner: *"do an audit of places that are candidates for place cards that do not yet have them"*, then *"chelsea hotel already being handled by another session. do the rest of the tier 1 tours"*, then *"chichen itza - make a place card for 2 of the tours, leave the ball court out of it. rosewood, make a placecard"*, then *"make casa lleo a place"*.** Branch cut clean off `origin/main` at `a997860a`. **Places 49 → 56 — this branch adds 7; `main` gained the Chelsea Hotel underneath it. Tours (1,552), link pins (283) and makers (206) all unchanged, byte-for-byte.** Content only — no Swift, no SQL, no gh-pages push, no build. **NO PR OPENED** (this session's harness forbids opening one unasked). Full detail: `archive/HANDOFF-260831.md`.

- **Built, each a tour + a link pin of one subject with a free third photograph:** **Casa Milà — La Pedrera** (Barcelona, 11 m) · **Operaparken** (Copenhagen, 28 m) · **Wave Hill** (Bronx, 63 m) · **Grand Central Terminal** (New York, 88 m). **The Chelsea Hotel was the fifth candidate and was deliberately left to the session already handling it — it merged as [#669](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/669) mid-session**, which is why this branch was rebuilt on top of it (see the verification note).
- **🔴 `check-place-candidates.py` COULD NOT SEE GRAND CENTRAL, AND THAT IS A PROPERTY OF ITS NEAR TIER RATHER THAN A BUG.** It matches on **title containment**, so `{south, facade, grand, central}` against `{grand, central, terminal}` — neither a subset of the other — never pairs. The same blind spot is already recorded for Washington Square Arch/Park and St Pancras. **The durable move is to run the tool AND a wider sweep by hand:** every marker pair within 40 m, plus every pair within 200 m sharing a distinctive (non-generic) word — **86 pairs**, each read.
- **🔴 24 OF THE 48 EXISTING PLACES ARE A WALK PAIRED WITH A SINGLE TOUR, so that is the dominant place shape, not an edge case — and 47 of 72 walks belong to no place.** 15 of those walks start within 60 m of a single tour, but **reading each walk's stop 0 title settles almost all of them**: only **Dreta de l'Eixample** (stop 0 **is** Casa Lleó Morera, 0 m) and **The Monumental Rosary of Montserrat** (stop 0 **is** Monestir de Montserrat, 31 m) are genuinely the same site. ⚠️ **`The Loop — Where the Skyscraper Was Born` sits 24 m from the Rookery and is NOT a candidate** — its stop 0 is **LaSalle & Adams**, the Home Insurance site, a different subject; the other twelve carry generic `"— Introduction"` stop 0s that name no landmark. Both live ones would need the walk's stop 0 moved onto the single's coordinate (places require exact equality); both are `manual`, so no geofence is disturbed.
- **The pin moved and the tour did not, all four times — verified, not asserted.** The parsed `tours` and `makers` arrays are byte-identical to `origin/main`, so **0 tour coordinates, trigger modes or radii changed**; exactly four fields moved per pin (`stops[0].latitude/longitude` plus the mirrored centroid).
- **⚠️ GRAND CENTRAL IS THE ONE JUDGEMENT, AND IT FOLLOWS THE PETERSEN PRECEDENT.** The **pin** had the better address — it sat on OSM's own `Grand Central Terminal` node at 89 East 42nd Street — while the **tour** sits 88 m south on the 42nd Street pavement. That is not an error: the tour's own text says *"standing at 42nd Street and Park Avenue, looking up at the south face"*, so it is a **deliberate vantage**, exactly like Petersen's Fairfax frontage and Grace Cathedral's Great Stairs. The place is anchored on the tour and **its `address` names the building**. ⚠️ Note the tour is `manual`, so the usual "moving a geofenced tour changes where audio fires" argument did not apply — the vantage did. **Operaparken and Casa Milà are the CalAcademy rounding case instead**: both pins sat on OSM's own nodes, 28 m and 11 m from tours stored at lower precision, and both tours are geofenced.
- **🔴 EVERY PLACE HERO IS A THIRD PHOTOGRAPH, OPENED AND LOOKED AT, AND THE BUILD ASSERTS IT.** All four are promoted from the member tour's own gallery — already uploaded, already verified, **nothing sourced** — and all four live URLs return 200: Casa Milà takes the rooftop chimney warriors at sunset, Operaparken the oblique aerial of the whole island park, Wave Hill the pergola and lawn, Grand Central the full south front in daylight. ⚠️ **Wave Hill's was the ONLY option** — that tour's gallery holds exactly one image. ⚠️ **Grand Central's carries a Christmas wreath on the centre window** — seasonal, not wrong, and chosen over the alternative because that one is a close-up of the sculpture group and an establishing shot beats a close-up (the session-95 criterion). One-line swap either way.
- **⚠️ THE PLACE-ID SLUG DOES NOT FOLD ACCENTS, and getting that wrong would have minted the wrong id.** `uuid5(NAMESPACE_URL, "atlas-place:<city-slug>:<name-slug>")` where the slug lowercases and replaces every non-`[a-z0-9]` run with a hyphen — so `Hackesche Höfe` → `hackesche-h-fe`. **That rule reproduces 46 of the 48 existing ids** (the two misses being the documented uppercase legacy ids, Green-Wood Cemetery and Oedo Antique Market); an accent-folding slug reproduces only 44.
- **Addresses are editorial, corroborated by geocoding, never taken from it.** OSM files La Pedrera under **Carrer de Provença 261-265** while the building's own published address is **Passeig de Gràcia 92**, which is what ships — the coordinate is where you stand, the address is what you are looking at. **Operaparken has no street number in OSM**, so it ships `Dokøen, Holmen, Copenhagen` rather than an invented one.
- **✅ TWO MORE BUILT ON OWNER INSTRUCTION, AND BOTH ARE PIN-ONLY SITES WITH A BORROWED HERO.** **Chichén Itzá** (`@architectanddesign`'s Temple of Kukulkán + `@bassforge.us`) and **Rosewood Mayakoba** (`@german.samvel` + the hotel's own `@rwmayakoba`). **Nothing moved for either** — both pairs were already exactly coincident, so the identity rule held on its own. ⚠️ **THE HERO IS BORROWED FROM A MEMBER IN BOTH CASES, DELIBERATELY**: neither site has an Atlas tour and every member is a link pin with an empty gallery, so **no third photograph exists in the catalogue** — the Waterlooplein / Legion of Honor / Glasshouse case, which the owner has already closed. **Do not go sourcing a replacement.** ⚠️ **Chichén Itzá had only ONE usable frame:** `@bassforge.us`'s thumbnail is a reaction screen — a black panel reading *"The Unreal Genius of the Ancient Mayans"* over waveform graphics and a talking head — so the Kukulkán photograph was the only choice. Rosewood took `@german.samvel`'s **aerial of the whole beachfront** over the hotel's own frame (a plunge pool beside a lagoon) on the establishing-shot criterion; the venue's own frame carries higher subject certainty and is a one-line swap. ⚠️ **THE GREAT BALL COURT IS DELIBERATELY EXCLUDED** (owner: *"leave the ball court out of it"*) — it stays its own pin 224 m north-west, so the map shows the place beside it. **Do not "complete" the place by adding it.** ⚠️ **Neither description carries its members' claims:** `@bassforge.us`'s post is pseudo-archaeological (*"synthesizing light and sound to influence biology"*) and the hotel's own post is award marketing; both stay in `longDescription`, which is the creator's verbatim words, and neither reaches anything this catalogue authors.
- **🔴 CASA LLEÓ MORERA CLOSES THE CATALOGUE'S LAST EXACT GROUP — `check-place-candidates.py` NOW REPORTS *NONE*, AND EXITS 0 FOR THE FIRST TIME.** The Barcelona deferral had stood since session 116. The members are the single tour and the **Dreta de l'Eixample** walk, whose **stop 0 IS Casa Lleó Morera** — the walk+single shape that makes up 24 of the existing places — and they were **already on an identical coordinate, so nothing moved**. ⚠️ **The hero is a third photograph** (`casa-lleo-morera_2.webp`, the corner facade from across Passeig de Gràcia): the tour's own hero is the interior stained-glass tribune the owner confirmed keeping in session 118, and the walk's is an aerial of the Eixample grid, so the establishing exterior neither member carries is exactly what was free. ⚠️ **The coordinate reverse-geocodes to a METRO PASSAGEWAY and that is correct** — it is the documented Barcelona opposite-pavement vantage (Casa Batlló 39 m, Casa Amatller 60 m, this one 31 m from its own OSM node); the forward geocode names `Casa Lleó Morera` on Carrer del Consell de Cent at exactly that 31 m. **Do not "fix" it to the building.** The `address` ships the building's published **Passeig de Gràcia 35** rather than OSM's side-street filing, as La Pedrera's does.
- **Flagged, still NOT built:** **Tribune Tower** (142 m) and **Petit Palais** (276 m), where the Atlas tour covers **two** subjects (*The Wrigley Building & Tribune Tower*, *Pont Alexandre III & Petit Palais*) so a place named for the pin only half-covers its own member — a naming decision, not a data question. ⚠️ **THERE IS NO GRAND PALAIS CANDIDATE and there cannot be one today:** the Grand Palais has exactly **one** entry in the catalogue, and a place needs two members. The pin beside it is the **Petit Palais** — its own burned-in text reads *"Le Petit Palais"* and the frame is that building's gilded gate — and the two palais are separate buildings 155 m apart across avenue Winston-Churchill, which is the LACMA / Academy Museum merge the 40 m rule was rejected for. ⚠️ **A tour + a link pin is two members**, which is why one Petit Palais *tour* still makes a candidate: **19 of the existing places are exactly a link pin beside a single tour.**
- **⚠️ Re-confirmed as decided; do not re-raise:** Little Island (*"NOT little island for now"*), Tibidabo vs Tibidabo Amusement Park, LACMA vs the Academy Museum at 22 m, and Casa Batlló / Amatller / Lleó Morera as three separate buildings. Checked and rejected as distinct subjects at close range: Westminster Abbey/Hall, Duomo/Piazza del Duomo, Stonewall Inn/National Monument, Christ Church/Old Spitalfields, Al Fahidi Fort/Neighbourhood, Marina City/Merchandise Mart, Castro Theatre/Harvey Milk, and both San Francisco Chinatown pairs. **No stack-cap risk:** the only coincident groups of 3+ are AMNH and Yankee Stadium, both already places.
...
- **✅ NEW: `scripts/check-place-candidates.py`, so this never depends on a session noticing again.**
  ...
  - **🔴 IT FOUND A REAL EXACT PAIR THE HAND QUERY MISSED, AND ITS OWN FALSE-POSITIVE MODE ON THE
    ...
    **exits 1 today and a clean exit is not the expected state** until Barcelona's place is written.
    ✅ **RESOLVED 2026-08-31 — the Casa Lleó Morera place is written, EXACT is empty and the script
    exits 0. A clean exit IS the expected state now; treat any EXACT group as a real finding.**
...
- **🔴 AN IAP REVIEW SCREENSHOT MUST BE 1242×2208 — THE OLD 5.5-INCH SIZE, NOT A MODERN DEVICE SIZE. Owner-confirmed 2026-08-31 after three rejections.** ⚠️ **This corrects what this line used to say.** A cropped 1206×2105 PNG uploaded fine and then came back `assetDeliveryState.state = FAILED`, `IMAGE_INCORRECT_DIMENSIONS`, and **1320×2868 (iPhone 17 Pro Max native) was recorded here as accepted — it is NOT, and neither is 1290×2796.** The slot wants the 5.5″ iPhone dimensions and nothing else tried worked. **A modern iPhone screenshot is therefore ALWAYS the wrong shape** (a 16/17 Pro shoots 1206×2622) and must be scaled to fit inside 1242×2208 and padded — **pad with pure black, which is invisible because the app's own background is `#000000`**. Do not letterbox onto white. So never crop to hide a blemish: relaunch with **`-UITestDisableMarquee`** (`UITestSupport.swift`) to freeze the scrolling mini-player title, which is the thing that makes cropping tempting in the first place.
...
- **Every tour carries `city` AND `country`** (`country` added session 99 — **262 cities, 40 countries** across tours and link pins together, re-derived 2026-09-01 after the 21-pin batch; the figures this line carried before that read 250 / 40 and, before that, 203 / 37 — and the 250 was already stale against `main`'s 259 when it was written, so **re-derive rather than quoting it** — `docs/app-store-screenshots.md` § Scale figures has the one-liner). `country` is denormalised onto the tour exactly as `city` is, so it travels with the content and updates over the air; **a new city batch must author it** or that tour drops out of the Settings → About count. `Tour.country` is optional so the bundled seed, the gh-pages mirror and maker-authored tours all keep decoding. Its column + `get_catalog` key are live (`backend/add_country.sql`, applied 2026-08-19).
