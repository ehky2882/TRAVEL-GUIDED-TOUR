...
### Four place cards built from an audit of what was missing — places 49 → 53 (branch `claude/place-cards-audit-px3z3o`, session 126 — content)

**Owner: *"do an audit of places that are candidates for place cards that do not yet have them"*, then *"chelsea hotel already being handled by another session. do the rest of the tier 1 tours"*.** Branch cut clean off `origin/main` at `a997860a`. **Places 49 → 53 — this branch adds 4; `main` gained the Chelsea Hotel underneath it. Tours (1,552), link pins (283) and makers (206) all unchanged, byte-for-byte.** Content only — no Swift, no SQL, no gh-pages push, no build. **NO PR OPENED** (this session's harness forbids opening one unasked). Full detail: `archive/HANDOFF-260831.md`.
...
- **Flagged, NOT built:** **Chichén Itzá** (three pins, no Atlas tour — two coincident plus **The Great Ball Court 224 m off**, which the checker also misses; a place would have to **borrow a member's hero**, the Waterlooplein case) · **Rosewood Mayakoba** (two coincident pins, borrowed hero, low value) · **Tribune Tower** (142 m) and **Petit Palais** (276 m), where the Atlas tour covers **two** subjects so a place named for the pin only half-covers its own member — a naming decision, not a data question.
