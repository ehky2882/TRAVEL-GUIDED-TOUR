...
- **TestFlight latest is 1.0 (61)** — `project.pbxproj` on `main` is at build **61** (the bump rode in with the parallel share/Universal Links work #297, not a separate `chore: bump`). Build 61 carries the share feature; **the Step-4 maker-authoring increments (#300, #302, and #304 once merged) landed on `main` today and may NOT be in the currently-live 61 binary** (archive timing vs. merge timing is uncertain). To guarantee maker authoring is testable on device, **cut a fresh build after #304 merges** (bump 61 → 62) via the short-lived-PR pattern + `-allowProvisioningUpdates`.
