...
| 8d | **A pin or tour needs a coordinate, or one looks wrong** | **Search the venue's NAME before ever saying an address cannot be found.** 🔴 On 2026-09-15 a session tried scraping byFood (403 Cloudflare), Tabelog (403), Overpass (blocked by the web egress proxy) and Instagram embeds (no location tag), concluded 42 Tokyo addresses were unobtainable, and handed the owner a list to source by hand. The owner asked *"the txt list all have names you can look up. you really think my brother can do a better job than you?"* — searching each name found **41 of 42**, taking the day from 4 pins fixed to **85**. **A blocked scrape is not an absent fact.** Search the name (in its own language), read the venue's own site out of the results, check OSM for a named node, and only then ask the owner — saying which routes you tried. ⚠️ **Nominatim CANNOT geocode a Japanese address**: in romaji it returns a postcode centroid indistinguishable from a venue hit (20 of 23). Use the GSI API, which states the precision of its own answer. ⚠️ **Verify by ward, not by distance** — that is what caught an address 229 km away in Aichi and a caption whose postcode contradicted its own place name. 🔴 **Never move a pin on a district-centroid distance**: three such pins came out 11 m right, 220 m wrong and 100 m wrong, having read as 269 m, 144 m and unparseable. Full detail: `docs/link-pin-runbook.md` § Getting the coordinate right. |
| 8c | **A link-pin batch or city drop has been merged** | **Run `python3 scripts/check-place-candidates.py` and put every EXACT group to the owner.** A new batch routinely lands a second entry on a site that already had one, and the pairing is invisible while you are looking at your own pins. 🔴 **This has now been missed twice.** The script exists because a San Francisco batch shipped with three candidates in it and the owner found them on the map; on 2026-09-14 a 34-pin batch shipped with **nine**, and the owner again spotted two of them (La Piscine, the Farnsworth House) before the script was run — then a third, the Fremont Lenin statue, unprompted. ⚠️ **A place is a COORDINATE: creating one MOVES every member onto a single point**, and `validate-tours` errors if a member's first stop is not exactly on it. Read `docs/places.md` first — two entries qualify, and part-vs-whole is explicitly undecided and goes to the owner. 🔴 **Then run `python3 scripts/join-places.py --max-move 100` too.** `check-place-candidates.py` only pairs UNPLACED entries, so an entry that belongs to a place that already exists is invisible to it; and join-places' own default radius is 25 m, which on 2026-09-22 had hidden **nine** exact-name joins at 26–96 m. CI now reports this in the *Place candidates* job. ⚠️ **Read each one before `--apply`** — a join can go the wrong way (MahaNakhon: the stray pin was the accurate one, so the place moved onto it), and **an Atlas tour's stop now sits ON its subject, exactly like a pin** (owner, 2026-09-23: *"as the catalog evolved I think [a deliberate spot to stand] would be increasingly difficult to manage for a large catalog"*). Move it as you would a pin. The one exception is a multi-stop walk whose stops are separate parts of one site (AMNH *Four Facades*): each stop's subject is its own facade, not the building's centre. |
...
| 10 | Opening or merging a PR · dispatching a TestFlight build · finding or clearing an owner-blocked item | **Record it with `python3 scripts/status.py`, in the same commit — NEVER by editing `STATUS.md` by hand.** `--add-entry` for a session update, `--add-owner <slug> <title>` for something the owner owes, `--add-build <n> <title>` after dispatching, `--clear-owner <slug>` when it is done (a **delete**, not an edit). 🔴 **This is why:** the board used to be one file every session prepended to, so two sessions writing at once collided every time — **67 commits in a week, and #776, #820 and #843 each blocked by a documentation conflict, none of them a disagreement about code.** One file per entry cannot collide. **Re-derive, never predict:** `gh pr list --state open`, and read a build number back from the Actions run list. ⚠️ A **merged** PR that still needs an owner paste is NOT finished — it becomes a `status/owner/` item. |
...
## ⚠️ Egress — `get_catalog` is 2.3 MB and Supabase bills every byte
...
document**, and there is no way to ask it "has anything changed?" (it is a
...
|---|---|---|
| the whole catalogue (keys, contract, audits) | `curl --compressed` / `Accept-Encoding: gzip` — **neither curl nor urllib asks for compression on its own** | 2.3 MB (was 3.4, was 10.5) |
...
`tours` rows with `kind='link'`, so it returns tours **+** pins (3,441 on
2026-09-12 against the RPC's 1,553 tours). Use it for "is the row count what I
expect", never as the catalogue's tour count.

🔴 **And when it does NOT match, that gap is a real signal — the database can
only ever be a SUPERSET of `Tours.json`.** `seed_from_toursjson.py` is
**upsert-only for tours and pins**, so a deletion from the catalogue reaches
the gh-pages mirror and the bundled seed and **never reaches Postgres, which
is the app's primary source**; removing content is always a two-part change,
the catalogue edit **plus** an SQL paste. Diff them with a paged id fetch
(`?select=id,title,kind&order=id&limit=1000&offset=N`, ~45 KB a page —
PostgREST silently caps at 1,000 rows, so **a single un-paged fetch will
invent thousands of false differences**), and **compare ids case-insensitively:
`Tours.json` stores pin ids UPPERCASE and Postgres returns them lowercase**, so
a naive diff reports every pin as missing. Two legitimate sources of extra rows:
**in-app maker uploads**, which live only in Postgres by design, and maker rows
with no content. Anything else is content the app is still serving after it was
deleted.
...
### Key facts
- **1582 tours + 3712 link pins, 528 makers, 1954 tour stops (5666 including one per pin), 442 places** in `Resources/Tours.json` (re-derived 2026-09-23 on `main` at `178f0f32` plus Queens Museum, Ara Pacis, Casa de Chá da Boa Nova and the Museum of the City of New York; **783 cities, 93 countries**). 🔴 **The link pins are NOT in the `tours` array — they are a sibling top-level `linkPins` array**, because one unknown `kind` inside `tours` fails the whole catalog decode on every build shipped before `TourKind.link` (see `TRAVEL GUIDED TOUR/Data/ToursData.swift`). The app merges them back at decode, so everything downstream still sees one list. **35 of the makers are Atlas studios, the other 493 are pinned creators (315 Instagram, 166 TikTok, 12 YouTube) — pinned creators now outnumber the studios nearly fourteen to one, and Instagram has overtaken TikTok as the largest platform.** ⚠️ This line has gone stale FORTY-TWO times already, and **three parallel sessions invalidated it on the same afternoon** — it has been rewritten inside a single session more than once because `main` moved under it every time, and not one session's own number has survived its merge — #733 invalidated it AGAIN during the very merge that was correcting it, while that branch sat open, and **#738 did it a fifteenth time while session 144’s own PR sat open with green CI, so it went stale inside the very merge that was correcting it — then #737 and #739 did it a sixteenth, invalidating session 145’s own number while ITS PR sat open, #742 did it a seventeenth while session 147’s batch was being validated, and #743 did it an eighteenth while session 148’s heroes were mid-upload, and **#758 did it a NINETEENTH inside its own merge — it landed 83 pins and left this line reading 1,343; a later session simply counted and got 1,426 — and a TWENTIETH time when #769 made Trinity Church, Madison Square Garden and Penn Station places and left this line reading 137 against a real 140 — and a TWENTY-FIRST time: this line read 1,551 pins while the catalogue held 1,588 — and a TWENTY-SECOND time, when it read 1,588 / 353 against a real 1,635 / 354 before session 155's own batch had added anything, because #784 and #787 both landed between the last correction and that session's start**. Session 156's own #790 (21 pins) landed cleanly on top of #789 with nothing else merging mid-flight — and #801 moved the **places** figure from 141 to **181** in one merge, the largest single jump it has taken; ⚠️ #802 landed between that branch's CI going green and its merge, so `main` moved under it once more — and a TWENTY-THIRD time, hours later: this line had just been corrected to 1,773 / 380 against the base carrying `ddba8ba` when #811's six pins merged and made it stale again, and `places` moved 250 → 266 in the same window — then a TWENTY-FOURTH, inside the very PR correcting the twenty-third: #814 and #815 landed while that docs branch waited on an 18-minute simulator build, taking 1,779 / 381 to 1,794 / 384 before it could merge, which is the whole argument for never quoting this line — and a TWENTY-FIFTH on 2026-09-12, when #834's 16 pins landed after the previous correction and left this line reading 1,870 / 387 / 499 against a real 1,886 / 388 / 501 — and a TWENTY-SIXTH the next day, when #867's 52 pins and #870's coordinate fix left it reading 1,886 / 388 / 501 against a real 2,043 / 389 / 532, **and the countries figure moved too, 64 → 67, which this line's own history shows it almost never does** — — and a TWENTY-SEVENTH on 2026-09-15, in the quietest way yet: #896's 20 Tokyo/Hakone pins and three place PRs (#901, #903, #905) landed in the hours after the previous correction, leaving this line reading 2,195 / 302 / 544 against a real 2,215 / 305 / 545 — **and the STUDIO count moved for the first time in this chain, 34 → 35, because Atlanta (#900) opened the 35th bureau; a session that corrected only the tour and pin figures would have left that one wrong.** — and a TWENTY-EIGHTH on 2026-09-15, the same day as the twenty-seventh: #910's 103 pins landed while a coordinate-precision pass was mid-flight, leaving this line reading 2,215 / 425 against a real 2,318 / 429 — **and CITIES moved 545 → 579 in that one merge, 34 of them, on the figure this line's own history calls the one that almost never moves**; a session correcting only tours and pins would have left the largest error of the three behind — and a TWENTY-NINTH on 2026-09-16, the plainest case yet: this line read 2,318 / 429 / 305 against a real 2,474 / 469 / 323 **before this session's own batch had added anything**, because #938, #940 and #941 all landed in the hours after the previous correction — the figure was wrong on every one of the three counts the moment it was written — **and then a THIRTIETH, inside this very PR: #943's 65 pins merged to `main` while #946's own CI was queuing, so the corrected 2,474 / 600 was stale before a single check had reported and had to be re-derived a second time, on the merge commit, to 2,539 / 606** — — and a THIRTY-FIRST on 2026-09-16, the largest single-day drift yet: **2,539 → 2,799 pins, 324 → 344 places, 604 → 619 cities and 469 → 472 makers**, from #959/#961's 260 food pins landing in an afternoon. — and a **THIRTY-SECOND on 2026-09-17, the largest COUNTRIES move this line has ever recorded**: it read 2,799 pins / 344 places / 619 cities / **69 countries** against a real 2,984 / 351 / 685 / **88**. Nineteen countries and sixty-six cities in a day, on the two figures this line's own history keeps calling the ones that almost never move — and the pinned-creator split moved with them (434 → 437, Instagram 273 → 276). **A session correcting only tours, pins and places would have left four of the six figures wrong.** 🔴 **And the COUNTER had fallen THREE behind the corrections it counts** — it still read TWENTY-SEVEN while #918 called itself the 28th and a 29th was made in a PR that then sat unmerged for a day, so the tally undercounted every time someone fixed the numbers without fixing the sentence. **The tally is part of the line; move it in the same edit.** — — and a THIRTY-THIRD on 2026-09-18, the first time PLACES was the figure that moved most: **351 → 388, thirty-seven of them**, because #993/#995 began minting places automatically from a shared Wikidata id rather than one at a time by hand. Pins went 2,984 → 3,083, cities 685 → 704, countries 88 → 89. — **a THIRTY-FOURTH on 2026-09-20, and the plainest demonstration yet that the line cannot be maintained by hand: it read 3,083 pins / 388 places / 704 cities / 89 countries while `origin/main` ALREADY held 3,226 / 390 / 707 / 90 before this session's batch added anything — every one of the four already wrong, from merges in the intervening days. This session's 54 pins then took it to 3,280 / 724 / 91, and MAKERS moved 473 → 514, forty-one of them, the largest single move that figure has taken. 🔴 A session correcting only tours and pins would have left four of the six figures wrong, again.** — and a **THIRTY-FIFTH on 2026-09-20, hours after the thirty-fourth, in a NEW way: the session that corrected the line then invalidated it TWICE MORE ITSELF.** #1017 set it to 3,280 / 390; that same session's #1022 and #1024 took places 390 → 398 and it did not go back. Every previous staling in this chain was another session moving `main` underneath; this one was the author's own later merges. 🔴 **The rule 'a live count moved → re-derive' applies to your OWN subsequent PRs, not just to other people's.** Five of seven figures were wrong when re-derived (pins 3,280 → 3,293 with #1025 landing too, makers 514 → 515, places 390 → 398, cities 724 → 731, creators 479 → 480); only tours and countries held. — and a **THIRTY-SIXTH on 2026-09-21**, the quietest kind: an overnight run's own #1023 took places 398 → 408 and another session's pins 3,293 → 3,294 while this line sat unchanged. Only two of seven figures moved, which is exactly when a session is most tempted to skip the re-derivation and quote the rest. — and a **THIRTY-SEVENTH on 2026-09-21**, hours after the thirty-sixth and in the way this chain keeps proving is the normal one, not the exception: the line read 3,294 pins / 515 makers / 731 cities / 91 countries against a real 3,348 / 520 / 743 / 92 **before this session had added a single entry** — five of seven figures wrong from other sessions' merges alone, including Instagram 304 → 309. Only tours and places held. 🔴 **A figure that had only ever moved a few at a time now moves in batches, because the thing that produces it stopped being a person.** — and a **THIRTY-EIGHTH on 2026-09-21, the sharpest demonstration in the whole chain: it went stale inside the very hour a session wrote "re-derived and UNCHANGED" in a commit message.** That claim was TRUE when made — nothing had merged in between — and #1040's 60 pins landed minutes later, while that session's own docs PR was still in CI. Pins 3,348 → 3,408, makers 520 → 526, cities 743 → 752, countries 92 → 93, Instagram 309 → 314: **five of seven figures, from one merge.** 🔴 **Verifying the line is not the same as the line staying verified, and the window between the two is minutes.** Re-derive at the moment you write, and expect it to be wrong again by the time anyone reads it — which is the whole reason this line says to re-derive rather than quote. — and a **THIRTY-NINTH on 2026-09-21**, which is the thirty-eighth's own lesson arriving by return of post: that entry was written to say the window between verifying the line and the line going stale is *minutes*, and it was itself stale by the time the PR carrying it merged. Pins 3,408 → 3,540 and cities 752 → 768, from #1045's 64 pins and the merges around it; tours, makers, places, countries and the Instagram split all held. 🔴 **And the TALLY had fallen SIX behind again** — the sentence still read THIRTY-THREE while the chain below it had reached thirty-eight, the same drift this line recorded at the thirty-second and then repeated. **The tally is part of the line; move it in the same edit.** — and a **FORTIETH on 2026-09-22**, which arrived with a matching lesson about a DIFFERENT number going stale the same way. Pins 3,540 → 3,657 and cities 768 → 774 from #1054's 88 pub-crawl pins and #1056's 29, with makers 526 → 527 and TikTok 165 → 166; tours, places, countries and the Instagram split held. 🔴 **The same re-derivation caught a live regression** — captions cut at 140 characters had been driven down to 24 the day before and were back to **111**, because #1054 was built on a branch taken BEFORE `make-link-pin.py` stopped truncating and carried the old code into `main`. Every check was green and the validator clean; it surfaced only because this line was being re-derived by hand. **Re-deriving a count is not bookkeeping — it is the only check some defects meet.** — and a **FORTY-FIRST on 2026-09-22**, which is the thirty-fifth's lesson repeating with nobody else involved: the line read **408 places** while `origin/main` ALREADY held **416**, because #1058 minted eight and did not move the figure it had just invalidated. 🔴 **Every other count held** — tours, pins, makers, stops, cities and countries were all still right — **which is exactly when a session skips the re-derivation and quotes the rest**, and the one figure that had moved was the one this session's own previous PR moved. The pinned-creator split was a second, quieter miss in the same sentence (491 / 165 TikTok against a real 492 / 166), stale from a merge nobody had reconciled. **A PR that changes a counted thing owes this line an edit in that same PR, and 'only one number moved' is the weakest possible reason to trust the others.** — **re-derive it, never quote it** — `grep -c '"displayName": "TikTok \|"displayName": "YouTube \|"displayName": "Instagram '` against the catalogue is the whole check. (101 Atlas Studio NYC + 100 Atlas Studio LDN + 71 Atlas Studio KYO + **68 Atlas Studio BCN** + **48 Atlas Studio MIL** + 66 Atlas Studio LIS + 63 Atlas Studio TYO + 57 Atlas Studio BKK + 54 Atlas Studio OPO + 52 Atlas Studio HKG + 50 Atlas Studio PAR + 46 Atlas Studio RIO + **45 Atlas Studio STO** + **40 Atlas Studio CPH** + 43 Atlas Studio CNX + 43 Atlas Studio SEL + 43 Atlas Studio SGN + 42 Atlas Studio LAX + 42 Atlas Studio SAO + 42 Atlas Studio YYZ + 38 Atlas Studio AMS + 37 Atlas Studio ROM + 36 Atlas Studio BER + 36 Atlas Studio BUE + 35 Atlas Studio MEL + 35 Atlas Studio SFO + 34 Atlas Studio MAD + **30 Atlas Studio CPT** + 30 Atlas Studio ORD + **30 Atlas Studio ATL** + 29 Atlas Studio SYD + 29 Atlas Studio YUL + 26 Atlas Studio DXB + 26 Atlas Studio RAK + 15 Atlas Studio NAO); audio on `gh-pages` at `https://ehky2882.github.io/TRAVEL-GUIDED-TOUR/audio/<file>.mp3`. **The catalog is remote-loaded** via `RemoteCatalogLoader`: since **PR #255 (2026-06-27)** the primary source is the **Supabase `get_catalog` RPC** (project "Dozent"), with `https://ehky2882.github.io/TRAVEL-GUIDED-TOUR/Tours.json` as a fallback mirror, then the on-disk cache, then the bundled offline seed. `.github/workflows/publish-catalog.yml` still auto-publishes the gh-pages mirror on every content merge to `main`; **but Supabase is now primary, so content changes must also reach the DB (rerun `backend/seed_from_toursjson.py`)** or the mirror could be newer than the live source. (Shipped in **TestFlight 1.0 (50)**, live 2026-06-27.)
- **1510 single-stop + 72 multi-stop** — all geofenced. **Atlanta added 30 singles with no walks**; Copenhagen added 40 singles with no walks; Rio launched as 46 singles with no walks; São Paulo added 41 singles + 1 walk; Berlin added 31 singles + 5 walks; Marrakech added 26 singles with no walks; Buenos Aires added 34 singles + 2 walks; Chicago added 25 singles + 5 walks; Melbourne added 34 singles + 1 walk; Sydney added 29 singles with no walks; Cape Town added 30 singles with no walks; Barcelona added 66 singles + 2 walks; Milan added 47 singles + 1 walk; **Stockholm added 42 singles + 3 walks**. Multi-stop walks by maker: London 5, Paris 5, Amsterdam 5, Rome 5, Berlin 5, Chicago 5, San Francisco 4, Toronto 4, Los Angeles 4, Madrid 4, Montreal 4, Dubai 4, Seoul 3, **Stockholm 3**, NYC 2, Naoshima 2, Buenos Aires 2, **Barcelona 2**, Bangkok 1, São Paulo 1, Melbourne 1, **Milan 1**. The 4 originally-named NYC/London walks ("American Museum of Natural History: Four Facades" (5 stops, NYC), "Fifth Avenue Walk" (6 stops, NYC), "After the Fire: Wren's City" (6 stops, London), "Albertopolis" (6 stops, London)) are still the reference multi-stop test cases; AMNH unblocks M-qa items 6 + 7.
...
- **Every tour carries `city` AND `country`** (`country` added session 99 — **704 cities, 89 countries**, re-derived 2026-09-17 on `main` + #988 — ⚠️ cities went DOWN by two, which this line has never done before: `Sao Paulo`/`São Paulo` and `Zurich`/`Zürich` were the same city spelled two ways and were merged, so an accent-folded duplicate check belongs in any batch that authors a city name — a TWENTY-EIGHTH staling: this line read 579 / 68 against a real 600 / 69 before this session's batch landed, **and the countries figure moved for the third time in four days**, on the figure this line's own history keeps calling the one that almost never moves; before that #910's 103 pins added 34 cities in a single merge; a TWENTY-SEVENTH staling: this line read 532 / 67 while a 33-pin batch took it to 542 / 68 in a single merge, **and the countries figure moved again — twice in two days, on a figure this line's own history says almost never moves**; the figures this line carried before that read 532 / 67, re-derived 2026-09-13 on `808af766`; the figures this line carried before that read 501 / 64; the figures this line carried before that read 466 / 64 and were **35 cities stale** across tours and link pins together — before that 447 / 63 — the figures this line carried before that read 442 / 63 — the figures this line carried before that read 427 / 63 — the figures this line carried before that read 414 / 61; the figures this line carried before that read 404 / 59 — stale within hours, because #758 added 83 pins without touching them — and before that 389 / 58 — session 147's batch alone added **eleven countries**, the largest single expansion — and before that 359 / 47, the figures this line carried before that read 274 / 41, then 250 / 40, then 203 / 37 — and the 250 was already stale against `main`'s 259 when it was written, so **re-derive rather than quoting it** — `docs/app-store-screenshots.md` § Scale figures has the one-liner). `country` is denormalised onto the tour exactly as `city` is, so it travels with the content and updates over the air; **a new city batch must author it** or that tour drops out of the Settings → About count. `Tour.country` is optional so the bundled seed, the gh-pages mirror and maker-authored tours all keep decoding. Its column + `get_catalog` key are live (`backend/add_country.sql`, applied 2026-08-19).
- **Tours AND link pins carry `relatedTourIds` — the "More like this" section** (up to 8 neighbour ids; 3,482 of 3,900 entries have at least one — re-derive, never quote). **Pins take part in both directions and may name other pins**, on the owner's decision of 2026-09-15; #915 had excluded them on an unmeasured guess about transcripts that turned out to be wrong (`docs/lessons.md`). 🔴 **A pin takes NO cross-city fill — a tour does.** 978 pins sit in a city with no Atlas tours, and filling their slots from elsewhere offers someone in Prague a walk in Vienna; `validate-tours.swift` enforces this and it is the only mechanical guard on it. ⚠️ **Letting pins in costs +291 KB gzip-1 on a payload that has drawn two egress overage notices** — measured on the live body, not estimated; see § Egress. Generated **offline** by `scripts/build-embeddings.py --write-related` from sentence embeddings and shipped in the catalogue, so **the phone runs no model**. ✅ **The re-run is AUTOMATIC since 2026-09-15** — the `rebuild-related` job in `.github/workflows/publish-catalog.yml` regenerates on every content merge, validates, and commits to `main` with `[skip ci]` before the mirror and the Supabase seed run, so a new city ships WITH its suggestions. Do not run it by hand after a city launch; do still **re-check the 0.45 floor with `--related-of`** rather than assuming it still separates good matches from filler. ⚠️ **The vectors are deliberately NOT cached** — a stale cache would keep a wrong neighbour list with no error and no red build, and the compute is free; only the model download is cached. ✅ **What IS cheap is knowing whether a rebuild is owed at all:** `scripts/related-text.digest` is a sha256 of exactly what the model reads, and the job skips in under a second when it matches. **Most content merges change no embedded text** — #930 moved 34 entries' coordinates across 170 changed lines and the digest did not move one bit, because `tour_text` never reads a coordinate or an image URL. 🔴 **The digest is committed in the SAME commit as the catalogue and advances only after a rebuild that actually succeeded**, which is what makes it self-correcting where a per-entry cache would not be. 🔴 Similarity is **mean-to-mean, deliberately not `tour_scores`' best-chunk blend** — see `docs/lessons.md`. **`createdAt` is served from a NEW nullable `authored_on` column, never from `created_at`** (an audit column holding seed time; serving it would rank four sort controls by seed order while looking fixed). ⚠️ **`backend/add_related_tours.sql` ends with `refresh_catalog_snapshot()` and that line is load-bearing** — `get_catalog()` serves a pre-built snapshot, so the migration patched the builder correctly, printed "Success. No rows returned." and changed nothing a phone could see for 25 minutes (`docs/lessons.md`). ⚠️ **It must also be applied BEFORE a content merge** — the seed writes both columns under `ON_ERROR_STOP=1`, so without them every catalogue publish aborts. ⚠️ **36 tours have no `createdAt` and were deliberately not invented**: 35 are the whole of Atlas Studio SFO, and this checkout is a shallow clone whose history stops at 2026-08-17, so the dates are not recoverable here. They sort last, as `Tour.createdAt`'s optionality promises.
...
| `scripts/status.py` | **The live board.** One file per entry under `status/`, so parallel sessions cannot conflict. Read with `python3 scripts/status.py`; write with `--add-entry` / `--add-owner` / `--add-build` / `--clear-owner`. **Never hand-edit `STATUS.md` to log something** |
| `status/owner/` | One file per thing the owner owes. **Clearing one is `git rm`, not an edit** — which is what keeps it conflict-free |
| `archive/STATUS-HISTORY.md` | The finished items moved out of `STATUS.md`, plus the dated header chain and the build/branch tables the 2026-09-13 restructure retired. **Search, never load whole** |
