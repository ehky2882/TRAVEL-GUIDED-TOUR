...
- **Also shipped this session: create-a-tour (draft) + audio import.** **Increment 2b ([PR #307](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/307), merged `9f398fe`)** — the "+" opens a real create-a-tour form (title/desc/category/tags + a MapKit pan-to-place pin + geofence radius circle/slider) writing a `draft` `tours`+`stops` row via new `Data/MakerTourService.swift`; the own-profile feed now shows the user's own tours across all statuses (`myTours`) with **DRAFT / IN REVIEW** badges (was the published catalog); `MakerProfileService.ensureMaker()` lazily creates the maker row on first authoring action; new `Models/TourStatus.swift`. **Increment 2c-audio ([PR #310](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/310), OPEN — owner review)** — tapping an owned tour opens a new `Features/Profile/TourAuthoringView.swift` editor; its Audio step **imports an audio file and uploads to the `tour-audio` Storage bucket** (`{maker_id}/{tour_id}/file`) then patches the stop `audio_url`/duration + tour duration (`MakerTourService.attachAudio`); Photos/Transcript/Submit shown as "Coming next". Import first (sim-testable); in-app recording is the next piece.
- **TestFlight 1.0 (62) is LIVE** (owner-confirmed on device 2026-07-02) — bump #308 (`88f75e2`), archived from a clean `main` checkout with `-allowProvisioningUpdates`, verified (`1.0 (62)`, `UIRequiresFullScreen` held, Apple + `associated-domains` entitlements, Supabase host + OAuth scheme compiled in, no poison string). **Carries the maker-authoring stack: profile tab (#300), standalone creator screen (#302), create/edit profile (#304), create-a-tour form (#307)** — plus the share/Universal-Links work already in 61. **Not yet in a build:** the 2c-audio editor/import (#310, in review) — fold into the next cut.
- **NEXT after 2c-audio:** in-app **recording** (AVAudioRecorder + `NSMicrophoneUsageDescription`), then **photos** (PHPicker → crop 1200×900 → `tour-images` bucket → patch hero/additional), **transcript** (patch `stops.transcript_text`), and **Submit** (`status` draft→`in_review`, wiring the deferred `tours` UPDATE→in_review `notify-moderation` webhook) → admin **Publish**.
