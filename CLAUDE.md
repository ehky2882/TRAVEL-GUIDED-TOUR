...
- **⚠️ New operational rule:** Supabase is now the **primary** source, so content changes must reach the DB (rerun `backend/seed_from_toursjson.py`, idempotent upsert), not only gh-pages — else the fallback mirror could be newer than the live source. Both currently mirror the same `Tours.json`. Worth automating later (a publish-to-Supabase step alongside `publish-catalog.yml`).
