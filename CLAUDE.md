...
### 🐛 Bottom-module install race fixed — mini-player/tab bar could be missing on launch (session 58 — code, PR open, awaiting device verify)

**The intermittent "no mini-player + tab bar on some launches" bug (owner + multiple TestFlight testers, no reliable repro) is a cold-launch timing race — fixed in [PR #357](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/357), CI + `test_sim` green, NOT merged (waiting on owner device confirmation).**

- **Root cause.** The mini-player + tab bar render in a **secondary `UIWindow`** (`Components/BottomModuleWindow.swift` → `BottomModuleWindowController.install()`), which requires a `.foregroundActive` window scene, **returned silently if none existed**, and was called **only once** from the App body's `.onAppear`. On launches where `.onAppear` fired before the scene reached `.foregroundActive`, install bailed with `window` still nil and **nothing retried it** → the whole bottom module was absent for the session. The adjacent `.onChange(of: scenePhase)` only refreshed the catalog. (The build-71 Home keep-alive change — heavier first layout — plausibly worsened the timing.)
- **Fix — recover instead of giving up once.** (1) The App now **also calls install on `scenePhase == .active`**, factored into `installBottomModule()` so the `.onAppear` site and the retry inject the **identical** `@Environment` services (can't drift). (2) `install()` is hardened: when no active scene, it registers a **one-shot `UIScene.didActivateNotification` observer** that retries with the scene that activated (iPad-multi-scene-correct — uses `note.object`), then removes itself; now returns `Bool`. (3) Both paths hit the same `guard window == nil` → **created exactly once, no double-install**. (4) A deferred install re-applies the cached `ColorSchemePreference` (the App's `onAppear` `apply()` ran while `window` was nil). All the window's other duties untouched (`windowLevel = .normal+1`, `PassThroughWindow`/`interactiveBottomInset`, the `overrideUserInterfaceStyle` `.onChange` sync, toast host, player, deep links).
- **Tests.** Extracted the pure decision into `BottomModuleWindowController.installOutcome(hasWindow:hasActiveScene:)` + new **`BottomModuleWindowTests`** (installNow / deferUntilActive / alreadyInstalled). **`test_sim` 162/162** (159 + 3) on iPhone 17 Pro / iOS 26.5; normal launch still renders the module (sim-verified, screenshot). A `Logger` line fires on successful install for device triage.
- **⚠️ Device verification owed (owner).** The race can't be reliably reproduced in the sim, so confirmation = launching the next TestFlight build **repeatedly** (cold force-quit + warm background→foreground, several each, ideally on the device it broke on); the module should appear **every** time. **Then merge #357 and it can ride the next build** (no build cut yet for this).
...
Key facts:
- **509 tours, 9 makers, 561 stops** in `Resources/Tours.json` (100 Atlas Studio NYC + 99 Atlas Studio LDN + 66 Atlas Studio LIS + 54 Atlas Studio OPO + 52 Atlas Studio HKG + 35 Atlas Studio SFO + 63 Atlas Studio TYO + 10 Atlas Studio YYZ + 30 Atlas Studio KYO); audio on `gh-pages` at `https://ehky2882.github.io/TRAVEL-GUIDED-TOUR/audio/<file>.mp3`. **The catalog is remote-loaded** via `RemoteCatalogLoader`: since **PR #255 (2026-06-27)** the primary source is the **Supabase `get_catalog` RPC** (project "Dozent"), with `https://ehky2882.github.io/TRAVEL-GUIDED-TOUR/Tours.json` as a fallback mirror, then the on-disk cache, then the bundled offline seed. `.github/workflows/publish-catalog.yml` still auto-publishes the gh-pages mirror on every content merge to `main`; **but Supabase is now primary, so content changes must also reach the DB (rerun `backend/seed_from_toursjson.py`)** or the mirror could be newer than the live source. (Shipped in **TestFlight 1.0 (50)**, live 2026-06-27.)
- **499 single-stop + 10 multi-stop** — the 4 named NYC/London walks ("American Museum of Natural History: Four Facades" (5 stops, NYC), "Fifth Avenue Walk" (6 stops, NYC), "After the Fire: Wren's City" (6 stops, London), "Albertopolis" (6 stops, London)) plus San Francisco's multi-stop walks — all geofenced. AMNH unblocks M-qa items 6 + 7.
- **All 52 Hong Kong (HKG) tours (`English | 中文`), all 63 Tokyo (TYO) and all 30 Kyoto (KYO) tours (`English | 日本語`) are bilingual** — on both tour and stop titles.
