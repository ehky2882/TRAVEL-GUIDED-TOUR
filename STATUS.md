...
⚠️ **Re-derived 2026-09-05: [#734](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/734) has MERGED** (33 Toronto/Mississauga pins, 6 places, 2 architects), so every Toronto line below is stale on the PR itself and survives only for its owner decisions. **Re-derive before trusting any line here.**

🟡 **NO PR — branch `claude/new-tour-links-lze4ab`, 41 link pins from TikTok `@itshistoryonair` (session 144).** The owner did not ask for one, so **nothing has compiled and nothing is live.** 46 owner-supplied links + Plus Codes → **41 distinct posts** (five pasted twice) → **41 shipped, 0 blocked**. On the merged base: `linkPins` **1,201 → 1,242** · `makers` **328 → 329** · `tours` and `places` **byte-identical**. **Cyprus and Bolivia are the catalogue's 46th and 47th countries**; 14 new cities. Content only — no Swift, no SQL, no place created; the seed carries it, so it would reach Supabase on merge with **nothing for the owner to run**. Heroes on gh-pages `94264b6c` (tree diff **exactly 42 additions, 0 deletions, nothing outside `images/`**, 7,020 → 7,062 paths, **re-verified against the remote**; deploy read **`in_progress`, never `cancelled`**, then **all 42 live URLs hash-verified against the uploaded bytes — 42 ok, 0 mismatch, 0 non-200**, after which `check-image-duplicates.py --pins` reads **`OK` over 1,237 images**, shared-URL half **0 errors / 208 documented reuses**). ⚠️ **`main` moved mid-validation (#734) and the edit was redone the documented way** — reset onto `main`, re-run the idempotent assembler, **0 overlap on sourceURLs, ids, maker id and hero filenames**, every check re-run, diff unchanged at **1,897 / 0**.
  - 🔴 **OWNER DECISION OWED — SIX SITES SIT EXACTLY AT `TourSetMap.maxStacked = 3`, AND ALL 41 PINS ARE ONE CREATOR**, so they stack on that creator's own maker page. **Three deep with NO headroom: Alcatraz · Hoover Dam · Fort Jefferson · the Leaning Tower of Niles.** Two deep with one spare: Paris Catacombs, Washington Monument, Ellis Island, Seven Mile Bridge, Alang. **Nothing is unreachable today**, but a fourth link at any of the four would put one marker permanently out of reach, invisibly. **Recommendation: make places for the four.** ⚠️ **A fifth, Eastern State, is a different shape** — the new pin sits **3.7 m** from the existing two-member place, so it is **not coincident**, never reaches EXACT, and joining it as a third member is a deliberate one-line move of a `manual` pin (the Guggenheim / ROM Crystal precedent). **Nothing was created here.**
...
🟢 **[#733](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/733) — branch `claude/tour-links-7ova6v`, 17 link pins from `@archimarathon` + the Serlachius place + 6 architects, session 143. OPEN on owner instruction.** ⚠️ **Now a CODE PR** (`Models/Tag.swift`), so it wants a simulator look; CI is the compile check.. 16 TikTok short links + 1 Instagram reel, **all one creator on two platforms**. `linkPins` **1,151 → 1,168** on the merged base; `tours`, `makers` and `places` byte-identical, **0 new maker rows** — the uuid5 scheme reproduced BOTH live ids field-for-field, and the avatar regenerated byte-identically and was **excluded rather than overwritten** (18 generated, 17 uploaded). Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with **nothing for the owner to run**. Heroes on gh-pages `0b15aa4` (tree diff **exactly 17 additions, 0 deletions, nothing outside `images/`**; deploy read **`in_progress`, never `cancelled`**, then **all 17 live URLs hash-verified against the uploaded bytes — 17 ok, 0 mismatch, 0 non-200**). **All three flagged owner decisions have now been ACTED ON:**
  - ✅ **SERLACHIUS IS NOW A PLACE (owner instruction).** The three coincident markers collapse into one capsule carrying `placeTourCount: 3`, so `TourSetMap.maxStacked` stops applying. **Nothing moved** — all three were already exactly on OSM's own `Serlachius Kartano` node, so the identity rule held on its own. Places 114 → 115; `check-place-candidates.py` **13 EXACT → 12**, falling by exactly that group and gaining nothing. ⚠️ **Its hero is BORROWED from a member and that is structural** — every member is a link pin with an empty gallery and there is no Atlas tour at the site, so no third photograph of it exists. **Do not source a replacement.**
  - ✅ **THE FOUR WEAK HEROES ARE CLOSED — owner: *"Don't worry about weak heroes."* THEY SHIP AS DELIVERED; do not re-raise or replace them.** ⚠️ **Anyone re-running the open-every-hero audit will flag all four again; they are settled** (the Ministry of Enterprise / Casa Lleó Morera precedent). Original finding: `Yoyogi National Gymnasium` is a **pure talking head** with nothing of the gymnasium in frame — the weakest; 🔴 **`Higashi Sanchome Public Toilet`'s hero is a building** while its caption *and* its supplied coordinate are both about a toilet (the reel is a compilation covering both); `Jingu-Dori Park` and the Sayamaike display model are milder. A link pin re-hosts only the thumbnail, so no other frame exists — the choice was only ever keep or pull, and the owner kept.
  - ✅ **THE ARCHITECT GAP IS CLOSED (owner instruction) — 6 names added to BOTH vocabularies, 419 → 425**, and 7 entries gained a name. 🔴 **The catalogue-wide sweep found two KYOTO TOURS** (`Ark Building`, `Pharaoh`) naming **Shin Takamatsu** as their author while carrying **no architect tag and no shelf tag** — the #493 mirror-image defect, closed. ✅ **AND THE TWO JULES DALOU OMISSIONS WERE THEN REVERSED ON OWNER INSTRUCTION (*"Yes I would like the architect tags"*) — vocabulary 425 → 427.** `Yoyogi National Gymnasium` gains **`Kenzō Tange`** (already in the vocabulary) and `Temppeliaukio Church` gains **`Timo Suomalainen` + `Tuomo Suomalainen`**, each **alongside** `Designed by a Master`, which neither had carried. ⚠️ **This is an owner decision overriding the rule for these two entries — both captions genuinely name no architect, so do not "restore" the omission.** ⚠️ **The Suomalainen name was verified externally, not recalled** — it appeared **nowhere** in the catalogue. 🎉 Tagging Yoyogi also makes the pin **match the Atlas TYO tour of the same building 67.8 m away**, so the pair shares shelves. 🔴 The sweep found a **fifth Tange mention that is correctly NOT tagged** — `Kyoto International Conference Center` names him only as the practice `Sachio Otani` came from (the Sullivan rule). Catalogue-wide after: **710 named-architect entries, 0 missing the shelf tag, 0 of 427 names unused.**
