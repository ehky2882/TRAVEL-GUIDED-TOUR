...
🆕 **OPEN — [#767](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/767) — thirty-six `@urbanistariel` link pins (session 152, 2026-09-09).** 36 TikTok links, all one creator, **all 36 ship** — 0 already pinned on `main`, 0 overlapping the open #749. **linkPins 1,453 → 1,489 · makers unchanged at 353 · `tours` byte-identical · places 137, two gaining members.** Content only — **auto-merge class**, squash on green CI.
  - 🔴 **Four pins would have overflowed `TourSetMap.maxStacked = 3` and were folded into places that already exist there** — Cathedral of St. John the Divine **5 → 8 members** (three separate posts about it) and The Morgan Library & Museum **5 → 6**. They take the place's own coordinate, as all ten existing members do; **no anchor moved**, including the geofenced Atlas tour. Deepest loose group containing a new pin afterwards is **3**.
  - ⚠️ **12 of the 36 links came with no location** and were derived from the caption, each checked against the bounding box of the city its caption names (**12/12 in box**) and reverse-geocoded at zoom 18 (**36/36**).
  - ⚠️ **Five pins were retitled**, three off a title an entry on the same spot already holds (`East Side Gallery`, `Cathedral of St. John the Divine`, `Conwell Coffee Hall`) and **two after looking at the picture rather than the caption** — *Inside* La Sagrada Família is a distant skyline shot, and `Wall Street's Slave Market` was a claim the caption never makes, so the creator's own card (`The Dark Secret of Wall Street`) ships instead.
  - ⚠️ **At the cap with no headroom:** `The Dark Secret of Wall Street`, 21 m from `The Red Room at One Wall Street` and 60 m from `The Buttonwood Tree at the NYSE`. Flagged, not acted on.
  - ✅ Mirror **selftest 32/32, control clean**, then **0 errors / 0 warnings across 1,552 tours + 1,489 pins + 137 places**. gh-pages `1d42a4b`: remote head re-read in the same command as the push, **36 additions / 0 deletions / nothing outside `images/`**; the creator's avatar regenerated byte-identically and was **excluded rather than overwritten**.
