...
**Last verified:** 2026-09-03 (session 141 — **sixty-one link pins from ONE creator, `@archimarathon`, on `claude/new-tour-links-cytwc6`. Pushed, NO PR opened.** `linkPins` **986 → 1,047**; `tours`, `makers` and `places` byte-identical, **0 new maker rows** (the uuid5 scheme reproduced the live `@archimarathon` row exactly). ⚠️ **The owner's heading read "Architmarathon" and the payload says otherwise — read the payload, not the heading.** **All 61 alive, 0 already pinned, 0 in-batch duplicates.** 44 of 61 coordinates named exactly by OSM; **4 are weak and stated rather than hidden** (Taipei Roofs is the weakest — no published address anywhere). 🔴 **The geocoder turned HTTP 429 into "NOT FOUND" and it took a hand query to catch** — a request that never completed must not be able to write a verdict; the harness now excludes errored rows entirely. **All 61 heroes opened and read against their captions — zero wrong subjects**; 4 flagged weak, Rozet the sharpest mismatch. ⚠️ **2 honest EXACT place candidates, neither coordinate nudged** — `check-place-candidates.py` goes **0 → 2 EXACT, 38 → 39 NEAR**. ⚠️ **The largest architect-vocabulary gap any batch has produced**, `Alvar Aalto` ×3 among them. Nothing compiled — no PR, so CI has not run.)
...
🟡 **`claude/new-tour-links-cytwc6` — 61 link pins from `@archimarathon` (session 141). PUSHED, NO PR — this session's harness forbids opening one unasked.** `linkPins` **986 → 1,047**; `tours`, `makers` and `places` byte-identical, **0 new maker rows**. Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with no owner SQL. **Two owner decisions are flagged and NOT acted on:**
  - **2 EXACT place candidates, and NEITHER coordinate was nudged to make them.** `check-place-candidates.py` goes **0 EXACT / 38 NEAR → 2 / 39**, ending the clean-exit state. Both are a new pin landing on an existing entry for the same subject, which is the honest shape: **Depot Boijmans Van Beuningen** (this batch's mirrored-facade pin lands on OSM's own museum node, where an existing pin already sits) and **the Cube Houses of Rotterdam**. Two markers is inside `TourSetMap.maxStacked = 3`, so nothing is unreachable — a place is the tidy fix, not a required one.
  ...
  - ⚠️ **The largest architect-vocabulary gap any batch has produced** — `Alvar Aalto` appears in **three** captions and is absent from `Models/Tag.swift`, alongside Aldo van Eyck, Willem Dudok, Gottfried Böhm, Peter Zumthor's neighbours and more. All ship the generic `Designed by a Master`. **Adding them is a CODE change** (`Models/Tag.swift` **and** `scripts/validate-tours.swift` — both, or it is an error per tagged tour) and was deliberately kept out of a content batch.
...
| Branch | State |
|---|---|
| `claude/new-tour-links-cytwc6` | **This session, pushed, no PR** — 61 `@archimarathon` link pins, cut off `origin/main`. gh-pages `317046a` carries its 61 heroes. |
...
**⚠️ Re-derived from `Tours.json` on the session-141 branch `claude/new-tour-links-cytwc6`: 1,552 tours + **1,047** link pins, 305 maker rows, 2,971 stops, 112 places.** Newest reading on this board — every count below it is an older snapshot, so **re-derive rather than quoting any of them**. ⚠️ **Not yet on `main`** (pushed, no PR), and ⚠️ **`main` moved twice while this batch was being wired** (#722, #725), so the base itself is a moving target.
