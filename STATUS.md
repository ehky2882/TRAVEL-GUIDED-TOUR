...
🟡 **OPEN — [#700](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/700) sixteen Hong Kong link pins (branch `claude/tour-links-9ynig6`, session 137).** Owner sent sixteen Instagram reels; all sixteen shipped. **linkPins 696 → 712, makers 246 → 247.** Content only — no Swift, no SQL, no build; the seed carries `linkPins`, so it reaches Supabase on merge with **no owner SQL**.
  - **Four weak heroes are the owner's call** — **#10 The Pokfulam Farm is a close-up of a blue COW SCULPTURE** with nothing identifying the farm or Hong Kong (the likeliest pull); PMQ is a basement, Green Hub an interior jail cell, HKDI a corridor detail. A link pin re-hosts only the thumbnail, so **no other frame exists** for any of them.
  - **Three place candidates, flagged not created** — Jamia Mosque (28 m), Sai Wan Swimming Shed (30 m) and PMQ (45 m) each sit beside an existing Atlas tour of the same subject. **Nothing was nudged**, so EXACT stays at zero. ✅ Unusually, **all three could take a real third photograph** from the Atlas tour's own gallery, so the borrowed-hero problem does not arise.
  - **⚠️ `Coldefy & Associés` is named in a caption and is absent from the vocabulary** — the Hong Kong Design Institute ships the generic `Designed by a Master`. Adding the name is a `Models/Tag.swift` **code** change for a future architect PR.
...
| Branch | State |
|---|---|
| `claude/tour-links-26dmsx` | **Open as [#701](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/701)** — twenty-eight Chicago link pins from 20 Instagram creators; linkPins 696 → 724, makers 246 → 261. Cut clean off `main` `eaa7cea`, then **`main` moved twice** (#698, #699 — 131 pins) and the catalogue edit was **redone by re-running the idempotent assembler on `main`'s file**, never hand-resolved. 🔴 **New maker rows went 17 → 15 because #699 created rows for `@bestaddress` and `@mvvvc_`** — uuid5 reproduced both ids exactly, so they merged; a naive resolve would have emitted `duplicate maker id` twice. ⚠️ **Four of the 34 links are blocked by Instagram** (`contextJSON: null`, confirmed against two live controls in the same pass) — **blocked now, not gone**; recheck in `archive/HANDOFF-260901-9.md`. ⚠️ **Adds 1 EXACT place candidate** — two Tribune Tower pins on one coordinate, honest convergence on OSM's own node, **three members counting the Atlas tour 142 m away, which covers two subjects** (the documented naming problem). ⚠️ **Five weak heroes flagged**, the Crain Communications Building sharpest. gh-pages `10ec78bf`, all 28 hero URLs hash-verified live. ⚠️ **Handoff filed as `-9`** — `-7` and `-8` were taken by parallel sessions |
