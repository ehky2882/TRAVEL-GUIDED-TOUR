...
🟡 **NO PR — branch `claude/tour-links-tp9fwp`, 29 link pins from `@archiwhisperer` and Avant Arte (session 140).** `linkPins` **957 → 986** on the merged base; `tours`, `makers` and `places` byte-identical. Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with **nothing for the owner to run**. Heroes on gh-pages `44d7cc00`, **all 29 live URLs hash-verified against the uploaded bytes (29 ok, 0 mismatch, 0 non-200)**; `check-image-duplicates.py --pins` run **after** the deploy: **`OK`** over 981 images, shared-URL half **0 errors / 208 documented reuses**. Mirror **22/22 with a clean control**, then **0 errors, 0 warnings** across 1,552 tours + 986 pins + 111 places; **`check-place-candidates.py` holds at 0 EXACT and exits 0 on both sides**, NEAR 36 → 43. **Serbia is the 42nd country.** ⚠️ **`main` moved twice mid-session** (#722, #725) and `Tours.json` conflicted — resolved by taking `main`'s file and **re-running the idempotent assembler**, every check re-run. **Five things want an owner call:**
  ...
  - 🔴 **ALWYN COURT IS NOW THREE DEEP — a place is the real fix.** The new pin lands **0.1 m** from an existing `Instagram @dmitrynycre` pin and **11.7 m** from an existing `TikTok @hereinnyc` pin: **three markers for one building, exactly `TourSetMap.maxStacked = 3` with no headroom.** ⚠️ **Nothing was nudged** — all three independently geocoded to OSM's own `Alwyn Court` node (convergence, the Tribune Tower case), and it reports **NEAR rather than EXACT** only because they differ in the 7th decimal. This is the Windsor Castle / Yankee Stadium / 120 Broadway shape; **flagged, not built**, since a place needs its own copy, address, photograph and approval.
  - 🟡 **The Guggenheim pin could join the EXISTING Guggenheim place as a third member.** It sits **29 m** from the place and from both its members; membership is exact coordinate equality, so this means moving a `manual` pin 29 m (the ROM Crystal precedent). One line either way.
  - 🟡 **The Gilder Center is 176 m from the AMNH place** (6 members). It is AMNH's own wing with its own Columbus Avenue entrance, so it is arguably a member on the Tudor Kitchens reading — but it is **not coincident**, so it was left as a separate pin rather than moved. Owner's call.
