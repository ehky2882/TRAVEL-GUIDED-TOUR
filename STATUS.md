...
⚠️ **NOT YET A PR — branch `claude/new-tour-links-nl8h63`, 13 Studio Gang link pins (session 144, 2026-09-05).** Committed as `88203b53` and pushed; **no PR opened, because the session's harness forbids opening one unasked.** `linkPins` **1,201 → 1,214**, `makers` **328 → 331**; `tours` and `places` byte-identical. Content only — no Swift, no SQL, no build. Heroes are live on gh-pages (`d22dca3`), all 13 hash-verified against the uploaded bytes.
  ...
  - ⚠️ **A `Tours.json` CONFLICT IS EXPECTED.** A parallel session pushed 41 `@itshistoryonair` heroes to gh-pages (`94264b6c`) without merging its catalogue change; both sides append to `linkPins` and `makers`. **Resolve by taking `main`'s file and re-running the idempotent assembler — never hand-resolve a JSON conflict.**
