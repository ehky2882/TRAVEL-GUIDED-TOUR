...
|---|---|---|
| **`backend/catalog_snapshot.sql`** | **The catalog RPC stops timing out (it fails ~1 call in 3 today)** | **The app keeps falling through to the gh-pages mirror on a third of launches** — 🔴 **paste it only AFTER [#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742) merges; the reverse order leaves every phone on a stale catalogue with nothing erroring** |
