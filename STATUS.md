...
🟢 **Branch `claude/tour-links-7ova6v` — 17 link pins from `@archimarathon`, session 143. PUSHED, NO PR OPENED** (this session's harness forbids opening one unasked; say the word and it goes up). 16 TikTok short links + 1 Instagram reel, **all one creator on two platforms**. `linkPins` **1,151 → 1,168** on the merged base; `tours`, `makers` and `places` byte-identical, **0 new maker rows** — the uuid5 scheme reproduced BOTH live ids field-for-field, and the avatar regenerated byte-identically and was **excluded rather than overwritten** (18 generated, 17 uploaded). Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with **nothing for the owner to run**. Heroes on gh-pages `0b15aa4` (tree diff **exactly 17 additions, 0 deletions, nothing outside `images/`**; deploy read **`in_progress`, never `cancelled`**, then **all 17 live URLs hash-verified against the uploaded bytes — 17 ok, 0 mismatch, 0 non-200**). **Three owner decisions are flagged and NOT acted on:**
  - 🔴 **Serlachius is now three coincident markers, all one creator — exactly `TourSetMap.maxStacked = 3` with no headroom.** #730's `Serlachius Museum Bridge` and `The Art Sauna at Serlachius` plus this batch's `Crossing the Serlachius Museum Bridge`, all on OSM's own point, **nothing nudged**. Because they share a creator they stack on that creator's own maker page, which is precisely the case a place page exists to fix. ⚠️ **Two more new EXACT groups**: **KAIT** (mine coincident with `@avant.arte`'s, plus #730's third marker 79 m away) and **Sayamaike Museum** (two different reels from consecutive parts of one walkthrough). ⚠️ **The clean-exit state was already broken by #730, not by this batch** — base **11 EXACT / 50 NEAR → 13 / 52**, with **NEAR gaining exactly two and losing none**.
  ...
  - ⚠️ **Six architects tagged by name.** `Junya Ishigami` became available only when [#731](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/731) merged mid-flight, so KAIT gained the name on the rebase. **Still absent and shipping the generic tag:** `BVN Architecture`, `EMTB`, `Hirvonen-Huttunen`, `MGT Architects`, `Kim Swoo-geun`, `Shin Takamatsu` — a `Models/Tag.swift` **code** change, kept out of a content batch.
