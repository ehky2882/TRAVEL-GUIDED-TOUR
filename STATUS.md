...
**Last verified:** 2026-09-02 (session 138 — **forty-two Miami and Mayakoba link pins on `claude/new-tour-links-mphq67`, plus FIVE places.** The owner sent 45 Instagram reels; **42 wired, 3 blocked by Instagram** (`contextJSON: null` proven against three already-live catalogue pins fetched in the same pass with the same UA). **This batch contributes 42 pins, 33 maker rows and 5 places; `tours` is byte-identical at 1,552 throughout.** ⚠️ **`main` moved THREE TIMES mid-session — #711 then #712 forty minutes later, 57 Hong Kong pins between them — so re-derive rather than quoting a delta**: on the merged base the catalogue reads **1,552 tours + 907 pins + 297 makers + 105 places**. Owner decided three places (*"Greenwood - join. Fairmont mayakoba and alila, make place"*): the Sophie Calle obelisk moved 175 m onto the **Green-Wood** place (the pin moves, never the geofenced tour), and **Fairmont Mayakoba** and **Alila Mayakoba** became places with nothing moved. Then three more (*"Make the 3 suggested places… Don't worry about weak heroes"*): **Vizcaya, Pérez Art Museum Miami and 1111 Lincoln Road**, pure additions, nothing moved, each swept 400 m first — **the Frost Museum of Science is 172 m from PAMM and is correctly NOT a member**. ✅ **The parked post was closed by an owner address** (*"open house miami - 4080 Douglas Road, Coconut Grove, Miami FL"*) and wired; ⚠️ that address returns **two** candidates and only one is in Coconut Grove. ⚠️ **All five new place heroes are borrowed and that is structural** — no Atlas tour at any of the sites, every member's gallery empty. ✅ **Weak heroes are CLOSED by owner instruction — do not re-raise.** 🎉 **`check-place-candidates.py` finishes BYTE-IDENTICAL to `origin/main` at 2 EXACT / 38 NEAR** — every coincident group this batch created is resolved into a place, and none was manufactured; the 2 left are Hong Kong pairs from other sessions. ⚠️ **`Tours.json` conflicted on both merges**, resolved by taking `main`'s file and re-running the idempotent assembler with every check re-run; the handoff renumbered `-5` → `-6` on an add/add collision. **NO PR OPENED.** Earlier: 2026-09-02 (session 137 follow-up — **three Hong Kong places built on owner instruction (*"make all 3 listed places"*): PMQ, Jamia Mosque and Sai Wan Swimming Shed, **places 97 → 100** on `claude/tour-links-9ynig6`, [#705](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/705). `tours` and `makers` **byte-identical**; exactly 3 link pins moved 45 / 28 / 30 m onto their tour, all `manual`, and **nothing here is geofenced**, so each anchor was decided on its own evidence. **`check-place-candidates.py` reports EXACT *none* and exits 0 on BOTH sides, NEAR 36 → 33** — so [#707](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/707)'s clean-exit state is **preserved, not broken**. **Every hero is a third photograph: borrowed-hero count 26 of 100 against `main`'s 26 of 97, so these three add zero.** ⚠️ **`main` moved THREE TIMES mid-session** — #701 (28 Chicago pins), #702 + #703 (seven places, 88 → 95), then #707 (95 → 97) — and took this session's handoff filename twice; the catalogue edit was redone on `main`'s file each time, every check re-run, **the diff unchanged at 57/12 on all four bases**, and the handoff renumbered `-10` → `-11` → `-13`. ⚠️ **`archive/README.md`'s bottom list carries `HANDOFF-260901-10.md` twice** (another session's duplicate, left alone rather than churned while several sessions are appending to that file). Content only — the seed carries `places`, so this reaches Supabase on merge with **no owner SQL**. Earlier: 2026-09-01 (session 135b — Casa Loma, the Distillery District and Osgoode Hall made places and the Crystal added to the ROM place, **88 → 91**, **[#702](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/702) MERGED as `855938a8`, verified live on BOTH sources — the RPC at 91 places with the ROM place at 3 members, and the gh-pages mirror converging on the same ~10 min later**. ⚠️ **That 91 is a reading taken AT MERGE TIME and #703 has since taken the catalogue to 95 — re-derive rather than quoting it.** ⚠️ **`main` moved FOUR times under that branch** — #698, #699, #700 and #701, the last two *after* all four checks had already gone green — and the catalogue edit was re-assembled on main's file each time with every check re-run. **A green PR is not a mergeable one; re-read `mergeable_state`.** Earlier: session 137 — sixteen Hong Kong link pins from Instagram `@love.in.hongkong` on `claude/tour-links-9ynig6`, **linkPins 696 → 712, makers 246 → 247**; heroes pushed to gh-pages `47a686d`. **[#700](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/700) opened on owner instruction.** `check-place-candidates.py` **EXACT byte-identical to the baseline (6 groups, all #699's) and NEAR 33 → 37**, so no coincident group was manufactured — ⚠️ it exits 1 on #699's groups, not on anything here; three NEAR pairs are place candidates for the owner (Jamia Mosque 28 m, Sai Wan Swimming Shed 30 m, PMQ 45 m). **`gh pr list --state open` re-derived.** Earlier: session 134c — Riverside Church made a place (**86 → 87**) and its geofenced Atlas tour moved off 3019 Broadway onto the church, radius re-derived 60 → 100 m. Earlier: session 134b — [#694](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/694) **MERGED** as `41a667d4` and **verified live on the Supabase RPC**, which serves 86 places with the Windsor hero repointed: the last three place cards, **places 83 → 86**, and a **sourced CC0 hero for Windsor Castle**, which closes the oldest item in § 1. 🎉 **`check-place-candidates.py` reaches ZERO exact groups and exits 0** — the place backlog is empty. ⚠️ **`main` moved again mid-session** (#693, The Charging Bull) and the branch was merged with `main`, its catalogue edit re-assembled on `main`'s file, and every check re-run. **`gh pr list --state open` re-derived.** Earlier: 2026-09-01 (session 134 — [#691](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/691) opened: two place cards, **places 80 → 82**, plus the seven `@nikola.matus` `images//` stop URLs fixed. **`gh pr list --state open` re-derived: exactly TWO open PRs, #691 and #692** — every other 🟡 in § 1 was stale and the `claude/shared-url-checks` one is flipped. ⚠️ **`main` moved mid-session** (#690, eleven Joshua Charow pins) and this branch was rebased onto it with every check re-run. ⚠️ **Two sessions both numbered themselves 133**; theirs merged first, so this one is 134. Earlier: 2026-08-31 (session 127d — the Natural History Museum audio/gallery collision fixed on `claude/nhm-gallery-fix`, see § 1. Earlier: session 127c — nine tier 2 place cards on `claude/tier2-place-cards`, see § 1. Earlier: session 127b — ten place cards, merged as [#676](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/676). Earlier: session 127 — 95 link pins, merged as [#674](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/674). Earlier: session 126 — [#673](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/673) opened, seven place cards, see § 1. Earlier: session 125 — seven `@nikola.matus` pins pushed, and a **pre-existing Chelsea Hotel coordinate defect** found, see § 1. Earlier: session 124 — [#662](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/662) merged as `845f0d86` after owner device-verification on **1.1.1 (137)**. 🔴 **The marketing version is now 1.1.1** — 1.1 is released and Apple refuses further builds on it, see § Builds. Session 122c: [#657](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/657) and the architect PR [#654](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/654) are both merged and re-verified on `main`, and **one dead hero image was found — see § 1** Earlier: session 128 — **`gh pr list --state open` re-derived: ZERO open PRs.** [#675](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/675) merged as `2bc05e30` (105 `@hereinnyc` link pins, linkPins 378 → 483), and the squash was verified to have actually changed files on `main` rather than trusting GitHub's success line. Five entries in § 1 were stale `OPEN` — #673, #674, #675, #663 and #658 are all merged — and have been flipped, keeping only the owner decisions that are still live. 🔴 **The marketing version is 1.1.1** — 1.1 is released and Apple refuses further builds on it, see § Builds) Earlier: 2026-09-02 (session 138 — **nineteen Hong Kong link pins from `@notbadgalriri__` opened as [#715](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/715)**, linkPins 846 → 865, makers 263 → 264, `tours` and `places` byte-identical; see § 1 for the Plus Code that belongs to another post and the three weak heroes. **`gh pr list --state open` re-derived: two open PRs, #714 and #715**, with zero content overlap between them. Earlier: session 137 follow-up — **three Hong Kong places built on owner instruction (*"make all 3 listed places"*): PMQ, Jamia Mosque and Sai Wan Swimming Shed, **places 97 → 100** on `claude/tour-links-9ynig6`, [#705](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/705). `tours` and `makers` **byte-identical**; exactly 3 link pins moved 45 / 28 / 30 m onto their tour, all `manual`, and **nothing here is geofenced**, so each anchor was decided on its own evidence. **`check-place-candidates.py` reports EXACT *none* and exits 0 on BOTH sides, NEAR 36 → 33** — so [#707](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/707)'s clean-exit state is **preserved, not broken**. **Every hero is a third photograph: borrowed-hero count 26 of 100 against `main`'s 26 of 97, so these three add zero.** ⚠️ **`main` moved THREE TIMES mid-session** — #701 (28 Chicago pins), #702 + #703 (seven places, 88 → 95), then #707 (95 → 97) — and took this session's handoff filename twice; the catalogue edit was redone on `main`'s file each time, every check re-run, **the diff unchanged at 57/12 on all four bases**, and the handoff renumbered `-10` → `-11` → `-13`. ⚠️ **`archive/README.md`'s bottom list carries `HANDOFF-260901-10.md` twice** (another session's duplicate, left alone rather than churned while several sessions are appending to that file). Content only — the seed carries `places`, so this reaches Supabase on merge with **no owner SQL**. Earlier: 2026-09-01 (session 135b — Casa Loma, the Distillery District and Osgoode Hall made places and the Crystal added to the ROM place, **88 → 91**, **[#702](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/702) MERGED as `855938a8`, verified live on BOTH sources — the RPC at 91 places with the ROM place at 3 members, and the gh-pages mirror converging on the same ~10 min later**. ⚠️ **That 91 is a reading taken AT MERGE TIME and #703 has since taken the catalogue to 95 — re-derive rather than quoting it.** ⚠️ **`main` moved FOUR times under that branch** — #698, #699, #700 and #701, the last two *after* all four checks had already gone green — and the catalogue edit was re-assembled on main's file each time with every check re-run. **A green PR is not a mergeable one; re-read `mergeable_state`.** Earlier: session 137 — sixteen Hong Kong link pins from Instagram `@love.in.hongkong` on `claude/tour-links-9ynig6`, **linkPins 696 → 712, makers 246 → 247**; heroes pushed to gh-pages `47a686d`. **[#700](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/700) opened on owner instruction.** `check-place-candidates.py` **EXACT byte-identical to the baseline (6 groups, all #699's) and NEAR 33 → 37**, so no coincident group was manufactured — ⚠️ it exits 1 on #699's groups, not on anything here; three NEAR pairs are place candidates for the owner (Jamia Mosque 28 m, Sai Wan Swimming Shed 30 m, PMQ 45 m). **`gh pr list --state open` re-derived.** Earlier: session 134c — Riverside Church made a place (**86 → 87**) and its geofenced Atlas tour moved off 3019 Broadway onto the church, radius re-derived 60 → 100 m. Earlier: session 134b — [#694](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/694) **MERGED** as `41a667d4` and **verified live on the Supabase RPC**, which serves 86 places with the Windsor hero repointed: the last three place cards, **places 83 → 86**, and a **sourced CC0 hero for Windsor Castle**, which closes the oldest item in § 1. 🎉 **`check-place-candidates.py` reaches ZERO exact groups and exits 0** — the place backlog is empty. ⚠️ **`main` moved again mid-session** (#693, The Charging Bull) and the branch was merged with `main`, its catalogue edit re-assembled on `main`'s file, and every check re-run. **`gh pr list --state open` re-derived.** Earlier: 2026-09-01 (session 134 — [#691](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/691) opened: two place cards, **places 80 → 82**, plus the seven `@nikola.matus` `images//` stop URLs fixed. **`gh pr list --state open` re-derived: exactly TWO open PRs, #691 and #692** — every other 🟡 in § 1 was stale and the `claude/shared-url-checks` one is flipped. ⚠️ **`main` moved mid-session** (#690, eleven Joshua Charow pins) and this branch was rebased onto it with every check re-run. ⚠️ **Two sessions both numbered themselves 133**; theirs merged first, so this one is 134. Earlier: 2026-08-31 (session 127d — the Natural History Museum audio/gallery collision fixed on `claude/nhm-gallery-fix`, see § 1. Earlier: session 127c — nine tier 2 place cards on `claude/tier2-place-cards`, see § 1. Earlier: session 127b — ten place cards, merged as [#676](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/676). Earlier: session 127 — 95 link pins, merged as [#674](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/674). Earlier: session 126 — [#673](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/673) opened, seven place cards, see § 1. Earlier: session 125 — seven `@nikola.matus` pins pushed, and a **pre-existing Chelsea Hotel coordinate defect** found, see § 1. Earlier: session 124 — [#662](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/662) merged as `845f0d86` after owner device-verification on **1.1.1 (137)**. 🔴 **The marketing version is now 1.1.1** — 1.1 is released and Apple refuses further builds on it, see § Builds. Session 122c: [#657](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/657) and the architect PR [#654](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/654) are both merged and re-verified on `main`, and **one dead hero image was found — see § 1** Earlier: session 128 — **`gh pr list --state open` re-derived: ZERO open PRs.** [#675](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/675) merged as `2bc05e30` (105 `@hereinnyc` link pins, linkPins 378 → 483), and the squash was verified to have actually changed files on `main` rather than trusting GitHub's success line. Five entries in § 1 were stale `OPEN` — #673, #674, #675, #663 and #658 are all merged — and have been flipped, keeping only the owner decisions that are still live. 🔴 **The marketing version is 1.1.1** — 1.1 is released and Apple refuses further builds on it, see § Builds)
...
🟡 **NOT A PR — branch `claude/more-tours-a1xhw4` pushed, 14 Hong Kong + Macau link pins (session 139).** `linkPins` **905 → 919** · `makers` **297 → 299** · `tours` and `places` byte-identical. Content only — no Swift, no SQL, no build; the seed carries `linkPins`, so it reaches Supabase on merge with **no owner SQL**. Heroes on gh-pages `0786a135`. **Four things for the owner:**
  - 🔴 **Two of the sixteen links CANNOT be pinned, ever.** They are Instagram **story highlights** (`/s/…`) by a third creator, `@kieranbrowntravel`. `LinkSource.embedURL` matches only `p`/`reel`/`tv`, so the app can build **no player**; the page also carries **no thumbnail and no caption**. ⚠️ **They are not dead or blocked** — the page is a healthy 476 kB — so **do not re-try them**; only a reel or permanent post from that creator would work. **Their two Plus Codes (`75J3+FJ Central`, `76M6+MX Quarry Bay`) are orphaned — which subjects were they for?**
  - 🟡 **A place candidate: the Hong Kong Railway Museum.** `@love.in.hongkong` already has a pin of that exact title and this batch's lands **0.46 m** away, because **both derive from OSM's own museum node** — convergence, not manufacturing, and **neither was nudged**. It ships as `The Trains at the Hong Kong Railway Museum` so two identical titles don't sit on one point. A place is the real fix and it is the owner's call.
  - 🟡 **One weak hero, flagged not fixed** — `The Four Columns at Kadoorie Farm` shows a **historical B&W photograph of the demolished building the columns came from**, not the columns at the farm the pin sits on. A link pin re-hosts only the thumbnail, so no other frame exists.
  - ⚠️ **10 of 14 will not play inline** — the documented licensed-music gate; poster + OPEN IN INSTAGRAM is the correct outcome.

🔴 **OWED — ONE SQL PASTE. `backend/pull_pins_260902.sql` has NOT been run, and that is MEASURED rather than assumed.** After [#716](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/716) merged (squash `613e23e7`) the live RPC serves **907 pins against the catalogue's 905**, and `@shivanidukhandee` reads **106 against 104** — the two pins the owner asked to remove (`Xiang Bo Bo`, `Cheung Hing Coffee Shop — Pineapple Bun Hunt`) are **still on every phone**. `seed_from_toursjson.py` is upsert-only by design, so a deletion reaches the gh-pages mirror and the bundled offline seed and **never reaches Postgres**, which is what the app reads first. One paste into the Supabase SQL Editor closes it; the file is idempotent and verifies inside its own transaction. **The same gap once ran eight days undetected.**
...
🟡 **OPEN — [#715](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/715) nineteen Hong Kong link pins from Instagram `@notbadgalriri__` (session 138)**, branch `claude/tour-links-exviiy`. Twenty links, one reel pasted twice, so **19 distinct posts ship**. **linkPins 846 → 865 · makers 263 → 264 · `tours` and `places` byte-identical.** Content only — no Swift, no SQL, no build; the seed carries `linkPins`, so it reaches Supabase on merge with **no owner SQL**. Heroes on gh-pages `e3503251`, **all 19 live URLs hash-verified against the uploaded bytes (19 ok, 0 mismatch)**; `check-image-duplicates.py --pins` **`OK`** over 860 images, shared-URL half **0 errors / 208 documented reuses**. Validator mirror **self-tested 40/40**, then **0 errors, 2 pre-existing warnings**; `check-place-candidates.py` output **byte-identical to `main`'s** (2 EXACT / 38 NEAR — nothing manufactured).
  - 🔴 **OWNER DECISION — a supplied Plus Code contradicted its own post.** `862H+Q4 Cha Kwo Ling` was pasted beside the second copy of `DZuR4QoP_8R`; it decodes onto the **Central Kowloon Bypass, a motorway under construction**, while that reel's caption gives an explicit address **15 km away** in Tsuen Wan. Confirmed three ways (OSM names 荃德花園 Tsuen Tak Gardens at **house number 208** exactly; the creator's own burned-in subtitle reads *"Nocturnal Stationery in Tsuen Wan is a humble shop"*; the code lands on a road, not a venue), so **the caption won**. The stray code most likely belongs to a reel that was not in the batch — **which one?**
  - 🟡 **OPEN — three weak heroes, flagged not fixed.** **Rednaxela Terrace's hero is a WET MARKET** (the subject is beyond doubt — the code reverse-geocodes onto the terrace by name — but the picture is not of it; the sharpest mismatch in the batch); **Victoria Skypark** is a mall walkway with none of the sunset-and-skyline its caption is about; **Camelpaint Building** is street b-roll. A link pin re-hosts only the thumbnail, so **no other frame exists**.
  - 🟡 **OPEN — the toy store ships with NO asserted name.** Its caption withholds it (*"COMMENT 'TOY' TO GET THE LOCATION"*) and the shopfront sign is only partly legible (`…0AMPM TOYS`, its first character behind the creator's own `[13/100]` overlay), so it is titled **`A Vintage Toy Store in Tsim Sha Tsui`** — the Banksy convention. **One line renames it.**
  - 🟡 **OPEN — Man Luen Choon's district.** The pin sits on OSM's node named for the shop (140-142 Des Voeux Road Central, which OSM files under **Central**) while the caption says **Sheung Wan**; OSM *also* names Wing Cheong Commercial Building, 19-25 Jervois Street (Sheung Wan) 230 m away. **The OSM node named for the business is the only in-session evidence tying the name to a point**, so it won.
...
**⚠️ Re-derived from `Tours.json` on `main` at `613e23e7`: 1,552 tours + 905 link pins, 297 maker rows, 2,829 stops, 105 places.** Newest reading on this board — every count below it is an older snapshot, so **re-derive rather than quoting any of them**.
  - ⚠️ **The live RPC serves 907 pins, not 905, and that gap is the point** — the two pins removed in [#716](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/716) are still in Postgres until `backend/pull_pins_260902.sql` is pasted (§ 1). It also reports **1,553 tours and 311 makers** against 1,552 / 297 — the documented `Zxxx` test tour and upsert-only maker accumulation. **Assert on link-pin counts, never on maker totals.**
