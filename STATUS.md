...
| ~~**Unique usernames — 9 choices**~~ | ✅ **DECIDED 2026-09-12: all nine recommendations**, on the owner's condition that they match industry standards — each checked in `docs/usernames-design.md` § Industry check (seven match named precedent; two are brand/process choices with no standard). **Not built yet**: next is `backend/usernames.sql` (one owner paste), then a content PR, then an app PR for device review. **No account deleted, merged or renamed.** | ✅ Decided · build not started |
| **Can a user delete their OWN account?** | 🔴 **Found 2026-09-12 while checking usernames against industry standards.** Apple requires in-app account deletion for any app with signup (Guideline 5.1.1(v)). A search of app, SQL, Edge Functions and website found **none** — re-check before acting, it may exist under other wording. It meets the owner's *"never delete any users"* head-on; the standard reading is **we** never delete anyone, **a person** may delete **their own**. Owner's call. | 🔴 Decision owed |
