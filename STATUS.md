...
|---|---|---|---|
| **141** | `bottom-module-missing-45z6ep` | The same fix at **1.1.2** (`e6570bdc`) — 140's payload plus the version bump | ✅ **UPLOADED — install this. 1.1.2 (141), notes attached.** Verified on the *Build and upload to TestFlight* step, not the run's colour or the unconditional *Done* echo. ⚠️ **The honest test is REPEATED cold launches** (force-quit, relaunch, 8–10 times), some while the phone is busy, plus a background→foreground round-trip — the bug is intermittent, so one clean launch proves nothing. **New: if the bars ever do go, backgrounding and returning should bring them back** |
