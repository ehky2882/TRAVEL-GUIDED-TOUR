...
| Branch | State |
|---|---|
| `claude/amazing-rubin-pkub81` | **Open as [#801](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/801)** — the place sweep. `check-place-candidates.py` gains a **TIGHT** tier (within 25 m, whatever the titles say), because the old title-containment rule is structurally blind to one site that two entries call by two unrelated names — *Hook & Ladder 8* / *The Ghostbusters Firehouse* are 4 m apart and share no word. **41 exact · 151 tight · 79 near**; 82 of the 151 could not have reached the old NEAR tier and 55 share no word at all. Behaviour-preserving otherwise: the old 148 NEAR pairs come back as 69 tight + 79 near exactly. Pair loop replaced by a spatial grid (5.3M haversines → 0.4 s), deliberately **not** bucketed by `city` (a place spans the New York / Brooklyn label). `docs/place-candidates-260911.md` is the sweep, numbered for picking: **A** 10 existing places missing a member standing on them (additive, no judgement), **B** 132 sites with 2+ entries and no place page, **C** 79 same-subject pairs 25–500 m apart. 🔴 **The two owner decisions already owed in § 5 are items in it** — the First National Bank of Hollywood pair is **B15**, the St Vincent de Paul pair **B83**. **§ A is APPLIED** on owner instruction (*"do section a except - il presidente, westminster abbey, channel gardens"*): seven places, eight entries — A4/A8/A9 each had only the excluded entry, so all three drop out and stay open. 🔴 **"Adding the id is the whole change" was wrong**: a place's identity is exact coordinate equality (`validate-tours`, 1e-9°), so the eight ids alone produced 8 `member not on the place coordinate` errors — each entry also had to be **snapped onto its place** (0.6–20.9 m, every one inside its own 30 m trigger radius, so no tour changes when it fires). Validator 0/0, selftest 32/32, control clean; sweep 41/151/79 → **40/132/79**. **§ B's 0 m rows then APPLIED too** (*"do the 0m candidates in section b"*): of the 40 EXACT-tier groups, **38 became places (142 → 180), 2 are HELD.** Eight were folded wider than 0 m on purpose — a place called *The Pantheon* that excluded the Pantheon tour 5.8 m away would have been the wrong answer — each snap asserted inside its own 30 m geofence. ✅ **Both held items are now RESOLVED by the owner.** *Grand Central*: the two pins joined the existing place (88.4 m each, 4 members) — this really does move where they fire, accepted. *Tai Kwun*: a new place at **the centre of OSM's 檢閱廣場 Parade Ground way**, holding *The Public Spaces of Tai Kwun* + the *Tai Kwun | 大館* tour (43.7 / 45.5 m — outside the geofence, deliberately: the tour will now fire in the courtyard instead of at the Hollywood Road gate). ⚠️ **Only ONE Atlas Tai Kwun tour exists**, not several. *Madame Fu* is relocated to its **own published coordinate** (0.3 m from OSM's Barrack Block centre — two sources agreeing) and **deliberately stays outside the place**; at 27.4 m it falls just beyond the TIGHT radius so the sweep will not re-suggest it — a Tibidabo-style standing decision, do not "fix" it. The geofence assertion was **waived per entry with a recorded reason, not removed**. Validator 0/0 at **181 places**; seed emits 181; **sweep 40 exact → 0 and the checker now exits 0**. Tours.json is now touched → still auto-merge class (content). |
