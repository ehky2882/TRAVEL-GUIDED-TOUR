...
🟡 **OPEN — four places, on `claude/places-miami-amfa` (session 145, 2026-09-05).** Owner: *"11 hoyt is Brooklyn. Make places for all suggested"*. **Places 121 → 125** — **Faena Hotel Miami Beach** (4 members), **The Surf Club** (3), **Patch of Heaven Sanctuary** (2), **Arkansas Museum of Fine Arts** (4) — closing every stack-cap finding the two batches below raised. A **pure addition**: `tours`, `makers` and `linkPins` byte-identical, the 121 existing places unchanged as a prefix, and **nothing moved** (all 13 members were already exactly coincident and every one is `manual`, asserted first). Diff **65 insertions / 0 deletions**. Content only — no Swift, no SQL, no gh-pages push, no build.
  ...
  - **Verification:** mirror **27/27 with a clean control**, then **0 errors, 0 warnings across 1,552 tours + 1,237 pins + 125 places**; ⚠️ **53 place-layer faults injected against THESE FOUR PLACES specifically — 53 caught, control clean before and after**; 🎉 **`check-place-candidates.py` EXACT 19 → 15, NEAR unchanged at 53**, the diff proving it fell by exactly the four groups resolved and gained nothing; seed clean at **345 / 2,789 / 3,161 / 125** with **0 `images//`**; all four hero URLs live **200** and hash-verified against the bytes actually looked at.
