...
✅ **MERGED as [#738](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/738) — 23 Miami link pins (session 145, 2026-09-05), shipped together with the Studio Gang batch below as one 36-pin PR.** Committed as `7ee3c6e2`, stacked on the Studio Gang batch below; **no PR opened, because the session's harness forbids opening one unasked.** `linkPins` **1,214 → 1,237**, `makers` **331 → 345**; `tours` and `places` byte-identical. Content only — no Swift, no SQL, no build. ⚠️ **One `scripts/` file changed too** (`validate-tours-mirror.py`, developer tooling, auto-merge class — it does not ship in the app). Heroes live on gh-pages (`88add69`), **all 23 hash-verified against the uploaded bytes after the deploy**; `check-image-duplicates.py --pins` **OK** over 1,232 images, shared-URL half **0 errors / 208 documented reuses**.
  - 🔴 **TWO MORE STACK-CAP FINDINGS, BOTH OWNER DECISIONS.** **FOUR pins sit on the Faena's single coordinate** — the penthouse twice by two creators, Tierra Santa and Los Fuegos — **past `TourSetMap.maxStacked = 3`**; ⚠️ **all four are different creators**, so unlike AMFA the cap bites on the Home map and shared lists rather than a creator page. **THREE sit on The Surf Club's**, exactly at the cap with no headroom. 🔴 **OSM maps none of the inner venues separately** (three bounded queries returned nothing inside those footprints), so siting them apart would be the manufacturing session 132 rejected for Arthur Ashe. **A place is the fix for both; neither was created.**
  - ⚠️ **A THIRD, FRIENDLIER PLACE CANDIDATE:** `The Garden at Patch of Heaven` lands **0.0 m** from the existing `@miamibucketlist` pin — two creators, one site (the Cube House / Vizcaya shape). **It was retitled for that reason** (the Railway Museum precedent), or two identically-titled pins on one point would read as a duplicate.
  ...
  - ⚠️ **`check-place-candidates.py` 16 EXACT → 19, NEAR unchanged at 53**, the diff proving the three added groups are exactly Patch of Heaven ×2, Faena ×4 and Surf Club ×3 — nothing manufactured, nothing dodged.

✅ **MERGED as [#738](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/738) — 13 Studio Gang link pins (session 144, 2026-09-05), shipped together with the Miami batch above.** Committed as `88203b53` and pushed; **no PR opened, because the session's harness forbids opening one unasked.** `linkPins` **1,201 → 1,214**, `makers` **328 → 331**; `tours` and `places` byte-identical. Content only — no Swift, no SQL, no build. Heroes are live on gh-pages (`d22dca3`), all 13 hash-verified against the uploaded bytes.
  - 🔴 **TWO STACK-CAP FINDINGS, BOTH OWNER DECISIONS.** **The Arkansas Museum of Fine Arts is FOUR coincident markers**, past `TourSetMap.maxStacked = 3`, so the fourth is permanently unreachable on every maker, place and list map — and **three of the four are `@studiogang`**, so that creator's own page sits exactly at the cap. **The Gilder Center is effectively three deep** (two already-live pins plus this one) and `check-place-candidates.py` reports it as **two**, because the existing pair differs in the 6th decimal (the CalAcademy artifact defeating the EXACT tier). **A place is the fix for both; neither was created.**
  ...
  - ⚠️ **11 Hoyt ships `city: "Brooklyn"` against the dominant `New York` convention** (94 Brooklyn-coordinate pins use `New York`, 10 use `Brooklyn`) — the caption and both neighbouring pins say Brooklyn. **One line to change.**
  - ⚠️ **A `Tours.json` CONFLICT IS EXPECTED.** A parallel session pushed 41 `@itshistoryonair` heroes to gh-pages (`94264b6c`) without merging its catalogue change; both sides append to `linkPins` and `makers`. **Resolve by taking `main`'s file and re-running the idempotent assembler — never hand-resolve a JSON conflict.** ✅ **IT HAPPENED AND IS RESOLVED — see [#737](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/737):** main's `Tours.json` was taken wholesale and the idempotent assembler re-run on it, never hand-resolved; **overlap between the two batches was 0 on sourceURLs, tour ids, stop ids and hero filenames.**
