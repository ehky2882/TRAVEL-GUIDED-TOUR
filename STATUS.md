...
**Last verified:** 2026-09-03 (session 142 — **PR [#729](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/729) MERGED** (squash `32531c71`: 61 pins, 2 places, 48 architects — verified live on the Supabase RPC at 1,047 pins / 114 places / 383 architects, and on the gh-pages mirror). **A SECOND `@archimarathon` batch is then pushed on `claude/new-tour-links-cytwc6`, restarted clean off the merged `main`, and opened as [#730](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/730).** 106 links → **105 distinct posts → 104 shipped**; `linkPins` **1,047 → 1,151**; `tours`, `makers` and `places` byte-identical, **0 new maker rows**. **The largest batch to date. Philippines is the catalogue's 45th country**; 22 new cities. 🔴 **A caption was pasted onto the wrong video** — two posts carry byte-identical captions while their heroes sit 63.35 apart perceptually, and the burned-in frames read Part 1 / Part 2 / Part 3. 🔴 **A supplied coordinate was 364 km from the venue its own caption names** (Café Mori no Terrace, pasted with a Kyoto point, is at Meiji Jingu in Tokyo) — the caption won, corroborated three ways. 🔴 **`check-place-candidates.py` goes 0 → 12 EXACT and exits 1**, ending the clean-exit state; **all twelve are genuine and nothing was nudged**. ⚠️ **The Barcelona Pavilion is now five markers deep against a cap of three** — the one that actually bites. **All 104 heroes opened and read against their captions — zero wrong subjects.** Nothing compiled locally — CI on #730 is the only compile check.)
...
⚠️ **`list_pull_requests(state=open)` re-derived 2026-09-03 (after the build-140 dispatch) — there is exactly ONE open PR, [#728](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/728).** #726, #729 and #730 have all merged. Every other "OPEN" line on this board is stale on the PR itself and survives only because it still carries a live owner decision. **Re-derive before trusting any line here.**

🟢 **[#728](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/728) — branch `claude/bottom-module-missing-45z6ep`, the bottom module going missing (session 141). CODE — needs a device look, and the build is cut: TestFlight 1.1.1 (140).** A tester sent a screenshot with **no mini-player and no tab bar**, on a settled Home tab with the drawer correctly reserving their 126pt. 🔴 **Three previous fixes all asked whether the window INSTALLED; this is a different failure — it installs fine and is then left HIDDEN.** The launch hides it deliberately (it sits above every window and would paint over the splash), but the hide was **latched** off `launchState.isSplashVisible` against a single unhide in `runLaunchGate` — and `phase` does not move until `beginHandOff()`, ≥32 ms and one guaranteed suspension later, so any `installBottomModule()` landing in that gap re-hid the window **with nothing left to undo it**. `scenePhase == .active` calls exactly that, unguarded, on a cold launch. 🔴 **The #443 inline fallback could not catch it because it asked `!isInstalled` and the window WAS installed** — `isShowingBars` (installed AND not hidden) is the right question. **The fix is structural, not another timing bet:** the App owns `launchHoldsBottomModule`, dropped in the same main-actor turn as the unhide; `installBottomModule()` **re-derives** visibility via `BottomModuleWindowController.shouldWithdraw` rather than commanding a hide, so a duplicated or mistimed call can only reach the right answer — **and because that site runs on every foreground, a background round-trip is now a guaranteed recovery**; `setHidden` writes the window on every call so a drifted window self-corrects. **7 new tests (15 total)**, the regression one pinning that re-deriving after the unhide says *show* at the exact instant the old code said *hide*. ⚠️ **Deliberately NOT fixed, stated rather than buried: a stuck `appShared.hidesBottomModule`** — this re-derives *from* that flag and cannot heal it; `LinkEmbedView` guards it with three restore paths, and adding a rule here could fight the tour wizard, which legitimately withdraws the module. ⚠️ **Nothing compiled locally** (Linux web session) — CI is the `test_sim` stand-in. **The device check is the real proof and the bug is intermittent, so the honest test is repeated cold launches (force-quit, relaunch, 8–10 times) plus a background→foreground round-trip** — if the bars ever do go, backgrounding and returning should now bring them back.
...
|---|---|---|---|
| **140** | `bottom-module-missing-45z6ep` | [#728](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/728) the bottom module was HIDDEN, not uninstalled — visibility re-derived rather than commanded, and the inline fallback asks `isShowingBars` instead of `isInstalled`. `main` merged in (`b52315f8`), so it also carries #729's 48 architects and #730's link pins. **1.1.1 (140)** | ⏳ dispatched 2026-09-03 — awaiting owner device check. ⚠️ **The honest test is REPEATED cold launches** (force-quit, relaunch, 8–10 times) plus a background→foreground round-trip, because the bug is intermittent. ⚠️ **Unverified from this session: whether 1.1.1 is still an open version train** — a web session has no App Store Connect key, so if Apple has released 1.1.1 the upload fails at the last step with error 90062 despite a clean compile and sign |
