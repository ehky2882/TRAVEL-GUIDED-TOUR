...
🟡 **[#722](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/722) — branch `claude/more-tours-a1xhw4`, 19 Hong Kong + Macau link pins (session 139), sent in two waves.** `linkPins` **905 → 924** · `makers` **297 → 303** · `tours` and `places` byte-identical. Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with nothing for the owner to run.
  ...
  - 🟡 **THREE PLACE CANDIDATES, none created, none nudged together.** **The Hong Kong Railway Museum** — an existing `@love.in.hongkong` pin of that exact title sits **0.46 m** away because both derive from OSM's own node (convergence, not manufacturing); ships as `The Trains at the Hong Kong Railway Museum`. **In's Point** — an existing `@shivanidukhandee` pin **37 m** away *whose caption is also about LEGO*; ships as `The Rare LEGO Sets at In's Point`. **Villain hitting at Canal Road** — an existing `@love.in.hongkong` pin **26 m** away; ships as `The Villain-Hitting Incantation at Canal Road`. 🔴 **`check-place-candidates.py` reports the first two and structurally CANNOT see the third** — the hyphen breaks its title-containment rule, so it is flagged by hand.
