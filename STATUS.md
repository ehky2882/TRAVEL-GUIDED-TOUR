...
🟡 **OPEN, no PR — Casa Loma, the Distillery District and Osgoode Hall become places, and the Crystal
joins the ROM (branch `claude/tours-links-upload-h6t2cs`, session 135).** Owner: *"ROM IS ALREADY PLACE -
...
no build; the seed carries `places`, so this reaches Supabase on merge with **no owner SQL**.
  ...
  - **Verification.** Validator mirror (vocabulary from **both** Swift files, **385 tags**) **0 errors,
    0 warnings** across 1,552 tours + 696 pins + 91 places. ⚠️ **Eleven place-layer faults were injected
    ...
    `check-place-candidates.py` **NEAR 33 → 29**, falling by exactly the four pairs resolved, with
    **EXACT unchanged at 6**, so no coincident group was manufactured (all six are other sessions' or
    ...
    `seed_from_toursjson.py` clean at **246 / 2,248 / 2,620 / 91**. `Tours.json` **byte-stable under a
  ...
  - ⚠️ **`main` moved mid-session** ([#699](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/699), 106
    `@hereinnyc` pins plus the 120 Broadway place), so the catalogue edit was **redone the documented
    way — take `main`'s file and re-run the idempotent assembler, never hand-resolve a JSON conflict** —
    and every check above was re-run afterwards. ⚠️ **Nothing compiled — CI is the authoritative
    validator, and no PR is open.**
