...
✅ **MERGED as [#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742), squash `9eaabdd1` — the catalog RPC is failing a third of the time (session 146, 2026-09-06).** All four checks green; backend + tooling only, so the auto-merge class. Squash **verified to carry real content — 671 insertions across 9 files** (the #629 empty-commit lesson). ✅ **THE OWNER SQL PASTE HAS BEEN DONE — verified live 2026-09-07, so nothing is owed here** (see § SQL pastes owed for the measurement). ⚠️ **This supersedes [#747](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/747), which landed on `main` hours earlier saying *"the fix is NOT live … has not been run"* — and #747 was NOT wrong when it was written.** It measured `catalog_snapshot_age()` and `get_catalog_built()` both returning **404 `PGRST202`**; they now return **200** and **500** respectively. **The owner pasted the SQL between the two sessions.** The lesson is not that either measurement was bad — it is that **an owner-owed item can be cleared while a session is mid-flight, so re-measure before repeating "still owed" from the board.** ✅ **Session 147 then confirmed the content path independently**: a normal content merge needs nothing extra, because the regenerated seed calls `refresh_catalog_snapshot()` as its last act. `get_catalog()` — the source the app reads **first** — returns `500 / 57014 statement timeout` on **4 calls in 12**, sampled long after any seed. Successes 2.2–4.9s, failures 3.5–5.0s: the query is **sitting on** the anon role's statement timeout, so variance decides each call. It rebuilds ~11 MB of JSON per request (a correlated subquery per tour, then `split_link_pins.sql` explodes the finished blob and re-aggregates it **twice**), and the payload is **identical for every caller** — verified, not assumed. `backend/catalog_snapshot.sql` renames the builder aside (the `places.sql` rename-and-wrap precedent, so nothing is retyped) and makes `get_catalog()` a single-row lookup. **Backend + tooling only — no Swift, no `Tours.json`, no build.**
...
serves it to a phone. **`get_catalog()` itself returned 200 on 14 of 14 calls**, TTFB **1.38–2.35 s**.
...
the builder times out and the served endpoint does not.**
