...
🟢 **[#729](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/729) — branch `claude/new-tour-links-cytwc6`, 61 link pins from `@archimarathon` plus TWO places (session 141).** `linkPins` **986 → 1,047**; `tours`, `makers` and `places` byte-identical, **0 new maker rows**. Content only — no Swift, no SQL, no build; the seed carries it, so it reaches Supabase on merge with no owner SQL. **Two owner decisions are flagged and NOT acted on:**
