...
|---|---|---|
| ✅ **`backend/catalog_snapshot.sql` — APPLIED 2026-09-07 21:44 UTC; nothing owed, do not ask the owner to run it again** (re-probed before #746 merged: `catalog_snapshot_age()` 200, `get_catalog()` 4/4 × 200 against 3/3 × 500 an hour earlier). Row kept for the reasoning; the text after this cell describes the state BEFORE the paste. | **The catalog RPC stops timing out (it failed ~1 call in 3)** | **The app keeps falling through to the gh-pages mirror on a third of launches.** ✅ **THE ORDERING GATE HAS CLEARED — [#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742) merged 2026-09-07, so this is safe to paste NOW.** ⚠️ **Still unapplied, and that is measured rather than assumed (2026-09-07, after #745):** `catalog_snapshot_age()` and `get_catalog_built()` both return **404 `PGRST202`** on the live database, so the old builder is still what `get_catalog()` runs — and it still fails, **2 of 5 spaced calls returning `500 / 57014 statement timeout`**. ⚠️ **The seed cannot do this for you:** its refresh call is guarded by `to_regprocedure(...)`, so a content merge succeeds, raises a notice, and leaves the slow path in place — which is exactly why content lands live (`linkPins` 1,306) while the timeouts continue. ⚠️ **How to tell it worked: `get_catalog()` should return in well under a second. The LATENCY BAND, not the pass/fail count, is the signal** — a 33%-failure endpoint comes back clean by chance often enough to fool you, so a clean run still measured in seconds means the snapshot is NOT being served. |
