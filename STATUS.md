...
✅ **MERGED as [#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742), squash `9eaabdd1` — the catalog RPC is failing a third of the time (session 146, 2026-09-06).** All four checks green; backend + tooling only, so the auto-merge class. Squash **verified to carry real content — 671 insertions across 9 files** (the #629 empty-commit lesson). 🔴 **THE OWNER SQL PASTE IS NOW UNBLOCKED and is the only thing this change still owes** — see § SQL pastes owed. ✅ **Session 147 then confirmed the content path independently**: a normal content merge needs nothing extra, because the regenerated seed calls `refresh_catalog_snapshot()` as its last act. `get_catalog()` — the source the app reads **first** — returns `500 / 57014 statement timeout` on **4 calls in 12**, sampled long after any seed. Successes 2.2–4.9s, failures 3.5–5.0s: the query is **sitting on** the anon role's statement timeout, so variance decides each call. It rebuilds ~11 MB of JSON per request (a correlated subquery per tour, then `split_link_pins.sql` explodes the finished blob and re-aggregates it **twice**), and the payload is **identical for every caller** — verified, not assumed. `backend/catalog_snapshot.sql` renames the builder aside (the `places.sql` rename-and-wrap precedent, so nothing is retyped) and makes `get_catalog()` a single-row lookup. **Backend + tooling only — no Swift, no `Tours.json`, no build.**
...
|---|---|---|
| **`backend/catalog_snapshot.sql`** | **The catalog RPC stops timing out (it fails ~1 call in 3 today)** | **The app keeps falling through to the gh-pages mirror on a third of launches** — ✅ **[#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742) IS MERGED (`9eaabdd1`), so the ordering constraint is satisfied and this is safe to paste now.** ⚠️ **How to tell it worked: `get_catalog()` should return in well under a second. The LATENCY BAND, not the pass/fail count, is the signal** — a 33%-failure endpoint comes back 8/8 clean about 4% of the time (it already did once, mid-session), so a clean run still measured in seconds means the snapshot is NOT being served. |
