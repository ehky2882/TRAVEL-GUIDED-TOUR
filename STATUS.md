...
Dispatched as TestFlight run **143** (`ef108754`, build notes attached to the build).**
...
|---|---|---|---|
| **143** | `creator-tours-china-visibility-wmnho7` | [#785](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/785) the link-pin embed failure state (`ef108754`) | ⏳ **dispatched 2026-09-10, awaiting owner** — check the negative first: no message over working pins |
...
| Branch | State |
|---|---|
| `claude/inspiring-gauss-qwi8vu` | **Open as [#782](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/782)** — 37 `@urbanistariel` link pins, `linkPins` 1,551 → 1,588, cut clean off `origin/main` `160f0d8`. Content + docs only → auto-merge class. 🔴 **Katz's Deli arrived on a Plus Code that recovers to Downtown Brooklyn, 3.5 km away in the wrong borough** — the same code from a Manhattan reference too, so the code is wrong, not the locality label; repinned at 205 East Houston Street. 🟡 **One link held back:** `.../video/7159393526156299566` (the Callanish Stones) — TikTok's oEmbed returns an empty author and no thumbnail on every retry while all 37 others return both, so the post looks gone; **its Plus Code is fine**, only the post is missing. ⚠️ `nominatim.openstreetmap.org` **429'd off a shared proxy IP** mid-batch; `photon.komoot.io` answered every request and is the fallback. Heroes live on gh-pages `5e15d8c`, hash-confirmed after the deploy. |
