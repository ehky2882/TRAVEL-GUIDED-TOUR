...
🟡 **OPEN — [#744](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/744), the Gilder Center becomes a place (session 147, 2026-09-07).** Owner: *"MAKE GILDER CENTER A PLACE. DONT WORRY ABOUT WEAK HEROES."* **Places 129 → 130** — the last stack-cap finding the Studio Gang batch raised, closed. Content only, so the **auto-merge class**: squash on green. `tours` and `makers` **byte-identical**, the 129 existing places unchanged as a prefix, **exactly 2 link pins moved in exactly their four coordinate fields each** — every one of the diff's 8 deletions is a coordinate line. Diff **24 insertions / 8 deletions**. No Swift, no SQL, no gh-pages push, no build. Detail: `archive/HANDOFF-260907.md`.
  ...
  - **🔴 A BUG THE MIRROR CANNOT SEE, CAUGHT BY READING THE DIFF.** The assembler's first run wrote **`centerLatitude` / `centerLongitude`** — fields that **do not exist on `Tour`** (`Models/Tour.swift:259`) — and left the real `centroidLatitude` / `centroidLongitude` **stale at the old coordinate**. **`validate-tours-mirror.py` passed it at 0 errors**; centroid-vs-stop agreement is a documented mirror blind spot that **`validate-tours.swift` DOES check**, so it would very likely have failed CI. The assembler now asserts **no member gains or loses a top-level key** and **every centroid mirrors its stop**, and because the fault suite confirms the mirror is still blind to it, the property is **asserted directly** — 0 drift on the members and **0 catalogue-wide across 2,830 single-stop entries**.
  ...
  - **⚠️ SWEPT 400 m; both things in range are correctly excluded.** The **AMNH cluster at 176.1 m** — ✅ **the owner ruled in session 140 that the Gilder Center stays separate** (*"Gilder center keep separate"*); **closed**, and a future audit will flag it again. ⚠️ **FLAGGED, NOT ACTED ON:** the `@poche_space` pin `Poché - Studio Gang | episode 2 of 3` is substantively about the Gilder Center (its whole `longDescription`, and its hero file is literally `amnh-gilder-center_hero.webp`) yet sits at the **AMNH** coordinate as one of that place's six members. Moving it would take AMNH 6 → 5 — **the owner's call**; its hero is a near-black podcast title card, so nothing photographic is lost by leaving it.
...
|---|---|---|
| **`backend/catalog_snapshot.sql`** | **The catalog RPC stops timing out (it fails ~1 call in 3 today)** | **The app keeps falling through to the gh-pages mirror on a third of launches.** ✅ **THE ORDERING GATE HAS CLEARED — [#742](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/742) merged 2026-09-07, so this is safe to paste NOW.** ⚠️ **Still unapplied, and that is measured rather than assumed (2026-09-07, after #745):** `catalog_snapshot_age()` and `get_catalog_built()` both return **404 `PGRST202`** on the live database, so the old builder is still what `get_catalog()` runs — and it still fails, **2 of 5 spaced calls returning `500 / 57014 statement timeout`**. ⚠️ **The seed cannot do this for you:** its refresh call is guarded by `to_regprocedure(...)`, so a content merge succeeds, raises a notice, and leaves the slow path in place — which is exactly why content lands live (`linkPins` 1,306) while the timeouts continue. ⚠️ **How to tell it worked: `get_catalog()` should return in well under a second. The LATENCY BAND, not the pass/fail count, is the signal** — a 33%-failure endpoint comes back clean by chance often enough to fool you, so a clean run still measured in seconds means the snapshot is NOT being served. |
