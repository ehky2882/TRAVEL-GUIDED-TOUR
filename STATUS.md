...
**⚠️ Re-derived from `Tours.json` on 2026-09-01 (session 135b, NOT yet merged): 1,552 tours + 696 link pins, 246 maker rows, 2,620 stops, **92 places**.** Branch `claude/tour-links-yg5yw2`, restarted off merged `main`. Five owner place decisions applied in one pass.
  ...
  - **✅ `check-place-candidates.py` 6 EXACT → 1.** ⚠️ **The one left is Toronto's Cube House pair from [#698](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/698) — not this session's, and the owner has not been asked about it.** NEAR unchanged at 33.
  - **✅ Eastern State has a SOURCED CC0 hero** (gh-pages `9252daff`), natively 4:3 so nothing is cropped and nothing upscaled; no CREDITS row. ⚠️ The other three borrow a member hero — borrowed-hero count **24 of 92**.
