...
|---|---|---|---|
| **141** | `bottom-module-missing-45z6ep` | The same fix at **1.1.2** (`e6570bdc`) — 140's payload plus the version bump | ⏳ dispatched 2026-09-04, run number **read back, never predicted**. ⚠️ **The honest test is REPEATED cold launches** (force-quit, relaunch, 8–10 times), some of them while the phone is busy, plus a background→foreground round-trip — the bug is intermittent, so one clean launch proves nothing. **New: if the bars ever do go, backgrounding and returning should bring them back** |
