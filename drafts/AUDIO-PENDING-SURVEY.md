...
**Last verified:** 2026-08-26 (🇩🇰 **Copenhagen COMPLETE** — 40 tours live under Atlas Studio CPH, the 34th Atlas maker, Denmark's first city and the catalog's 22nd country; catalog 1552 tours / 1924 stops. **Never in this table** — the thirteenth consecutive complete drop (audio + scripts + images in one Dropbox `/scl/fo/` drop, 103 MB, first try; 155 images already 1200×900, 40 clean/TTS-safe pairs 1:1, zero byte-duplicates) and wired the same day.)

> 🔴 **THE QUEUE MAY NOT ACTUALLY BE EMPTY — an ATLANTA batch appears to be staged and has no row here.** gh-pages commit `c533f3c4` (2026-08-24) pushed *"Atlanta tour images: 9 subjects (41 files)"* (Historic Fourth Ward Park, Krog Street Tunnel, The Temple, Georgian Terrace, the Atlanta Flatiron, the Hurt Building, Big Bethel AME, Prince Hall Masonic Temple, Ebenezer Baptist). Whether scripts exist, and on which branch, was **not** established. **This is the Dubai failure of 2026-07-27 repeating exactly** — a staging session that pushed assets without landing its tracker row on `main`. Re-derive before telling anyone the queue is empty: `git ls-tree -r --name-only origin/gh-pages | grep -iE 'atlanta|krog|ebenezer'` and check the drafts branches.
...
_(✅ 🇦🇺 **Melbourne = DONE (2026-08-11): 35 tours LIVE** — 34 single-stop (geofenced 30 m) + **1 walk** (`melbourne-fedsquare-walk` "Federation Square", intro+6, 550 m — the walk arrived with 7 content stops and no intro track; stop 01, Flinders Street Station under the clocks, wired as the manual stop 0 per the Chicago Riverwalk precedent) under new maker **Atlas Studio MEL** (`117eebb3-5e5d-51e9-83cf-8ae0f3747bf0`) — the **28th maker and the first Australian city**. 41 MP3s, 4,987 s (~1h23m). **Never in this table** — arrived complete and wired the same day. ⚠️ Four Pillars Gin Distillery + TarraWarra Museum of Art ship with `city: "Healesville"` (Yarra Valley, ~50 km NE). 🐛 One wrong staged image (the Royal Arcade's Gog & Magog clock filed under Flinders Street Station) caught by the open-every-image audit and dropped **pre-upload** — never staged to gh-pages.)_
...
⚠️ **Imperial Spine stop 4 (Lustgarten) shipped with `museum-island_3.webp`, not the staged `museum-island_hero.webp`** — the hero is the **Bode Museum from the water**, 600 m north, while the script names the Altes Museum colonnade, the Dom and the palace facade. Same finding as [PR #475](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/475)'s 94-image reuse audit (still open at wire-in), verified independently by opening the file. **All 21 Berlin walk-stop images were opened and checked against their scripts; the other 20 are correct.** ✅ **The single-stop `museum-island` hero was swapped too, on owner instruction (2026-08-12)** — now `museum-island_3`, done as a true swap so the Bode shot keeps a gallery slot. `heroImageURL`, the gallery entry and `stop0.imageURL` all move together, and `_3` must leave the gallery or the hero-in-gallery check hard-errors.
