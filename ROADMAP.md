...
| **4. Maker dashboard** | Phase 1 single-piece creation (record/import audio, pin+radius, photos, transcript, metadata, submit→review), then Phase 2 multi-stop | 🟡 **IN PROGRESS (session 51).** Architecture: the **Me tab is a Profile = a maker page** (one `MakerView`, modes; owner: "each maker is a user too"). Shipped to `main`: **profile tab (#300)**, **standalone public creator screen via `MakerPresenter` (#302)**, **create/edit creator profile → `makers` row (#304, in review)** — first client→Supabase content write; `MakerProfileService`. Owner decision: one login = one profile. **Next:** create-a-tour form (metadata + map pin/radius → `draft`), then audio/photos/transcript/submit. Storage buckets + owner RLS already live (PR #222). `main` is at build 61 (carries share #297); cut a fresh build after #304 to get maker authoring on device. |
