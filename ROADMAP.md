...
**Status (2026-06-01, session 15):** every V1 functionality milestone is shipped on `main`. **TestFlight 1.0 (22) is live** (uploaded 2026-06-01 evening) — carries the eleven-item home-screen polish pass via [PR #103](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/103) (default location at launch + Look Around removal) and [PR #104](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/104) (drawer `.large` detent capped below the search bar + chip row, placecard fill unified to `AtlasColors.secondaryBackground`, bookmark *AFFORDANCE* replacing the hero-corner category chip, animated `.` / `..` / `...` ellipsis in the tours-in-view header while panning). Build bumped 21 → 22 via [PR #105](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/105). A real geometry bug was caught mid-review via bright-color diagnostic — `BottomSheet.heightForDetent(.large)` was double-counting the top safe-area inset because the `GeometryReader` bounds already start below it while `safeAreaInsets.top` still reports the device value; fix removed `topInset` from the `.large` formula and its two mirror calculations. Earlier the same day, session 14 (web/PM) added 6 Porto-area tours (PR #100) and cut build 21 (PR #101); session 13 (web/PM) added 10 NYC tours and fixed hero URLs + two coordinates. Catalog still **97 tours, 3 makers**. Earlier status follows.
