...
**Status (2026-09-23, session “SCALE PINNED TOURS”, overnight):** web session (**content**). An OSM name sweep of the 2,175 link pins Wikidata cannot match: **557 independently confirmed**, **11 pins moved** (8 from the sweep, 3 where OSM and Wikidata agreed against us), 61 left alone and recorded, 12 Atlas tours listed for the owner. Places **435** after #1073. See `archive/HANDOFF-260922-titles.md` Part 3.

**Status (2026-09-22, session “SCALE PINNED TOURS”, titles pass):** web session (**content**). Asked *"are we really done with existing content?"*, and the answer was no. **269 caption-titled pins renamed** from their own captions (caption titles 355 → 89), **331 stale stop titles synced**, **8 place descriptions** written, **9 coordinates corrected** where OSM and Wikidata agreed against us (The Warrington place among them), and the spine cache refreshed: CONFIRMS **1,861 → 1,923**. Four new place candidates and the Petit Palais question go to the owner. See `archive/HANDOFF-260922-titles.md`.

**Status (2026-09-22, session “SCALE PINNED TOURS”, [#1060](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1060)–[#1068](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1068) — MERGED):** web session (**tooling + content**) — **every place decision the tools can make on their own is now made: NAME and EXACT empty, `join-places.py` at 100 m finds nothing, `make-places.py` has nothing to mint. 421 → 431 places.** The owner's Amap links resolved four China pins, each needing GCJ-02 → WGS-84 (`scripts/gcj02.py`, 8/8) — on the Seashore Library the raw number would have moved an **already-correct** pin 551 m, and the Beidaihe "shared centroid" turned out to be one pin's correct point given wrongly to a building 1.5 km away. Seventeen places and joins from TIGHT and NEAR, each on an independent source (OSM, Wikidata, GSI); a Barbican Centre whose pins sat on a **bike dock**; a MahaNakhon join that ran the other way because the stray pin was the accurate one. 🔴 **`check-place-candidates.py` had been crashing on any EXACT group and reading as a pass** (#1060, exit 1 = "candidates found"); it now exits 2. **CI now reports entries that should join an existing place** (#1068), and fails on a crash or a count-less exit 0. 🔴 **CAM was made worse and then fixed**: an Atlas stop is its audio trigger, and #1066 moved it off the canopy its caption names. Stale PRs #1015/#1019/#1020 closed; **"#1019 must not be merged" was never the owner's instruction** — a session's inference, carried forward until it read like one. The contributor skill told sessions to reseed Supabase by hand (it is automatic) and never mentioned China — both corrected. Detail: `archive/HANDOFF-260922-places.md` parts 3–5.

**Status (2026-09-22, session “SCALE PINNED TOURS”, [#1059](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1059)):** web session (**tooling + content**) — **asked to create two place candidates; one already existed, and the defect sitting beside it was invisible to every check we have.** `Walden 7` has been a place since #1022 — a **fourth** entry on its *identical* coordinate simply was not a member of it, because every tier of `check-place-candidates.py` hunts for sites with **no** place page, so the moment a place exists that site reads as finished. **After a pin batch, `join-places.py` must run too**; Rule 8c only covers *should a place exist*, never *is this one complete*. Created `Franklin D. Roosevelt Four Freedoms Park` from two entries 14.6 m apart with nothing else within 400 m. 🔴 **Neither title resolves in Wikidata, so the spine could offer nothing and the coordinate had to come from a footprint**: OSM way `108315458` files the site under a **third** name, `FDR Four Freedoms State Park`, and **both pins fall inside its polygon — as does Wikipedia’s own point 131 m away**, because the park is a 229 m wedge. Distance would have called that a serious error; the polygon says all three are in the park, which is rule 8d arriving in a new form — **the locality can be the subject’s own footprint**. Then the finding that matters: `join-places.py` proposed a **third** join, `Dead Rabbit` → `Fraunces Tavern` at 18.2 m — different bars at different addresses, matched because the Dead Rabbit’s description says *“in the historic Fraunces Tavern **district**”*, a **locator**, and a join **moves** the member onto the place. Instrumenting the caption tier against the live catalogue: **seven rows matched, one was right, and four were refused only by `declined_pairs()` — rulings the OWNER had already made one at a time**, every one of them a part-vs-whole. 🔴 **So the tier’s clean record was the owner’s judgement replayed, not a property of the tier: a refusal list cannot refuse a case nobody has judged, and the first such case went straight through.** `title_introduces_another_name()` now strips the place’s name out of the entry’s title and refuses if a name is left over — keeping `@centrepompidou is the coolest museum ever`, refusing Dead Rabbit, the Cosmati Pavement, `Bar Luce at Fondazione Prada` and the library that once matched its own TOWN, all cases the file’s own docstrings record as having gone wrong. Ruled the five remaining spine disagreements: **all pub-name collisions, with all four of our coordinates 0 m from OSM’s** (Wikidata holds **three** items called `The Roebuck`, and OSM a **fourth** 4.6 km away in the same London borough) — the second sweep running where hand-sampled top findings contained **no catalogue error**. ⚠️ **A fourth “true of the wrong population” was caught before it shipped**: a grep reading one line instead of two nearly recorded 13 of 15 mutation harnesses as not failing on a skip — they all do. Verification: `join-places.py` **41/41 selftests, 21/21 mutants with 0 SKIP** (three pre-existing mutants quoted the predicate edited here and were re-anchored in the same commit), `check-place-candidates.py` 51/51, validator exit 0 with control clean, spine audit **exit 0 — `DISAGREES: 0 unexamined`, `STALE: 0`, CONFIRMS 1,860**. **417 places.** Two NAME candidates left for the owner (`Brunswick Centre`, `Smithfield Market`). Detail: `archive/HANDOFF-260922-places.md`.

**Status (2026-09-22, session “SCALE PINNED TOURS”, [#1055](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1055) + [#1057](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1057) + [#1058](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1058) — MERGED):** web session (**tooling + content**) — **the catalogue got its addresses back.** `make-link-pin.py` had stored `caption[:140]` and discarded the rest; **2,364 captions sat at exactly 140 characters**, nearly all mid-word, and because creators put their address LAST the cut landed on the most valuable line — *Modern Coffee House* shipped **22.9 km wrong**, on Staten Island, because the postcode separating two East Broadways was in the deleted tail. Re-read every post: **2,351 recovered and 897,983 characters restored** — 🔴 **though the headline first reported for this, ‘location markers 535 → 943’, is true of the caption FIELD and misleading about the evidence available: `longDescription` was never truncated and already carried 913 of them, so the addressable base gained 32, not 408. The real win is the display — 2,320 captions ended mid-word on screen — and the audit now reads BOTH fields**. 🔴 **The prefix guard is the whole safety property** — a refetched caption is accepted only if the stored 140 characters are a PREFIX of it, compared on **normalised** text, because the pipeline stores whitespace collapsed and a raw comparison would have rejected every Instagram post with a line break, i.e. the entire platform; one post failed it, having been edited since. 🔴 **And the egress warning I gave before running it was wrong by 15x**: I said ~+12% on a payload with two overage notices and it cost **+0.8% — 35 KB, not 300** — measured at gzip level 1, on a day I had quoted the measure-compressed rule in my own commit messages; captions compress hard because they are the most repetitive text in the catalogue. Then built the audit the recovery was for: `check-caption-address.py` asks whether the city a caption names is the entry's own, using **no external source** (the gazetteer is the catalogue's city list) and **testing LOCALITY rather than distance**, per rule 8d — an address-only geocode had put **Cube House 1,421 m from a CORRECT pin**, and Sun Tower was settled by district, not distance. **15 of 551 disagree and not one is a wrong pin**; what it did find is **`St Louis` and `St. Louis` stored as two cities**, the punctuation blind spot in the accent-folding written after the São Paulo merge. Two findings cut against the day's own premise: **the creator's caption can be wrong** — *Rock Pools at Tai Long, Sai Kung* says **Sai Wan Ho**, 17 km away, an urban MTR district with no rock pools, while our point reverse-geocodes to 下鹿湖, Sheung Luk Wu, Sai Kung, exactly where people cliff-jump — and 🔴 **a pipeline fix does not reach work already in flight**: the day after the recovery left 24 captions at the cut there were **111**, 87 of them from a batch built on a branch taken before the fix, with every check green and the validator clean. It surfaced only because a count was being re-derived by hand. 86 recovered, and `refetch-captions.py --check` now **fails CI** on any cut the recovery cache does not excuse, stating its own scope in the output (`24 sit at 140 · 10 have no source URL · 14 excused · 0 NOT excused`). Second pass over the 79 city-outlier flags against full captions: **no coordinate errors**. Verification: `refetch-captions` 16/16 + 12/12, `check-caption-address` 22/22 + 17/17, audit board 30/30 + 21/21, validator 0 errors, control clean. Detail: `archive/HANDOFF-260922-captions.md`.

**Status (2026-09-21, session “SCALE PINNED TOURS”, [#1047](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1047) + [#1048](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/1048) — MERGED):** web session (**tooling + data**) — **the coordinate audit reached ZERO unexamined disagreements, and the last two errors were found by a check that asks no external source at all.** Fifteen coordinates moved across the day. 🔴 **The day's first finding was that the audit had been LYING**: `spine-match.py` printed cached distances against coordinates the catalogue no longer held, so nine fixes from the night before all read as still broken — the digest that invalidates the lookup cache was consulted by the *fetcher only*. A stale number is a confident wrong answer, not a weak one, so such rows now carry **no distance at all**: banded `STALE`, out of the denominator, exit 2. **It then went red on this session's own PR** for exactly that reason, which is the guard working. 🔴 **Three ways the cross-check lied**, each now a lesson: *two sources agreeing is NOT corroboration when the name is generic* (five of seven London pubs called wrong by up to 8.7 km — OSM and Wikidata had each resolved *"The Royal Oak"* to a namesake and **landed on the same one**); *a recorded verdict can be wrong, and suppression makes that permanent* (two entries ruled "ours is right" were 1,316 m and 966 m out, hidden by that morning's own suppression — `--show-ruled` now lists what a verdict hides); and *`UNMATCHED` is sometimes not a coverage gap* (83% of a sampled sweep had no OSM result because the titles are **stories** — *The Million Dollar Corner*, *A Greek Goddess on Fifth Avenue* — which no gazetteer will ever match). **That last one is what forced the new check.** `check-city-outliers.py` scores an entry against the **median** of its own city's **other** entries, scaled by that city's **own** spread — **no gazetteer, which is the point** — and found **"Great Court, British Museum" 58 km away in a business park in GREAT NOTLEY, Essex** (a geocoder had matched the wrong word, and the entry was `UNMATCHED`, so every lookup was silent on it) and **"Modern Coffee House" 22.9 km out on Staten Island**. 🔴 **That second one exposed something larger and NOT fixed here: 2,375 of 5,463 captions are exactly 140 characters and 2,320 end mid-sentence** (`make-link-pin.py:621` is `caption[:140]`), and **creators put the address LAST** — the postcode that separated two East Broadways was in our own data and we had cut it off. Raised as an owner decision; the caption is what a user reads. ⚠️ **Two method lessons cut opposite ways.** Sweeping the address method across all 13 captions that survive the cut found **no catalogue errors and four false positives**, one of which — *Cube House, Toronto* — would have **moved a correct pin 1.4 km** on an address-only geocode, when searching the NAME lands on our exact point. But *Ginza Kojyu* was settled **by** its address, name-first then **GSI** (authoritative where Nominatim is not), 33 m from Wikidata against 258 m for ours. So it is an **order**, not a preference: name first, address to disambiguate — and **independence is a property of the NAME, not the count of sources** (OSM and Wikidata agreeing within 34 m on *Nostra Signora del Cadore* **is** corroboration, because there is no second one to land on). Also: place membership is stored **both** directions, and a one-directional guard broke the catalogue on the twelfth move — `validate-tours-mirror.py` returned `control DIRTY` and refused to print a verdict, which is why it was caught in seconds; **the eleven earlier moves were safe by luck**. Two further classes probed and **closed empty**: the geocoder-centroid cluster (0 of 126 identical-coordinate groups survive) and coordinate precision (a weak signal — trailing zeros are indistinguishable from rounding). Verification: `check-city-outliers.py` **21/21 selftests, mutants 17/17 with 2 PROVEN EQUIVALENT** (the `min_others` rule is written twice, so neither copy can be broken alone — the harness asserts the equivalence rather than trusting it); `spine-match.py` 81/81 + 27/27; `check-same-name.py` 20/20 + 14/14; validator **0 errors, control clean**; audit **`DISAGREES: 0 unexamined`, `STALE: 0`, CONFIRMS 1,824**, 193 verdicts. Detail: `archive/HANDOFF-260921-3-city-outliers.md`.

**Status (2026-09-20, session “SUBMIT 1.1.3” — SUBMITTED TO APPLE):** local session (**release, no code**) — **1.1.3 is with Apple, `WAITING_FOR_REVIEW` since 2026-09-20 22:13 UTC, on the TestFlight build that already existed: 176**, owner-tested on device (*“looks good! merge it”*). **No new binary was produced.** 🔴 **`fastlane release` and `release.yml` could not be used** — both call `build_ipa`, so they would have compiled a fresh binary and submitted *that* rather than the one the owner approved, and `upload_to_app_store` runs with `force: true` skipping only screenshots, so it would also have pushed `fastlane/metadata/` unprompted. Driven instead through six verified App Store Connect REST calls (create version → What's New → attach build → phased release → add item → submit). **Build 176 is current, checked not assumed:** `git diff a0671e39 HEAD` over `*.swift`, `*.pbxproj`, `Info.plist` and `Assets.xcassets` is **empty** — everything merged since is content, scripts and docs, which reach phones over the air. **Screenshots carried forward untouched, with an identical `sourceFileChecksum` on all ten**, and nothing was uploaded: `fastlane/screenshots` is empty and gitignored on purpose, so a lane that “uploads” them replaces the live set with nothing. **No IAPs attached** — eleven tiers sit in `READY_TO_SUBMIT` by design and the App Review notes already tell Apple they are out of scope. Release is **MANUAL** and phased release is **ON**, so approval does *not* put it on the store. **Owed next: the owner presses “Release this version”, then a session bumps `MARKETING_VERSION` 1.1.3 → 1.1.4** — a released train closes and the next upload is refused with 90186, exactly as build 173 was three days ago. Handoff: `archive/HANDOFF-260920-submit-1-1-3.md`.

**Status (2026-09-17, session “SCALE PINNED TOURS”, [#988](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/988) — MERGED):** web session (**tooling + data**) — **the place spine's first real payoff: an audit that asks whether a coordinate is RIGHT.** `check-coordinates.py` asks whether a point is plausible and `validate-tours.swift` whether it is well-formed; neither catches a point that landed on the wrong building in the right neighbourhood, which is what all six of the day's earlier errors were. `spine-lookup.py` (network, cached, resumable) + `spine-features.py` + `spine-match.py` (offline, CI-wired) sweep **4,566 entries with zero failures**. 🔴 **A NAME lookup, not the tiled harvest `docs/place-spine-design.md` § 4 specified** — tiling is affordable (1,612 tiles of 2 km, ~67 min, measured) and structurally cannot catch these errors, because *a tile is drawn around the coordinate the entry already has*. **Result: CONFIRMS 1,667 · EXTENDED 114 · UNMATCHED 2,507 · DISAGREES 86.** 🔴 **Five of the top findings were opened by hand and NONE was a catalogue error** — two were Wikidata's own error (Prada Aoyama, Peréz Art Museum Miami), two a different place of the same name — because the errors that prompted the work were already fixed and the false-positive classes now dominate. **The durable outputs are the 1,667 confirmations and the per-creator coverage** (Atlas Studio LDN **86%**, `@pasttworld` 85%, NYC 75% against `@jacksdiningroom` **7%** and `@japanbyfood` **1%**), which settles the routing question: food needs `parse-caption-address.py` + GSI, never this. Four guards each paid for by a live failure (a same-name item **9,570 km** away; `Brooklyn Museum` returning its own art school 75 m away; Han script not naming its own language; containment matching a town to the library inside it). Also records that **a background process does not outlive an idle session here** (the first sweep died five minutes after the turn ended, at 316 of 4,566) and that **the faster endpoint was the wrong one** — `wbsearchentities` 429s hard in bulk where WDQS does not, so batching beat switching, 3 s/entry → 0.31 s. Verification: 43/43 · 52/52 · 6/6 selftests, **mutation-tested 8/8 and 14/14**, which found **three checks that passed while unable to fail**. Detail: `archive/HANDOFF-260917-2-spine-audit.md`.

**Status (2026-09-17, session “SCALE PINNED TOURS”, PRs #934 #936 #941 #950 #952 #958 #971 #972 — all MERGED):** web session (**content + tooling + backend**). Two things shipped. **(1) The place checker now tests the rule instead of a proxy.** `docs/places.md` Rule 1 is *two names for one thing*; `check-place-candidates.py` was testing *coincident coordinates*, so everything the proxy missed stayed invisible however often the catalogue was scanned — which is why the owner kept finding candidates by eye. [#950](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/950) adds a **NAME tier** (set **equality** not containment — containment matched *“Akihabara”* to *“Gyukatsu Ichi Ni San, Akihabara”*; the **union** of both entries’ city words, without which the tier silently lost Asakusa Underground Street; 1,500 m bound; NAME reported first and suppressed from TIGHT/NEAR; selftests 35 → **42**) and a **`place-candidates` CI job**, so rule 8c no longer depends on a session remembering it. **EXACT 18 → 0, NAME 11 → 1.** Clearing the backlog ([#941](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/941) 9, [#952](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/952) Grace Farms, [#958](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/958) 10, [#936](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/936) 7) minted **29 places** — and found the **tour** displaced in **13 of 15 groups** (Grace Farms **6.1 km**, Geisel **5.4 km**, La Collina **1,440 m**, Casa de Vidro **833 m**, Asakusa **718 m**), every one precise and in the right city, which is why no check had ever caught them. #936 also closed the other half of #930: it had moved 26 coordinates and **zero** centroids, leaving 36 entries with the pin in one place and every distance calculation in another (**4,270 m** of drift). **(2) Phase 3, server half — the seed can finally delete** ([#971](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/971)). `seed_from_toursjson.py` was upsert-only, so a deletion reached the gh-pages mirror and the bundled seed and **never reached Postgres, the app’s primary source**. It now emits a guarded prune **keyed on the maker**, so in-app uploads are untouched, with `PRUNE_CEILING = 300` inside the transaction and `MIN_SANE_TOURS = 1000` in `validate()`. ✅ Verified on production before shipping (all 4,382 rows `published`; the one non-catalogue row is *Wes the Wanderer*’s upload, protected by the maker key) **and after merging on a real run** — `publish-catalog.yml` on `5a80849e` green, live count `Tours.json` 4,381 / Postgres 4,382 / difference 1: it ran and deleted nothing, as predicted. 🔴 **The client half is NOT built** — `removedIds` is still always empty, so a deletion still reaches a phone only via a full download; reconciliation in `RemoteCatalogLoader` is designed (83,542 bytes, chosen over tombstones) but is app code needing TestFlight + device review. Also [#934](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/934) (the last two checkers stamp their output, with a selftest that walks `scripts/` and names the offender — the first version of that guard *could not fail*, matching a commented-out import) and [#972](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/972) (Monkey King hero; the pasted CDN **resizer** URL served 1200×800 against a 7200×4802 original). Detail: `archive/HANDOFF-260917-places-and-deletion.md`.

**Status (2026-09-13, session "LOCAL - USERNAMES", PRs #841 #844 #846 #847 #849 #851 #854 #855 — all MERGED):** local Mac session (**code + backend**). **Unique usernames, shipped end to end.** Owner requirement: *"every user [must] have a unique username."* Every `makers` row now has a `platform` and a `handle`, unique as a pair; display names are unchanged and may repeat. There were three owner SQL pastes (`usernames.sql`, a one-time "New Creator" → `@user.…` rename, the `former.` prefix reservation), each verified from outside Supabase, not taken from its receipt. The app on build 153 (Edit Profile username field with live availability, `@handle` under names, handle search) was device-tested by the owner. `merge-link-pins.py` now warns when a new pinned creator's handle is held by a Dozent account. **No account was deleted or merged.** ⚠️ Vercel was rate-limited on every PR, which silently stalled watchers that wait on all checks. Handoff: `archive/HANDOFF-260913.md`.

**Status (2026-09-12, session 160, [PR #835](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/835), branch `claude/filtering-chip-system-htjq7k` — OPEN, build 145):** web session (**code**) — **the home map's filter row becomes a door and four chips.** The flat eighteen-toggle tag row is replaced by **All · Format · Price · Dozents · Tags**, each chip opening a panel, every panel multi-select, every option carrying a **contextual** count. Designed in Claude Design first against the owner's own AllTrails screen recording (~30 artboards, iterated through the owner's artifact comments) and only then built; the spec of record is `docs/filter-chips-design.md`. New `Models/TourFilter.swift` reduces the whole row to one pure `Equatable` value carrying the D6 combine rule — **any within a group, all across groups** — so it is testable without a catalogue (17 tests, weighted to the combine rule and the counts, the two things that would break invisibly). 🔴 **The design nearly shipped with no Price chip on a false reading:** `Tours.json` reports `priceTier: nil` for every tour **and always will**, because `seed_from_toursjson.py` omits the column deliberately — the live database has **66 paid tours**. The chip exists because the owner asked for it, not because the check caught it. **Format splits seven ways** (audio stop · audio walk · Instagram posts · Instagram Reels · TikTok · YouTube Shorts · YouTube videos), in each platform's own words. Chip heights are one decision: **44 pt in the row** (Apple's minimum, the one-handed-while-walking surface) and **32 pt in the panels inside a 48 pt frame**, because a thumb feels the vertical pitch rather than the drawn height. ⚠️ **Never compiled** — no Mac in this session, so CI on the PR is the first honest check and build 145 on the owner's device is the second. **Sort on the drawer header is designed and deliberately not built** — the one open item. Handoff: `archive/HANDOFF-260912-3.md`.
...
- [x] **In-app account deletion** (Apple Guideline 5.1.1(v)) — **shipped 2026-09-13, [PR #853](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/853) → `0438466f`; server half live and owner device-verified on TestFlight 155.** Open: the post-deletion catalogue snapshot refresh did not run (see STATUS entry). Settings → Account → Delete account → `delete-account` Edge Function + `backend/account_deletion.sql`. Owner rule: remove unsold tours, keep sold ones as "Former creator". See `docs/account-deletion.md`.
