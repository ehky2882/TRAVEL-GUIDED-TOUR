...
**Status (2026-07-04, session 56):** code session — **perf fix: Home + its map now stay alive across tab switches, killing the return-to-Home lag** ([PR #347](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/347), squash `b3441c6`, owner-reviewed on device → merged). `ContentView.tabContent` was a `switch` that unmounted `HomeView` on every tab-away, so returning rebuilt the SwiftUI `Map` from scratch (reload tiles + re-add/re-cluster ~509 stop annotations) and lost the `@State` camera to the NYC default. Profiling (temp timing, reverted) confirmed the Swift recompute is negligible (`filteredTours` ≈0.02 ms, cluster ≈1–3 ms) — the cost is the map's UIKit reconstruction. Fix: `tabContent` → a `ZStack` keeping `HomeView` permanently mounted (visibility toggled via `.opacity`/`.allowsHitTesting`/`.accessibilityHidden`), Library/Me overlaid opaque when active; off-screen Home made inert (`allowsHitTesting(false)` + a new `isActive` guard on the camera callbacks) so it isn't burning CPU/battery. Preserved drawer detent, `HomeSharedState`, tour/maker UIKit layers, deep links. `test_sim` **140/140**; CI green; Home + overlay composition sim-verified, the tab-switch *timing* owner-device-verified (the modal-window tab bar isn't automatable). **App code merged to `main` but NOT yet on TestFlight — needs a build cut** (owner cutting one in a parallel session). See `archive/HANDOFF-260704-4.md`. Earlier status follows.
