...
**Status (2026-07-25, session 70):** CI/docs session — **every TestFlight build now carries its own description, inside TestFlight.** Owner ask: *"i need to know what new features are added when we cut new builds."* [PR #425](https://github.com/ehky2882/TRAVEL-GUIDED-TOUR/pull/425) (squash `4c20bd3`, CI/docs auto-merge, three checks green) adds a post-upload `testflight.yml` step that writes the build notes into the build's **"What to Test"** field via fastlane **`set_changelog`**, using the **App Store Connect key already in secrets** — no new secrets, no owner setup. Notes resolve from the *Build notes* input → **PR title + body** (label trigger) → commit subject, so the field is never blank (4000-char cap). **Apple needs ~5–15 min to process a build before notes can attach**, so the step retries 25× / 60s and, failing that, warns and exits 0 rather than failing a build that already uploaded fine; job timeout 40 → 70 min. Marketing version is grepped from the pbxproj (highest of the test target's 1.0 / app target's 1.1) so it survives future bumps. Closes the "future enhancement" note in `docs/testflight-ci.md`; automation rule #9 updated. **Verify on the next build cut** (tap the build in TestFlight — notes may land a few minutes behind the build). Branch-delete owed via GitHub UI. See `archive/HANDOFF-260725.md`. Earlier status follows.
