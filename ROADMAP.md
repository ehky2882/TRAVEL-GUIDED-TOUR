...
| **4. Maker dashboard** | Phase 1 single-piece creation (record/import audio, pin+radius, photos, transcript, metadata, submit→review), then Phase 2 multi-stop | 🟡 **IN PROGRESS (session 51).** Architecture: the **Me tab is a Profile = a maker page** (one `MakerView`, modes; owner: "each maker is a user too"). Shipped: **profile tab (#300)**, **standalone creator screen via `MakerPresenter` (#302)**, **create/edit profile → `makers` row (#304)** — first client→Supabase content write, **create-a-tour draft form (metadata + map pin/radius) → `tours`+`stops` (#307)** with a My-Tours feed (status badges), and **tour editor + audio import → `tour-audio` Storage (#310, in review)**. `MakerProfileService` + `MakerTourService`. Owner: one login = one profile. **Live in TestFlight 1.0 (62)** (owner-confirmed) through #307; #310 audio + the rest not yet in a build. **Next:** in-app recording, photos (PHPicker→crop 1200×900→`tour-images`), transcript, submit (`in_review`) → admin publish. Storage buckets + RLS already live (PR #222). |
