...
| `HANDOFF-260525-2.md` | **Active handoff.** Second session 2026-05-25: shipped PR #68 (`fe11d99`), three connected follow-ups to PR #66 surfaced by the visual review. (1) `SearchBar` switched from `.sheet` to `NavigationLink` so the mini-player + tab bar stay visible while searching. (2) `AtlasTabBar` button row anchored at the same screen-y in both geometries — safe-area inset moved OUT of the painted button area into a separate background rectangle below. (3) Detail screens always render with the full-edge module now via a typed `AtlasModuleGeometry` preference (replaces the deleted `\.atlasIsHomeTab` env value). `AtlasBottomModule.height` math updated for the new outer-gap-plus-safe-area layout. 84/84 tests pass. Auto-merged on green. | 2026-05-25 |
