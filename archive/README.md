...
| `HANDOFF-260524-2.md` | **Active handoff.** Afternoon/evening Mac session 2026-05-24: shipped PR #61 (mini-player end-of-tour state fix — kills the post-tour "Loading…" flicker, adds in-place replay via new `AudioPlayerService.replayLast()`). Sibling session pushed `5db7aaa` to `claude/home-visual-polish-bundle` (drawer-stacking + sizing + tours-in-view iteration) — branch is on origin but **not PR'd yet**. TestFlight 1.0 (7) still queued for owner Archive/Upload. | 2026-05-24 |
| `HANDOFF-260524.md` | Mac session 2026-05-23/24 morning: populated `heroImageURL` for all 38 tours via CC-licensed Wikimedia Commons photos (landscape-optimised batch), bumped build number to 7, opened Xcode for TestFlight upload. Superseded by the `-2` handoff above. | 2026-05-24 |
