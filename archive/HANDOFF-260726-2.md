...
## Final state — TestFlight 1.1 (51), built from `main`

Because the merge swapped in `main`'s bottom-module fix, the shipped combination
differed from what build 50 tested. A build was cut from `main` (`dfc3a5d`) and
owner-verified: **"SHINSEGAE VIDEO WORKS, IS FINE. TAB BAR IS PRESENT FROM FIRST
FRAME. DEAD TAB SEEMS FINE."** That closes the one gap this session couldn't
otherwise close — my saving work and the parallel session's window fix had never
run in the same binary until then.
