# HANDOFF — 2026-08-27 (session 117, web/PM: content)

**⚠️ Renumbered on merge.** This was written as `HANDOFF-260827.md`; a parallel session shipping
**nineteen San Francisco link pins** (#626) claimed that name first and landed on `main`, producing
an add/add conflict — both real, neither wrong. Theirs keeps `-260827`, this one became `-2`.
Both also called themselves "session 116"; this one is relabelled 117. **Check `archive/` for the
next free suffix before writing, and expect to renumber if a merge collides.**

**Ten Atlanta TikToks; nine shipped, and the owner pulled the tenth.** The owner sent ten share links under the heading
*"Atlanta Architecture"* — URLs and nothing else, no coordinates and no captions. Branch
`claude/new-tour-links-yr5o7r`, restarted clean off `origin/main` (`c5e8862`). **Post-merge with #626: linkPins 76 → 85, makers 90 → 99.** Content only — no Swift, no SQL, no build. **NO PR OPENED** (this session's
...
**⚠️ THE CONSEQUENCE, WHICH IS NOT A DEFECT BUT IS WORTH THE OWNER KNOWING.** Both protected files
were written before the stadium pin was pulled, so the suffix genuinely saved both. **With that pin
gone the live overlap is Oakland Cemetery alone: once the staged Atlanta tour batch merges it will
carry an Atlas tour AND a link pin at the same site.** That is exactly what the place layer exists
for — but a place requires **exact coordinate equality** and human approval, so nothing was created
here.
...
## 4. 🔴 THE TENTH PIN WAS PULLED BY THE OWNER — the Mercedes-Benz Stadium frame is probably not a photograph
...
exists** (the Brick Award lesson), so the only options were ship-and-flag or drop it. It was
shipped and flagged on the **DIFC Gate precedent**, where a likely-AI hero was raised to the owner
and kept on their instruction — **and the owner went the other way here: *"Remove Mercedes Benz."***

**🔴 SO DIFC GATE IS NO LONGER THE ONLY PRECEDENT, and the durable rule is the process rather than
the outcome: raise a suspect-synthetic hero and let the owner decide. Do not assume they will keep
it, and do not quietly drop it either.** The pin and its sole creator row (`TikTok @morganjamesjr`,
who had no other content) are removed. **⚠️ Its two gh-pages files are deliberately left orphaned** —
`mercedes-benz-stadium-morganjamesjr_hero.webp` and `avatar-tiktok-morganjamesjr.webp`. Nothing
references them, they cost nothing, and a deletion push onto a branch four other sessions were
writing to buys nothing; the same call the wrong-bridge DuSable file got. The other nine heroes are
ordinary phone footage.
...
- **✅ ALL 20 LIVE URLs HASH-VERIFIED against the uploaded bytes** at 03:20 UTC, about 27 minutes
  after the push. **⚠️ FIVE CONSECUTIVE PAGES DEPLOYS CANCELLED ONE ANOTHER** while the Atlanta tour
  session pushed images one at a time; mine was run 721. **A cancelled deploy is not a lost upload** —
  each commit is an ancestor of the next, so the first run that survives carries them all, which is
  what happened. **Verify by hashing the live URL, never by the push succeeding, and check ancestry
  rather than assuming a cancellation lost the files.**
...
- **🔴 THIS BRANCH MERGED `main` MID-SESSION AND CONFLICTED IN FOUR FILES WITH #626.** A parallel
  session shipped **nineteen San Francisco link pins** an hour earlier. Both batches touched
  `Tours.json`, `CLAUDE.md` and `STATUS.md`, and **both claimed `archive/HANDOFF-260827.md`
  (add/add)** — the exact collision CLAUDE.md warns about. Resolved the documented way: **take
  main's `Tours.json` and re-run the append** (the assembler is idempotent, so this is safe and
  produced 0 id, stop-id or hero-URL clashes against the SF pins), keep **both** Current State
  blocks and **both** branch rows, and renumber this handoff to `-2`. **Post-merge state: 1,552
  tours + 86 link pins + 100 makers (34 Atlas studios, 66 pinned creators), validator still 0
  errors and the same 2 pre-existing warnings.**
