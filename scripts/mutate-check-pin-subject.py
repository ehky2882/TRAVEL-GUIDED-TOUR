...
MUTANTS = [
 ...
 ("gate D: a yes does not withdraw the finding",
  'return "DISPUTED" if gate_d else "CONTRADICTS"', 'return "CONTRADICTS"'),
 ("gate D: it withdraws every finding, answered or not",
  'return "DISPUTED" if gate_d else "CONTRADICTS"', 'return "DISPUTED"'),
 ("🔴 gate D: an UNANSWERED gate D excuses the finding",
  'return "DISPUTED" if gate_d else "CONTRADICTS"',
  'return "CONTRADICTS" if gate_d is False else "DISPUTED"'),
 ("gate D: it can also overturn a CONFIRMS",
  'if gate_b:\n        return "CONFIRMS"', 'if gate_b and not gate_d:\n        return "CONFIRMS"'),
 ("gate D: the replica clause is dropped",
  'or if one is a replica or copy that the other is a copy OF.', ''),
 ("gate D: the former-name/nickname/address clause is dropped",
  '"Answer YES if they are the same thing under two names, a former and "',
  '"Answer YES if they are the same thing. "  # '),
 ("gate D: it is shown the image after all",
 ...
 ("🔴 --all silently drops tours, checking only pins",
  'for entry in (catalog.get("tours") or []) + (catalog.get("linkPins") or []):\n        if entry.get("heroImageURL"):',
  'for entry in (catalog.get("linkPins") or []):\n        if entry.get("heroImageURL"):'),
 ("--all includes entries with no hero, which cannot be asked",
  'if entry.get("heroImageURL"):\n            out.append(entry)',
  'if True:\n            out.append(entry)'),
