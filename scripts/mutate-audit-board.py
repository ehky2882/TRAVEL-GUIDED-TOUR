...
MUTANTS = [
 ...
 ("🔴 hero: the source post excuses a shared photo again (the old bug)",
  'standalone = [e for e in group if not is_walk(e)]',
  'standalone = [] if len({e.get("sourceURL") for e in group}) == 1 else [e for e in group if not is_walk(e)]'),
 ("🔴 hero: a walk is treated as a standalone entry",
  'standalone = [e for e in group if not is_walk(e)]',
  'standalone = list(group)'),
 ("🔴 hero: the same subject twice is reported as a defect",
  'if len({fold(e.get("title")) for e in standalone}) == 1:\n            continue',
  'if False:\n            continue'),
 ("hero: the two-standalone minimum is dropped",
  'if len(standalone) < 2:\n            continue', 'if False:\n            continue'),
 ("is_walk: an explicit kind is ignored",
  'return (entry.get("kind") == "walk") or len(entry.get("stops") or []) > 1',
  'return len(entry.get("stops") or []) > 1'),
 ("is_walk: a multi-stop entry is not a walk",
  'return (entry.get("kind") == "walk") or len(entry.get("stops") or []) > 1',
  'return entry.get("kind") == "walk"'),
