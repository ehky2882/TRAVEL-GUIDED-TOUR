...
struct Tour: Codable {
    ...
    let country: String?
    /// Mirrors `Tour.relatedTourIds` — the "More like this" neighbours,
    /// generated by `scripts/build-embeddings.py --write-related`. Optional
    /// so a tour with no neighbours (or a catalog predating the key) still
    /// validates. Unlike `country`, this one REFERENCES other rows, so it has
    /// real rules below — see "More like this".
...
// `relatedTourIds` is the only tour field that names OTHER tours, so "it
// decodes" is not enough — a dangling id renders a shorter section with no
...
do {
    ...
    for (i, t) in file.tours.enumerated() {
        ...
        var seen = Set<UUID>()
        for (j, raw) in related.enumerated() {
            guard let id = UUID(uuidString: raw) else {
                err(loc, "relatedTourIds[\(j)] '\(raw)' is not a UUID")
                continue
            }
            if id == t.id {
