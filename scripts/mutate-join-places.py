...
MUTANTS = [
 ...
 ("🔴 title-guard: it is not consulted, so a locator caption joins again",
  'and not title_introduces_another_name(r[1], r[2])', 'and True'),
 ("🔴 title-guard: a leftover NAME is treated as filler",
  'if word[:1].isupper():\n            return word', 'if False:\n            return word'),
 ("title-guard: generic words are treated as names",
  'if word.lower() in GENERIC_NAME_WORDS:\n            continue', 'if False:\n            continue'),
 ("🔴 title-guard: a non-Latin word is treated as filler, not a name",
  'if not folded:                     # non-Latin script: treat as a name\n            return word',
  'if not folded:\n            continue'),
 ("🔴 title-guard: ANY word is taken for part of the place name",
  'if folded and folded_place and folded in folded_place:',
  'if folded_place:'),
 ("title-guard: one-letter fragments are inspected",
  'if len(word) < 2:\n            continue', 'if False:\n            continue'),
