...
> ## ⚠️ Rescued from an abandoned branch, 2026-09-16 — read this first
>
> This document was written on 2026-09-14 on `claude/scale-pinned-tours-automation-dba3lx`
> and **never reached `main`**, so nothing in it has been visible to a session or to the
> owner since. It is landed here **verbatim**, with only this banner added, because the
> analysis is sound and the alternative was losing it.
>
> 🔴 **Two things in it are now out of date, and both are marked below:**
>
...
>
> | | 2026-09-14 (as written) | **2026-09-16 (re-derived)** |
> |---|---|---|
> | groups sharing an exact coordinate | 205 | **307**, covering 756 entries |
> | groups larger than 3 | 19 | **28**, covering 130 entries |
> | entries invisible on the map | ~32 | **46** |
> | largest group | 7 (Strahov Library) | **8** |
>
> ⚠️ **And there is a SECOND stacking limit the original audit missed.**
> `TourSetMap.swift:243` is `maxStacked = 3`, but `HomeView.swift:633` is a separate
> `maxStackedPlacecards = 4`. Any fix has to address both or it will half-work.
>
> ⚠️ Every figure here is perishable — **re-derive, never quote** (CLAUDE.md § READ FIRST).
