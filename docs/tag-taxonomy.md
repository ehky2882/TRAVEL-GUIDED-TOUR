> **STATUS: SHIPPED, not a proposal.** Written 2026-06-12 as a proposal; the
> system it describes went live in session 57 (Tag Phase 2). The five facets
> below are exactly what `Models/Tag.swift` implements, and the rules are what
> `scripts/validate-tours.swift` enforces. Kept because it is the only written
> record of the *editorial* rules — in particular that **`Designed by a Master`
> is implied by any `Architect` tag**, which no code checks.
>
> **`Models/Tag.swift` is the authority on the vocabulary** (87 tags live in the
> catalog today). If the two ever disagree, the code is right and this is stale.
